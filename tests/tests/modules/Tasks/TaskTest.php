 ...
 * Class TaskTest
 */
class TaskTest extends \SuiteCRM\Tests\SuiteCRMFunctionalTest
{
    ...
    public function testcreate_export_query()
    {
        ...
        $expected =
            'SELECT tasks.*, users.user_name as assigned_user_name  FROM tasks   LEFT JOIN users ON tasks.assigned_user_id=users.id where tasks.deleted=0 ORDER BY tasks.name';
        $actual = $task->create_export_query('', '');
        $this->assertSame($expected, $actual);
    
        //test with valid string params
        $expected =
            'SELECT tasks.*, users.user_name as assigned_user_name  FROM tasks   LEFT JOIN users ON tasks.assigned_user_id=users.id where users.user_name = "" AND tasks.deleted=0 ORDER BY tasks.name';
    ...
    public function testget_list_view_data()
    {
        ...
        $expected = array(
            ...
            'SET_COMPLETE'    => '~'
                . preg_quote("<a id='' onclick='SUGAR.util.closeActivityPanel.show(\"Tasks\",\"\",\"Completed\",\"listview\",\"1\");'><img src=\"themes/$current_theme/images/close_inline.png?v=")
                . '[\w-]+'
