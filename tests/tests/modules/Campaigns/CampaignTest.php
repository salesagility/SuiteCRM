 ...
 * Class CampaignTest
 */
class CampaignTest extends \SuiteCRM\Tests\SuiteCRMFunctionalTest
{
    ...
    public function testget_list_view_data()
    {
        ...
        $expected = array(
            ...
            'TRACK_CAMPAIGN_IMAGE' => '~' . preg_quote('themes/' . $current_theme . '/images/view_status.gif?v=')
                . '[\w-]+~',
            'LAUNCH_WIZARD_TITLE'  => 'Launch Wizard',
            'LAUNCH_WIZARD_IMAGE'  => '~' . preg_quote('themes/' . $current_theme . '/images/edit_wizard.gif?v=')
                . '[\w-]+~',
    ...
    public function testtrack_log_leads()
    {
        ...
        $expected =
            "SELECT campaign_log.*  FROM campaign_log WHERE campaign_log.campaign_id = '' AND campaign_log.deleted=0 AND activity_type = 'lead' AND archived = 0 AND target_id IS NOT NULL ";
    ...
    public function testtrack_log_entries()
    {
        ...
        $expected =
            "SELECT campaign_log.*  FROM campaign_log WHERE campaign_log.campaign_id = '' AND campaign_log.deleted=0 AND activity_type='targeted' AND archived=0 ";
        $actual = $campaign->track_log_entries();
        $this->assertSame($expected, $actual);
    
        //test with parameters
        $expected =
            "SELECT campaign_log.*  FROM campaign_log WHERE campaign_log.campaign_id = '' AND campaign_log.deleted=0 AND activity_type='test1' AND archived=0 ";
