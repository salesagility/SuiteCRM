...
class AOR_ReportTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testbuild_report_query_join()
    {
        ...
        $actual = $aor_Report->buildReportQueryJoin('contacts', 'accounts_contacts', 'accounts', new Account(), 'custom', array());
        $this->assertSame($expected, $actual);
        ...
        $actual = $aor_Report->buildReportQueryJoin('contacts', 'accounts_contacts', 'accounts', new Account(), 'relationship', array());
        $this->assertSame($expected, $actual);
    }
