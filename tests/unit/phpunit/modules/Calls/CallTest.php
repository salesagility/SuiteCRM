...
class CallTest extends SuitePHPUnitFrameworkTestCase
{
    ...
    public function testcreate_export_query()
    {
        ...
        //test with empty string params
        $expected = " SELECT  calls.*  , jt0.user_name modified_by_name , jt0.created_by modified_by_name_owner  , 'Users' modified_by_name_mod , jt1.user_name created_by_name , jt1.created_by created_by_name_owner  , 'Users' created_by_name_mod , jt2.user_name assigned_user_name , jt2.created_by assigned_user_name_owner  , 'Users' assigned_user_name_mod , LTRIM(RTRIM(CONCAT(IFNULL(contacts.first_name,''),' ',IFNULL(contacts.last_name,'')))) contact_name, jtl3.contact_id contact_id FROM calls   LEFT JOIN  users jt0 ON calls.modified_user_id=jt0.id AND jt0.deleted=0

 AND jt0.deleted=0  LEFT JOIN  users jt1 ON calls.created_by=jt1.id AND jt1.deleted=0

 AND jt1.deleted=0  LEFT JOIN  users jt2 ON calls.assigned_user_id=jt2.id AND jt2.deleted=0
 ...
 AND contacts.deleted=0 where calls.deleted=0 ORDER BY calls.name";
