...
$server->register(
    ...
    array(
        ...
        'row_offset' => 'xsd:int',
        'limit' => 'xsd:int',
        'filter' => 'tns:name_value_operator_list'
    ),
    array('return' => 'tns:get_entry_list_result'),
    $NAMESPACE);
...
function portal_get_entry_list_filter($session, $module_name, $order_by, $select_fields, $row_offset, $limit, $filter)
{
    ...
    if ($sugar != null) {
        ...
        return portal_get_entry_list_limited($session, $module_name, $where, $order_by, $select_fields, $row_offset,
            $limit);
    } else {
        $error->set_error('no_module_support');
...
function portal_set_entry($session, $module_name, $name_value_list)
{
    ...
    if (!$is_update) {
        if (isset($_SESSION['assigned_user_id']) && (!key_exists('assigned_user_id',
                    $values_set) || empty($values_set['assigned_user_id']))
        ) {
            $seed->assigned_user_id = $_SESSION['assigned_user_id'];
        }
        if (isset($_SESSION['account_id']) && (!key_exists('account_id',
                    $values_set) || empty($values_set['account_id']))
        ) {
...
function portal_get_related_list(
    $session,
    $module_name,
    $rel_module,
    $module_id,
    ...
    $list = get_related_in_module("('" . $GLOBALS['db']->quote($module_id) . "')", $module_name, $rel_module, $order_by,
        $offset, $limit);
...
function portal_get_module_fields($session, $module_name)
{
    ...
    if (($_SESSION['type'] == 'portal' || $_SESSION['type'] == 'contact') && !key_exists($module_name,
    ...
    if (is_subclass_of($seed, 'Person')) {
        $returnFields['module_fields']['email1'] = array(
            'name' => 'email1',
            'type' => 'email',
            'required' => 0,
            'label' => translate('LBL_EMAIL_ADDRESS', $seed->module_dir)
        );
        $returnFields['module_fields']['email_opt_out'] = array(
            'name' => 'email_opt_out',
            'type' => 'bool',
            'required' => 0,
            'label' => translate('LBL_EMAIL_OPT_OUT', $seed->module_dir),
            'options' => array()
        );
        $returnFields['module_fields']['email_opt_in'] = array(
            'name' => 'email_opt_in',
            'type' => 'bool',
            'required' => 0,
            'label' => translate('LBL_EMAIL_OPT_IN', $seed->module_dir),
            'options' => array()
        );
    } //if
...
function portal_get_subscription_lists($session)
{
    ...
    foreach ($result['unsubscribed'] as $newsletter_name => $data) {
        $return_results['unsubscribed'][] = array(
    ...
    foreach ($result['subscribed'] as $newsletter_name => $data) {
        $return_results['subscribed'][] = array(
            'name' => $newsletter_name,
            'prospect_list_id' => $data['prospect_list_id'],
            'campaign_id' => $data['campaign_id'],
            'description' => $data['description'],
    ...
    return array(
        'unsubscribed' => $return_results['unsubscribed'],
        'subscribed' => $return_results['subscribed'],
        'error' => $error->get_soap_array()
    );
}
