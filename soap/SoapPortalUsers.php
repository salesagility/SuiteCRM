...
$server->register(
    'portal_login',
    ['portal_auth' => 'tns:user_auth', 'user_name' => 'xsd:string', 'application_name' => 'xsd:string'],
...
function portal_login($portal_auth, $user_name, $application_name)
{
    ...
    if ($user_name == 'lead') {
        ...
        return ['id' => session_id(), 'error' => $error->get_soap_array()];
    }
    if ($user_name == 'portal') {
        session_start();
        $_SESSION['is_valid_session'] = true;
    ...
    $contact = $contact->retrieve_by_string_fields(['portal_name' => $user_name, 'portal_active' => '1', 'deleted' => 0]);
    if ($contact != null) {
        session_start();
        $_SESSION['is_valid_session'] = true;
        $_SESSION['ip_address'] = query_client_ip();
...
function portal_get_sugar_id($session)
{
    $error = new SoapError();
    if (portal_validate_authenticated($session)) {
        return ['id' => $_SESSION['portal_id'], 'error' => $error->get_soap_array()];
    }
    $error->set_error('invalid_session');
...
function portal_get_sugar_contact_id($session)
{
    $error = new SoapError();
    if (portal_validate_authenticated($session)) {
        return ['id' => $_SESSION['user_id'], 'error' => $error->get_soap_array()];
    }
    $error->set_error('invalid_session');
...
$server->register(
    ...
    ['session' => 'xsd:string', 'module_name' => 'xsd:string', 'where' => 'xsd:string', 'order_by' => 'xsd:string', 'select_fields' => 'tns:select_fields'],
...
$server->register(
    ...
    ['session' => 'xsd:string', 'module_name' => 'xsd:string', 'order_by' => 'xsd:string', 'select_fields' => 'tns:select_fields', 'row_offset' => 'xsd:int', 'limit' => 'xsd:int', 'filter' => 'tns:name_value_operator_list'],
    ['return' => 'tns:get_entry_list_result'],
    $NAMESPACE
);
...
function portal_get_entry($session, $module_name, $id, $select_fields)
{
    ...
    set_module_in(['list' => [$id => $id], 'in' => '(' . $id . ')'], $module_name);
...
function portal_set_entry($session, $module_name, $name_value_list)
{
    ...
    $id = $seed->save();
    set_module_in(['in' => "('" . DBManagerFactory::getInstance()->quote($id) . "')", 'list' => [$id]], $module_name);
    ...
    return ['id' => $id, 'error' => $error->get_soap_array()];
}
...
$server->register(
    ...
    ['session' => 'xsd:string', 'module_name' => 'xsd:string', 'module_id' => 'xsd:string',  'select_fields' => 'tns:select_fields', 'order_by' => 'xsd:string'],
    ['return' => 'tns:get_entry_result'],
    $NAMESPACE
);
...
$server->register(
    ...
    ['session' => 'xsd:string', 'module_name' => 'xsd:string', 'rel_module' => 'xsd:string', 'module_id' => 'xsd:string',  'select_fields' => 'tns:select_fields', 'order_by' => 'xsd:string', 'offset' => 'xsd:int', 'limit' => 'xsd:int'],
    ['return' => 'tns:get_entry_result'],
    $NAMESPACE
);
...
function portal_get_related_list($session, $module_name, $rel_module, $module_id, $select_fields, $order_by, $offset, $limit)
{
    ...
    return ['result_count' => $list['result_count'], 'next_offset' => 0, 'field_list' => $field_list, 'entry_list' => $output_list, 'error' => $error->get_soap_array()];
}
...
function portal_get_module_fields($session, $module_name)
{
    ...
    if (!portal_validate_authenticated($session)) {
        ...
        return ['module_name' => $module_name, 'module_fields' => $module_fields, 'error' => $error->get_soap_array()];
    }
    if ($_SESSION['type'] == 'lead' && $module_name != 'Leads') {
        $error->set_error('no_access');
    ...
    if (is_subclass_of($seed, 'Person')) {
        $returnFields['module_fields']['email1'] = ['name' => 'email1', 'type' => 'email', 'required' => 0, 'label' => translate('LBL_EMAIL_ADDRESS', $seed->module_dir)];
        $returnFields['module_fields']['email_opt_out'] = ['name' => 'email_opt_out', 'type' => 'bool', 'required' => 0, 'label' => translate('LBL_EMAIL_OPT_OUT', $seed->module_dir), 'options' => []];
    } //if
...
$server->register(
    ...
    ['session' => 'xsd:string'],
    ['return' => 'tns:get_subscription_lists_result'],
    $NAMESPACE
);
...
function portal_get_subscription_lists($session)
{
    ...
    $return_results = ['unsubscribed' => [], 'subscribed' => []];

    foreach ($result['unsubscribed'] as $newsletter_name => $data) {
        $return_results['unsubscribed'][] = ['name' => $newsletter_name, 'prospect_list_id' => $data['prospect_list_id'],
            'campaign_id' => $data['campaign_id'], 'description' => $data['description'],
            'frequency' => $data['frequency']];
    }
    foreach ($result['subscribed'] as $newsletter_name => $data) {
        $return_results['subscribed'][] = ['name' => $newsletter_name, 'prospect_list_id' => $data['prospect_list_id'],
            'campaign_id' => $data['campaign_id'], 'description' => $data['description'],
            'frequency' => $data['frequency']];
    }

    return ['unsubscribed' => $return_results['unsubscribed'], 'subscribed' => $return_results['subscribed'], 'error' => $error->get_soap_array()];
}

$server->register(
    'portal_set_newsletters',
    ['session' => 'xsd:string', 'subscribe_ids' => 'tns:select_fields', 'unsubscribe_ids' => 'tns:select_fields'],
    ['return' => 'tns:error_value'],
    $NAMESPACE
);
