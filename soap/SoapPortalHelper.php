...
function build_relationship_tree($contact)
{
    ...
    set_module_in(array('list' => array($contact->id), 'in' => "('" . $GLOBALS['db']->quote($contact->id) . "')"),
        'Contacts');
        ...
        if ($module_name == 'Contacts') {
            ...
            if ($module_name == 'Accounts') {
                ...
                if ($module_name == 'Bugs') {
                    ...
                    if (!empty($_SESSION['viewable'][$module_name])) {
                        $list = get_related_list(get_module_in($module_name), new Bug(), $where, $order_by, $row_offset,
                            $limit);
                    }
                } else {
...
$valid_modules_for_contact = array(
    'Contacts' => 1,
    'Cases' => 1,
    'Notes' => 1,
    'Bugs' => 1,
