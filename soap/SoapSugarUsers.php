 ...
 * @param int $max_results -- The maximum number of records to return.  The default is the sugar configuration value for 'list_max_entries_per_page'
...
function get_note_attachment($session, $id)
{
    ...
    return [
        'note_attachment' => ['id' => $id, 'filename' => $note->filename, 'file' => $file],
        'error' => $error->get_soap_array()
    ];
}
...
function search_by_module($user_name, $password, $search_string, $modules, $offset, $max_results)
{
    ...
    $query_array = [
        'Accounts' => [
            'where' => [
                'Accounts' => [0 => "accounts.name like '{0}%'"],
                'EmailAddresses' => [0 => "ea.email_address like '{0}%'"]
            ],
            'fields' => 'accounts.id, accounts.name'
        ],
        'Bugs' => [
            'where' => ['Bugs' => [0 => "bugs.name like '{0}%'", 1 => 'bugs.bug_number = {0}']],
        ...
        'Project' => [
            'where' => ['Project' => [0 => "project.name like '{0}%'"]],
            'fields' => 'project.id, project.name'
        ],
        'ProjectTask' => [
            'where' => ['ProjectTask' => [0 => "project.id = '{0}'"]],
        ...
        'Opportunities' => [
            'where' => ['Opportunities' => [0 => "opportunities.name like '{0}%'"]],
            'fields' => 'opportunities.id, opportunities.name'
        ],
        'Users' => [
            'where' => ['EmailAddresses' => [0 => "ea.email_address like '{0}%'"]],
