...
class M2MRelationship extends SugarRelationship
{
    ...
    public function getLinkedDefForModuleByRelationship($module)
    {
        ...
        //Multiple links with same relationship name
            ...
            if (is_array($results)) {
                // Stic-Custom 20220221 MHP - Change log level to Debug to avoid showing the warning message in Error log level, 
                // which seems excessive and produces frequent output to log files, making difficult to find important stuff in them 
                // STIC#611
                // $GLOBALS['log']->error("Warning: Multiple links found for relationship {$this->name} within module {$module}");
                $GLOBALS['log']->debug("Warning: Multiple links found for relationship {$this->name} within module {$module}");
                //END Stic-Custom
    ...
    public function add($lhs, $rhs, $additionalFields = array())
    {
        ...
        //STIC-Custom 20230415 EPS: The original IFs define an special treatment on every relationship with SecurityGroups except for users and roles
        // The new IFs add the exception when both sides are SecurityGroups, so users, roles and securityGroups will use the standard code when related to SecurityGroups
        // if (get_class($rhs) != 'User' && get_class($rhs) != 'ACLRole' && get_class($lhs) == 'SecurityGroup') {
        if (get_class($rhs) != 'User' && get_class($rhs) != 'ACLRole' && get_class($rhs) != 'SecurityGroup' && get_class($lhs) == 'SecurityGroup') {
            // STIC#1111
            // END STIC-Custom
        ...
        } elseif (get_class($lhs) != 'User' && get_class($lhs) != 'ACLRole' && get_class($lhs) != 'SecurityGroup' && get_class($rhs) == 'SecurityGroup') {
            // STIC#1111
            // END STIC-Custom
