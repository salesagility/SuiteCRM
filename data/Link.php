...
class Link
{
     ...
     * @var string
     */
    private $_bean_table_name;

    /**
     * @var string
     */
    private $_bean_key_name = 'id';
     ...
     * @var bool
     */
    private $_swap_sides = false;

    /**
     * @var bool
     */
    private $_rhs_key_override = false;

    /**
     * @var mixed|string
     */
    private $_bean_filter_field = '';
     ...
     * @var
     */
    private $_duplicate_key;

    /**
     * @var
     */
    private $_duplicate_where;
    ...
    *
    ...
    public function getJoin($params, $return_array = false)
    {
        ...
        if ($this->_relationship->relationship_type == 'one-to-one'
            ...
            or ($this->_relationship->relationship_type == 'one-to-many' && !$bean_is_lhs)) {
            if ($bean_is_lhs) {
        ...
        if ($this->_relationship->relationship_type == 'many-to-many') {
            ...
            if ($bean_is_lhs) {
                $other_table = (empty($params['left_join_table_alias'])
                    ? $this->_relationship->lhs_table : $params['left_join_table_alias']);
                $join .= $join_type . ' ' . $rel_table_with_alias . ' ON ' . $other_table . "."
                    . $this->_relationship->lhs_key . "=" . $rel_table . "." . $this->_relationship->join_key_lhs
                    . "  AND " . $rel_table . ".deleted=0\n";
            } else {
                $other_table = (empty($params['right_join_table_alias'])
                    ? $this->_relationship->rhs_table : $params['right_join_table_alias']);
                $join .= $join_type . ' ' . $rel_table_with_alias . ' ON ' . $other_table . "."
                    . $this->_relationship->rhs_key . "=" . $rel_table . "." . $this->_relationship->join_key_rhs
                    . "  AND " . $rel_table . ".deleted=0\n";
            }
            ...
            if ($bean_is_lhs) {
                if ($this->_rhs_key_override) {
                    $join .= $join_type . ' ' . $table_with_alias . ' ON ' . $table . "."
                        . $this->_relationship->rhs_key . "=" . $rel_table . "." . $this->_relationship->join_key_rhs
                        . " AND " . $table . ".deleted=0";
                } else {
                    $join .= $join_type . ' ' . $table_with_alias . ' ON ' . $table . "."
                        . $this->_relationship->lhs_key . "=" . $rel_table . "." . $this->_relationship->join_key_rhs
                        . " AND " . $table . ".deleted=0";
                }
            } else {
                $join .= $join_type . ' ' . $table_with_alias . ' ON ' . $table . "."
                    . $this->_relationship->rhs_key . "=" . $rel_table . "." . $this->_relationship->join_key_lhs
                    . " AND " . $table . ".deleted=0";
            }
            $join .= "\n";
        }
    ...
    )
    {
        ...
        $GLOBALS['log']->debug(
            "getQuery, Relationship role column name=" . $this->_relationship->relationship_role_column
        );
        ...
        if ($this->_relationship->relationship_type == 'one-to-one'
            ...
            if ($bean_is_lhs) {
                ...
                $where = 'WHERE ' . $this->_relationship->rhs_table . '.'
                    . $this->_relationship->rhs_key . $bean_filter;
                if (!empty($this->_relationship->relationship_role_column) && !$this->ignore_role_filter) {
                    $where .= " AND " . $this->_relationship->rhs_table . '.'
                        . $this->_relationship->relationship_role_column;
        ...
        if ($this->_relationship->relationship_type == 'many-to-many') {
            ...
            if (($bean_is_lhs && !$swap) || (!$bean_is_lhs && $swap)) {
                ...
                $from = 'FROM ' . $this->_relationship->rhs_table;
                $subjoin = ' INNER JOIN ' . $this->_relationship->join_table . ' ON ('
                    . $this->_relationship->rhs_table . "."
                    . $this->_relationship->rhs_key . "="
                    . $this->_relationship->join_table . "."
                    . $this->_relationship->join_key_rhs . " AND "
                    . $this->_relationship->join_table . "."
                    . $this->_relationship->join_key_lhs . $bean_filter;
                $join_tables[] = $this->_relationship->join_table;
                if (!empty($this->_relationship->relationship_role_column) && !$this->ignore_role_filter) {
                    $subjoin .= " AND " . $this->_relationship->join_table . '.'
                        . $this->_relationship->relationship_role_column;
                ...
                $from = 'FROM ' . $this->_relationship->lhs_table;
                $subjoin = ' INNER JOIN ' . $this->_relationship->join_table . ' ON ('
                    . $this->_relationship->lhs_table . "." . $this->_relationship->lhs_key . "="
                    . $this->_relationship->join_table . "." . $this->_relationship->join_key_lhs . " AND "
                    . $this->_relationship->join_table . "." . $this->_relationship->join_key_rhs . $bean_filter;
                $join_tables[] = $this->_relationship->join_table;
                if (!empty($this->_relationship->relationship_role_column) && !$this->ignore_role_filter) {
    ...
    public function _add_one_to_many_table_based($key, $bean_is_lhs)
    {
        ...
        $query .= ' SET ' . $this->_relationship->rhs_table . '.'
            . $this->_relationship->rhs_key . "='" . $set_key_value . "'";
    ...
    public function _add_many_to_one_bean_based($key)
    {
        ...
        if (!empty($this->_relationship->relationship_role_column)) {
            $value = $this->_relationship->relationship_role_column_value;
            $bean->{$this->_relationship->relationship_role_column} = $value;
        }
    ...
    *
        ...
        foreach ($keys as $key) {
            ...
            if ($this->_relationship->relationship_type == 'many-to-many') {
                ...
                if ($this->_is_self_relationship() && !empty($GLOBALS['dictionary'][$this->_relationship_name]) &&
                    ...
                    $additional_values[$this->_relationship->join_key_lhs]
                        = $additional_values[$this->_relationship->join_key_rhs];
    ...
    private function _insert_row(&$value_array)
    {
        ...
        $insert_string = 'INSERT into ' . $this->_relationship->join_table . ' ('
    ...
    *
    ...
    {
        $GLOBALS['log']->debug(
            sprintf("delete called with these parameter values. id=%s, related_id=%s", $id, $related_id)
        );
        ...
        if ($_relationship->relationship_type == 'many-to-many') {
            ...
            $query = 'UPDATE ' . $_relationship->join_table . " SET deleted=1, date_modified='"
            ...
            if ($bean_is_lhs && $use_bean_is_lhs) {
                if (!empty($this->_relationship->reverse)
                    ...
                    if (empty($related_id)) {
                        ...
                        $query .= " WHERE (" . $_relationship->join_key_lhs . "= '" . $id . "' AND "
                            . $_relationship->join_key_rhs . "='" . $related_id . "') OR ("
                            . $_relationship->join_key_rhs . "='" . $id . "' AND " . $_relationship->join_key_lhs
                            . "='" . $related_id . "')";
                    }
                } else {
                    if (empty($related_id)) {
                        ...
                        $query .= " WHERE " . $_relationship->join_key_lhs . "= '" . $id . "' AND "
                            . $_relationship->join_key_rhs . "= '" . $related_id . "'";
                    }
                }
            } else {
                if (!empty($this->_relationship->reverse)
                    ...
                    if (empty($related_id)) {
                        ...
                        $query .= " WHERE (" . $_relationship->join_key_rhs . "= '" . $id . "' AND "
                            . $_relationship->join_key_lhs . "='" . $related_id . "') OR ("
                            . $_relationship->join_key_lhs . "='" . $id . "' AND " . $_relationship->join_key_rhs
                            . "='" . $related_id . "')";
                    }
                } else {
                    if (empty($related_id)) {
    ...
    *
     ...
     * @return array
     */
    private function _get_alternate_key_fields($table_name)
    {
        $alternateKey = null;
    ...
    *
     ...
     * @return bool|string
     */
    private function _get_link_table_role_field($table_name)
    {
