    ...
    public function cleanBean()
    {
        if (!is_array($this->field_defs) && !is_object($this->field_defs)) {
            ...
            foreach ((array)$this->field_defs as $key => $def) {
                ...
                // STIC#607
                // STIC#7b37782f768a797743aa826ce525e40370927ff1 INTERNAL STIC
                } 
                elseif($this->module_name=='EmailTemplates' && $def['name']=='body_html' && $def['type']=='longtext'){    
                    $this->$key = htmlentities($this->$key, true);
                // END STIC
                } elseif (
