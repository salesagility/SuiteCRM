...
class SugarWebServiceImplv4 extends SugarWebServiceImplv3_1
{
    ...
    {
        $GLOBALS['log']->info("Begin: SugarWebServiceImpl->login({$user_auth['user_name']}, $application, " . print_r($name_value_list,
                true) . ")");
        ...
        if ($isLoginSuccess) {
            ...
            if ($usr_id && isset($user->user_name) && ($user->getPreference('lockout') == '1')) {
                ...
                if (function_exists('mcrypt_cbc') && $authController->authController->userAuthenticateClass == "LDAPAuthenticateUser"
                    ...
                    if ($authController->authController->userAuthenticateClass == "LDAPAuthenticateUser"
                        ...
                        if ($authController->login($user_auth['user_name'], md5($user_auth['password']),
                                array('passwordEncrypted' => true))
        ...
        if (!empty($search_string) && isset($search_string)) {
            ...
            foreach ($modules_to_search as $name => $beanName) {
                ...
                if ($beanName != "User"
                    ...
                    $ret_array = $seed->create_new_list_query('', $where, $filterFields, $list_params, 0, '', true,
                        $seed, true);
    ...
    public function job_queue_next($session, $clientid)
    {
        ...
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access',
            $error)
        ) {
