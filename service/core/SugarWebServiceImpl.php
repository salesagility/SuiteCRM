...
class SugarWebServiceImpl
{
    ...
    function get_entry_list(
        ...
        return array(
            ...
            'relationship_list' => $linkoutput_list
        );
    } // fn
    ...
    function set_relationship(
        ...
        if (self::$helperObject->new_handle_set_relationship($module_name, $module_id, $link_field_name, $related_ids,
            $name_value_array, $deleted)
        ) {
            if ($deleted) {
                $deletedCount++;
            } else {
    ...
    function set_relationships(
        ...
        foreach ($module_names as $module_name) {
            ...
            if (self::$helperObject->new_handle_set_relationship($module_name, $module_ids[$counter],
                $link_field_names[$counter], $related_ids[$counter], $name_value_list, $deleted)
            ) {
                if ($deleted) {
                    $deletedCount++;
                } else {
    ...
    function set_entries($session, $module_name, $name_value_lists)
    {
        ...
        if (self::$helperObject->isLogLevelDebug()) {
            $GLOBALS['log']->debug('SoapHelperWebServices->set_entries - input data is ' . var_export($name_value_lists,
                    true));
        } // if
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'write',
    ...
    function get_server_info()
    {
        ...
        return array(
            ...
            'gmt_time' => TimeDate::getInstance()->nowDb()
        );
    } // fn
    ...
    function set_note_attachment($session, $note)
    {
        ...
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, $module_access,
            'no_access', $error)
        ) {
    ...
    function search_by_module($session, $search_string, $modules, $offset, $max_results)
    {
        ...
        if (!empty($search_string) && isset($search_string)) {
            $search_string = trim($GLOBALS['db']->quote(securexss(from_html(clean_string($search_string,
                'UNIFIED_SEARCH')))));
            foreach ($modules_to_search as $name => $beanName) {
                ...
                if ($beanName != "User"
                    ...
                    $ret_array = $seed->create_new_list_query('', $where, $filterFields, array(), 0, '', true, $seed,
                        true);
...
LEFT JOIN email_addresses ea ON (ea.id = eabl.email_address_id) ";
    ...
    function get_entries_count($session, $module_name, $query, $deleted)
    {
        ...
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'list',
            'no_access', $error)
        ) {
