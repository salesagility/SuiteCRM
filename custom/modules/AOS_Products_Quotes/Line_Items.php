    ...
    if ($view == 'EditView') {
        ...
        while ($row = $focus->db->fetchByAssoc($result)) {
            ...
            if ($line_item->product_id != '0' && $line_item->product_id != null) {
                ...
                $product .= "<td class='tabDetailViewDF' style='padding:2px;'>".cstmstripDecimalPointsAndTrailingZeroes(format_number($line_item->product_qty), $sep[1])."</td>";
                ...
                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".cstmget_discount_string($line_item->discount, $line_item->product_discount, $params, $locale, $sep)."</td>";
                ...
                $service .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".cstmget_discount_string($line_item->discount, $line_item->product_discount, $params, $locale, $sep)."</td>";
...
function cstmstripDecimalPointsAndTrailingZeroes($inputString, $decimalSeparator)
{
    return preg_replace('/'.preg_quote((string) $decimalSeparator).'[0]+$/', '', (string) $inputString);
}

function cstmget_discount_string($type, $amount, $params, $locale, $sep)
{
