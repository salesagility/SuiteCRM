...
CAL.make_draggable = function (id, type) {
    ...
    dd.onDragDrop = function (e, id) {
        ...
        var data = {"current_module": this.el.getAttribute("module_name"), "record": this.el.getAttribute("record"), "datetime": slot.getAttribute("datetime"), "calendar_style": type};
...
CAL.make_resizable = function (id, slot) {
    ...
    resize.on('endResize', function (e) {
        ...
        if (el) {
            ...
            var data = {"current_module": el.getAttribute("module_name"), "record": el.getAttribute("record"), "duration_hours": duration_hours, "duration_minutes": duration_minutes};
            YAHOO.util.Connect.asyncRequest('POST', url, callback, CAL.toURI(data));
        }
    });
}
        ...
        for (w = 0; w < portions_count; w++) {
            ...
            CAL.each(portion, function (i, v) {
                ...
                if (slot) {
                    ...
                    var el = CAL.create_item({item: item, type: 'basic', head_text: head_text, id: portion[i].id, position: portion[i].position, slot: slot, id_suffix: id_suffix, days: portion[i].days});
...
CAL.add_item_to_grid = function (item) {
    ...
    var el = CAL.create_item({item: item, type: 'advanced', head_text: head_text, duration_coef: duration_coef, id: item.record, id_suffix: id_suffix, item_text: item_text, content_style: content_style, content_style_value: content_style_value, related_to: related_to});
    YAHOO.util.Event.on(el, "click", function () {
        if (this.getAttribute('detail') == "1")
            CAL.load_form(this.getAttribute('module_name'), this.getAttribute('record'), false);
    });
...
CAL.init_edit_dialog = function (params) {
    ...
    CAL.editDialog = new YAHOO.widget.Dialog("cal-edit", {draggable: true, visible: false, modal: true, close: true, y: 1, zIndex: 10});
...
CAL.toggle_settings = function () {
    ...
    if (!CAL.settingsDialog) {
        CAL.settingsDialog = new YAHOO.widget.Dialog("settings_dialog", {fixedcenter: true, draggable: false, visible: false, modal: true, close: true});
        var listeners = new YAHOO.util.KeyListener(document, {keys: 27}, {fn: function () {
...
CAL.load_create_form = function (params) {
    ...
    var url = "index.php?module=Calendar&action=QuickEdit&sugar_body_only=true";
    var data = {"current_module": params.module_name, "assigned_user_id": params.user_id, "assigned_user_name": params.user_name, "date_start": params.date_start};
    YAHOO.util.Connect.asyncRequest('POST', url, callback, CAL.toURI(data));
}
...
CAL.dialog_create = function (cell) {
    ...
    var params = {'module_name': module_name, 'user_id': user_id, 'user_name': user_name, 'date_start': cell.getAttribute("datetime")};
...
CAL.toggle_shared_edit = function () {
    ...
    if (!CAL.sharedDialog) {
        CAL.sharedDialog = new YAHOO.widget.Dialog("shared_cal_edit", {fixedcenter: true, draggable: false, visible: false, modal: true, close: true});
        var listeners = new YAHOO.util.KeyListener(document, {keys: 27}, {fn: function () {
...
CAL.update_vcal = function () {
    ...
    var callback = {success: function (result) {
...
YAHOO.extend(YAHOO.util.DDCAL, YAHOO.util.DD, {cont: null, init: function () {
