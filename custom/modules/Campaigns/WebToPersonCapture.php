...
// STIC-Custom 20210111 MHP - Select the language indicated in the form. If it is not indicated, the default is selected
// STIC#528
$language = isset($_POST['language']) ? $_POST['language'] : $sugar_config['default_language'];
// END STIC-Custom

$mod_strings = return_module_language($language, $moduleDir);
...
if (isset($_POST['campaign_id']) && !empty($_POST['campaign_id'])) {
    ...
    if (isset($camp_data) && $camp_data != null) {
        ...
        if (!empty($person)) {
            ...
            foreach ($_POST as $k => $v) {
                ...
                if ($k === 'client_id_address' || $k === 'req_id' || $k === 'moduleDir' || $k === 'dup_checked') {
                    ...
                    // STIC#1066
                    //  - PHP Deprecated: Using array_key_exists() on objects is deprecated. Use isset() or property_exists() instead
        ...
        if (!empty($person)) {
            ...
            for ($i = 0; $i < $total_files; $i++) {
                ...
                if ($_FILES["documents"]['name'][$i] != "") {
                    ...
                    $document->description = $mod_strings['LBL_ATTACHMENT_DESCRIPTION'] . $camp_data['name'];
                    $document->save();
                    ...
                    switch ($moduleDir) {
                        case "Leads":
                            $person->load_relationship("leads_documents_1");
                            $person->leads_documents_1->add($document->id);
                            break;
                        case "Contacts":
                            $person->load_relationship("documents");
                            $person->documents->add($document->id);
                            break;
                        case "Prospects":
                            $person->load_relationship("prospects_documents_1");
                            $person->prospects_documents_1->add($document->id);
                            break;
                        default:
                            break;
                    }
        ...
        if (isset($_POST['redirect_url']) && !empty($_POST['redirect_url'])) {
            ...
            // STIC-custom - 20220324 - jch - Send field values to the redirect URL using POST method.
            // By using SugarApplication::headerRedirect() function, params sent in the web form are added
            // to the redirect URL, so they become visible in the browser address bar, which should not occur.
