...
if (!empty($app_strings['LBL_SUBTHEMES'])) {
    ...
    // If default subtheme is custom defined in config, set it. If not, set to Stic.
    if (!isset($sugar_config['default_subtheme'])) {
        $themedef['config_options']['sub_themes']['default'] = 'Stic';
    } else {
        $themedef['config_options']['sub_themes']['default'] = $sugar_config['default_subtheme'];
