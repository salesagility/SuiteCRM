...
class PackageManager
{
    ...
    function getCategoryPackages($category_id = '', $filter = array())
    {
        ...
        if (!empty($results['categories'])) {
            foreach ($results['categories'] as $category) {
                ...
                $nodes[$mycat['id']] = array(
                    'id' => $mycat['id'],
                    'label' => $mycat['name'],
                    'description' => $mycat['description'],
                    'type' => 'cat',
                    'parent' => $mycat['parent_id']
                );
        ...
        if (!empty($results['packages'])) {
            ...
            foreach ($results['packages'] as $package) {
                ...
                $nodes[$mypack['category_id']]['packages'][$mypack['id']] = array(
                    'id' => $mypack['id'],
                    'label' => $mypack['name'],
                    'description' => $mypack['description'],
                    'category_id' => $mypack['category_id'],
                    'type' => 'package'
                );
                ...
                if (!empty($releases['packages'])) {
                    foreach ($releases['packages'] as $release) {
                        ...
                        $nodes[$mypack['category_id']]['packages'][$mypack['id']]['releases'][$myrelease['id']] = array(
                            'id' => $myrelease['id'],
                            'version' => $myrelease['version'],
                            'label' => $myrelease['description'],
                            'category_id' => $mypack['category_id'],
                            'package_id' => $mypack['id'],
                            'type' => 'release',
                            'enable' => $enable
                        );
                    }
    ...
    function getPackages($category_id, $filter = array())
    {
        ...
        if (!empty($results['packages'])) {
            foreach ($results['packages'] as $package) {
                ...
                foreach ($releases['packages'] as $release) {
                    $myrelease = PackageManager::fromNameValueList($release);
                    $arr_releases[$myrelease['id']] = array(
                        'release_id' => $myrelease['id'],
                        'version' => $myrelease['version'],
    ...
    function getPackagesInStaging($view = 'module')
    {
        ...
        foreach ($upgrade_contents as $upgrade_content) {
            if (!preg_match('#.*\.zip$#', strtolower($upgrade_content)) || preg_match("#.*./zips/.*#",
                    strtolower($upgrade_content))
            ) {
                continue;
            }
            ...
            if (empty($md5_matches)) {
                ...
                if (!empty($dependencies)) {
                    ...
                    if (!empty($not_found) && count($not_found) > 0) {
                        $file_install = 'errors_' . $mod_strings['ERR_UW_NO_DEPENDENCY'] . "[" . implode(',',
                                $not_found) . "]";
                    }
                }
