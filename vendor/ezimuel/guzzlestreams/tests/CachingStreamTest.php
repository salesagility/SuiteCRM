...
class CachingStreamTest extends TestCase
{
    ...
    public function testCannotSeekPastWhatHasBeenRead()
    {
        ...
        $this->expectErrorMessage('Cannot seek to byte 10');
        $this->body->seek(10);
    }
    ...
    public function testSkipsOverwrittenBytes()
    {
        ...
        $skipReadBytes = new ReflectionProperty(CachingStream::class, 'skipReadBytes');
        $skipReadBytes->setAccessible(true);

        $this->assertEquals("0000\n", Utils::readline($body));
        $this->assertEquals("0001\n", Utils::readline($body));
        // Write over part of the body yet to be read, so skip some bytes
        $this->assertEquals(5, $body->write("TEST\n"));
        $this->assertEquals(5, $skipReadBytes->getValue($body));
        // Read, which skips bytes, then reads
        $this->assertEquals("0003\n", Utils::readline($body));
        $this->assertEquals(0, $skipReadBytes->getValue($body));
