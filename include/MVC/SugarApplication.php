...
class SugarApplication
{
    ...
    function loadUser()
    {
        ...
        $allowed_actions = (!empty($this->controller->allowed_actions)) ? $this->controller->allowed_actions : $allowed_actions = array(
            'Authenticate',
            'Login',
            'LoggedOut'
        );
        ...
        if (isset($_SESSION['authenticated_user_theme_color'])) {
            ...
            self::setCookie('ck_login_theme_color_20', $_SESSION['authenticated_user_theme_color'],
                time() + 86400 * 90);
        }
    ...
    protected function checkHTTPReferer($dieIfInvalid = true)
    {
        ...
        if ($strong && empty($_SERVER['HTTP_REFERER']) && !in_array($this->controller->action,
                $this->whiteListActions) && $this->isModifyAction()
        ) {
            ...
            if (!empty($_SERVER['HTTP_REFERER']) && !empty($_SERVER['SERVER_NAME'])) {
                ...
                if ($http_ref['host'] !== $_SERVER['SERVER_NAME'] && !in_array($this->controller->action,
        ...
        if (empty($row_count)) {
            $version_query = "SELECT count(*) as the_count FROM config WHERE category='info' AND name='sugar_version' AND " .
                $GLOBALS['db']->convert('value',
                    'text2char') . " = " . $GLOBALS['db']->quoted($GLOBALS['sugar_db_version']);
