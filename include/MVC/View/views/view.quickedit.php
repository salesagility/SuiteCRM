...
class ViewQuickedit extends ViewAjax
{
    ...
    public function preDisplay()
    {
        if (!empty($_REQUEST['source_module']) && $_REQUEST['source_module'] != 'undefined' && !empty($_REQUEST['record'])) {
            ...
            if ($this->bean instanceof SugarBean
                    && !in_array($this->bean->object_name, ['EmailMan'])) {
    ...
    public function display()
    {
        ...
        if (file_exists($editFileName)) {
            ...
            if (class_exists($c)) {
                ...
                if ($view->useForSubpanel) {
                    ...
                    echo json_encode(['title' => $this->bean->name, 'url' => 'index.php?module=' . $this->bean->module_dir . '&action=DetailView&record=' . $this->bean->id, 'html' => $captured, 'eval' => true]);
                }
            }
        }
        ...
        if ($defaultProcess) {
            ...
            ob_clean();
            echo json_encode(['title' => $this->bean->name, 'url' => 'index.php?module=' . $this->bean->module_dir . '&action=DetailView&record=' . $this->bean->id, 'html' => $this->ev->display(false, true), 'eval' => true]);
        }
    }
