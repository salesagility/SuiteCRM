...
class SugarDateTime extends DateTime
{
    ...
    public static function createFromFormat($format, $time, $timezone = null)
    {
        ...
        if (self::$use_php_parser && is_callable(['DateTime', 'createFromFormat'])) {
            // 5.3, hurray!
            if (!empty($timezone)) {
    ...
    protected function _strptime($time, $format)
    {
        ...
        foreach (self::$parts_match as $part => $datapart) {
            if (isset($regexp['positions'][$part], $dateparts[$regexp['positions'][$part]])) {
