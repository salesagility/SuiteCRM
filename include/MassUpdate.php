...
class MassUpdate
{
     ...
     * @param $multi_select_popup boolean if it is a multi-select value
     * @return string
     */
    ...
    public function getMassUpdateFormHeader($multi_select_popup = false)
    {
        ...
        if ($multi_select_popup)
            ...
            $tempString = "<form action='index.php' method='post' name='MassUpdate'  id='MassUpdate' "
                . "onsubmit=\"return check_form('MassUpdate');\">\n"
...
eoq;
    ...
    public function handleMassUpdate()
    {
        ...
        foreach ($_POST as $post => $value) {
            if (is_array($value)) {
                ...
                if (isset($this->sugarbean->field_defs[$post])
                    && $this->sugarbean->field_defs[$post]['type'] == 'radioenum' && isset($_POST[$post])) {
                    $_POST[$post] = '';
                } else {
                    unset($_POST[$post]);
                }
            ...
            if (is_string($value) && isset($this->sugarbean->field_defs[$post])) {
                if (($this->sugarbean->field_defs[$post]['type'] == 'bool'
                    || (!empty($this->sugarbean->field_defs[$post]['custom_type'])
                        && $this->sugarbean->field_defs[$post]['custom_type'] == 'bool'
                    ))) {
                    if (strcmp($value, '2') == 0) {
                        $_POST[$post] = 0;
                    }
                    if (!empty($this->sugarbean->field_defs[$post]['dbType'])
                        && strcmp($this->sugarbean->field_defs[$post]['dbType'], 'varchar') == 0) {
                        if (strcmp($value, '1') == 0) {
                            $_POST[$post] = 'on';
                        }
                        if (strcmp($value, '2') == 0) {
                ...
                if (($this->sugarbean->field_defs[$post]['type'] == 'radioenum'
                        && isset($_POST[$post]) && strlen($value) == 0)
                    || ($this->sugarbean->field_defs[$post]['type'] == 'enum'
                        && $value == '__SugarMassUpdateClearField__')
                ) {
                    // Set to '' if it's an explicit clear
                    $_POST[$post] = '';
                }
                if ($this->sugarbean->field_defs[$post]['type'] == 'bool') {
            ...
            foreach ($_POST['mass'] as $id) {
                ...
                if (isset($_POST['Delete'])) {
                    ...
                    if ($this->sugarbean->object_name == 'Contact'
                        && isset($_POST['Sync'])) { // special for contacts module
                        if ($_POST['Sync'] == 'true') {
                            $this->sugarbean->retrieve($id);
                    ...
                    if ($this->sugarbean->ACLAccess('Save')) {
                        ...
                        if (!empty($email_address_id)) {
                            $query = "UPDATE email_addresses SET opt_out = {$optout_flag_value} "
                                . "where id = '{$emailAddressRow['email_address_id']}'";
    ...
    public function getMassUpdateForm($hideDeleteIfNoFieldsAvailable = false)
    {
        ...
        if ($this->sugarbean->bean_implements('ACL')
            && (!ACLController::checkAccess($this->sugarbean->module_dir, 'edit', true)
        ...
        $html = "<div id='massupdate_form' style='display:none;'><table width='100%' cellpadding='0' "
            . "cellspacing='0' border='0' class='formHeader h3Row'><tr><td nowrap><h3><span>"
            . $app_strings['LBL_MASS_UPDATE'] . "</h3></td></tr></table>";
        $html .= "<div id='mass_update_div'><table cellpadding='0' cellspacing='1' border='0' width='100%' "
            . "class='edit view' id='mass_update_table'>";

        $even = true;

        if ($this->sugarbean->object_name == 'Contact') {
            $html .= "<tr><td width='15%' scope='row'>$lang_sync</td><td width='35%' class='dataField'>" .
                "<select name='Sync'><option value=''>{$GLOBALS['app_strings']['LBL_NONE']}</option>" .
        ...
        foreach ($this->sugarbean->field_defs as $field) {
            if (!isset($banned[$field['name']]) && (!isset($field['massupdate']) || !empty($field['massupdate']))) {
                ...
                if (isset($field['type']) && $field['type'] == 'relate'
                    && isset($field['id_name']) && $field['id_name'] == 'assigned_user_id') {
        ...
        if ($this->sugarbean->object_name == 'Contact' ||
            ...
            $html .= "<tr><td width='15%'  scope='row' class='dataLabel'>$lang_optout_primaryemail</td>" .
                "<td width='35%' class='dataField'><select name='optout_primary'><option value=''>" .
                "{$GLOBALS['app_strings']['LBL_NONE']}</option><option value='false'>" .
                "{$GLOBALS['app_list_strings']['checkbox_dom']['2']}</option><option value='true'>" .
                "{$GLOBALS['app_list_strings']['checkbox_dom']['1']}</option></select></td></tr>";

        }
        $html .= "</table>";

        $html .= "<table cellpadding='0' cellspacing='0' border='0' width='100%'><tr><td class='buttons'>" .
            "<input onclick='return sListView.send_mass_update(\"selected\", \"" .
            "{$app_strings['LBL_LISTVIEW_NO_SELECTED']}\")' type='submit' id='update_button' name='Update' " .
            "value='{$lang_update}' class='button'>&nbsp;<input onclick='toggleMassUpdateForm();' " .
            "type='button' id='cancel_button' name='Cancel' " .
            "value='{$GLOBALS['app_strings']['LBL_CANCEL_BUTTON_LABEL']}' class='button'>";
        // TODO: allow ACL access for Delete to be set false always for users

        // only for My Inbox views - to allow CSRs to have an "Archive" emails feature
        // to get the email "out" of their inbox.
...
EOJS;
        ...
        if ($field_count > 0) {
            ...
            //If no fields are found, render either a form that still permits Mass Update deletes
            // or just display a message that no fields are available
            $html = "<div id='massupdate_form' style='display:none;'><table width='100%' cellpadding='0' "
                . "cellspacing='0' border='0' class='formHeader h3Row'><tr><td nowrap><h3><span>"
                . $app_strings['LBL_MASS_UPDATE'] . "</h3></td></tr></table>";
            if ($this->sugarbean->ACLAccess('Delete', true) && !$hideDeleteIfNoFieldsAvailable) {
                $html .= "<table cellpadding='0' cellspacing='0' border='0' width='100%'><tr><td><input "
                    . "type='submit' name='Delete' value='$lang_delete' "
                    . "onclick=\"return confirm('{$lang_confirm}')\" class='button'></td></tr></table></div>";
            } else {
...
addToValidateBinaryDependency('MassUpdate', '{$varname}', 'alpha', false, '{$app_strings['ERR_SQS_NO_MATCH_FIELD']} ' +
 '{$app_strings['LBL_ASSIGNED_TO']}','{$id_name}');
-->
</script>
EOHTML;
    }
     ...
     *
     * @param $displayname string Name to display in the popup window
     * @param $varname string name of the variable
     * @param $id_name string name of the id in vardef
     * @return string
     */
    public function addAccountID($displayname, $varname, $id_name = '')
    {
        ...
        $html = '<td scope="row">' . $displayname . " </td>\n"
            . '<td><input class="sqsEnabled" type="text" autocomplete="off" id="mass_' . $varname . '" name="'
            . $varname . '" value="" /><input id="mass_' . $id_name . '" type="hidden" name="'
            . $id_name . '" value="" />&nbsp;<span class="id-ff multiple"><button type="button" '
            . 'name="btn1" class="button" title="'
            . $app_strings['LBL_SELECT_BUTTON_LABEL'] . '"  value="' . $app_strings['LBL_SELECT_BUTTON_LABEL']
            . '" onclick='
            . "'open_popup(\"Accounts\",600,400,\"\",true,false,{$encoded_popup_request_data});' />"
            . "<img alt=\"$img\" src=\"$img\"></button></span></td>\n";
        $html .= '<script type="text/javascript" language="javascript">if(typeof sqs_objects == \'undefined\')'
            . '{var sqs_objects = [];}sqs_objects[\'MassUpdate_' . $varname . '\'] = '
            . $json->encode($qsParent) . '; '
            . 'registerSingleSmartInputListener(document.getElementById(\'mass_' . $varname . '\'));
					addToValidateBinaryDependency(\'MassUpdate\', \'' . $varname . '\', \'alpha\', false, \''
            . $app_strings['ERR_SQS_NO_MATCH_FIELD'] . $app_strings['LBL_ACCOUNT'] . '\',\'' . $id_name . '\');
					</script>';
...
EOHTML;
    ...
    public function addParent($displayname, $field)
    {
        ...
        $change_parent_button = "<span class='id-ff'><button title='" . $app_strings['LBL_SELECT_BUTTON_TITLE']
            . "'  type='button' class='button' value='" . $app_strings['LBL_SELECT_BUTTON_LABEL']
            . "' name='button_parent_name' onclick='open_popup(document.MassUpdate.{$field['type_name']}.value, "
...
addToValidateBinaryDependency('MassUpdate', 'parent_name', 'alpha', false, '{$app_strings['ERR_SQS_NO_MATCH_FIELD']} ' +
 '{$app_strings['LBL_ASSIGNED_TO']}','parent_id');
...
document.getElementById('mass_{$field['type_name']}').onchange = function()
{
	...
	var new_module = document.forms["MassUpdate"].elements["parent_type"].value;
...
EOHTML;
	...
	<td class='dataField' width="30%"><input type="text"
	  name='$varname' size="12" id='{$varname}' maxlength='10' value=""></td>
...
EOQ;
    ...
    public function addAssignedUserID($displayname, $varname)
    {
        ...
        $html = <<<EOQ
		<td width="15%" scope="row">$displayname</td>
		<td ><input class="sqsEnabled" autocomplete="off" id="mass_assigned_user_name" name='assigned_user_name'
		    type="text" value=""><input id='mass_assigned_user_id' name='assigned_user_id' type="hidden" value="" />
		<span class="id-ff multiple"><button id="mass_assigned_user_name_btn"
		    title="{$app_strings['LBL_SELECT_BUTTON_TITLE']}" type="button" class="button"
...
EOQ;
        $html .= '<script type="text/javascript" language="javascript">if(typeof sqs_objects == \'undefined\')' .
            '{var sqs_objects = [];}sqs_objects[\'MassUpdate_assigned_user_name\'] = ' .
            $json->encode($qsUser) .
            '; registerSingleSmartInputListener(document.getElementById(\'mass_assigned_user_name\'));
				addToValidateBinaryDependency(\'MassUpdate\', \'assigned_user_name\', \'alpha\', false, \''
            . $app_strings['ERR_SQS_NO_MATCH_FIELD'] . $app_strings['LBL_ASSIGNED_TO'] . '\',\'assigned_user_id\');
				</script>';
     ...
     * @param string $displayname Name to display in the popup window
     * @param string $varname name of the variable
     * @param array $options of options for status
     * @return string
     */
...
EOQ;
        ...
        $html = <<<EOQ
		<td scope="row" width="20%">$displayname</td>
		<td class='dataField' width="30%"><input onblur="parseDate(this, '$cal_dateformat')" type="text"
		    name='$varname' size="12" id='{$varname}_date' maxlength='10' value="">
		<img border="0" src="$jscalendarImage" alt='{$app_strings['LBL_MASSUPDATE_DATE']}' id="{$varname}_trigger"
		    title="{$app_strings['LBL_MASSUPDATE_DATE']}"  align="absmiddle">&nbsp;$javascriptend
		...
		addToValidateBinaryDependency('MassUpdate', '{$varname}_hours', 'alpha', false,
		"{$app_strings['ERR_MISSING_REQUIRED_FIELDS']}", '{$varname}_date');
		addToValidateBinaryDependency('MassUpdate', '{$varname}_minutes', 'alpha', false,
		"{$app_strings['ERR_MISSING_REQUIRED_FIELDS']}", '{$varname}_date');
		addToValidateBinaryDependency('MassUpdate', '{$varname}_meridiem', 'alpha', false,
		"{$app_strings['ERR_MISSING_REQUIRED_FIELDS']}", '{$varname}_date');
		</script>
...
EOQ;
    ...
    public function addDate($displayname, $varname)
    {
        ...
        $javascriptend = <<<EOQ
		 <script type="text/javascript">
		Calendar.setup ({
			inputField : "${varname}jscal_field", daFormat : "$cal_dateformat", ifFormat : "$cal_dateformat",
			  showsTime : false, button : "${varname}jscal_trigger", singleClick : true, step : 1, weekNumbers:false
		});
		</script>
EOQ;
        $jscalendarImage = SugarThemeRegistry::current()->getImageURL('jscalendar.gif');
	...
	<td scope="row" width="20%">$displayname</td>
	<td class='dataField' width="30%"><input onblur="parseDate(this, '$cal_dateformat')" type="text"
	  name='$varname' size="12" id='{$varname}jscal_field' maxlength='10' value="">
    <img src="$jscalendarImage" id="{$varname}jscal_trigger" align="absmiddle"
        title="{$app_strings['LBL_MASSUPDATE_DATE']}" alt='{$app_strings['LBL_MASSUPDATE_DATE']}'>&nb
    <span class="dateFormat">$userformat</span>
	$javascriptend</td>
    ...
    {//this function is similar with function prepareSearchForm() in view.list.php
        ...
        if (file_exists('modules/' . $module . '/SearchForm.html')) {
            if (file_exists('modules/' . $module . '/metadata/SearchFields.php')) {
                ...
                //bug 26026, sometimes some module doesn't have a metadata/SearchFields.php,
                // the searchfrom is generated in the ListView.php.
                //So currently massupdate will not gernerate the where sql. It will use the sql stored in the SESSION.
                // But this will cause bug 24722, and it cannot be avoided now.
            ...
            if (empty($searchdefs) || empty($searchFields)) {
                //for some modules, such as iframe, it has massupdate,
                // but it doesn't have search function, the where sql should be empty.
                $this->where_clauses = '';
                return;
            }
