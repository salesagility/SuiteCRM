...
class vCard
{
    ...
    public function saveVCard()
    {
        ...
        echo $locale->translateCharset($content, 'UTF-8', $locale->getExportCharset());
    }
                ...
                if ($keys[0] == 'ORG') {
                    ...
                    if (!empty($value)) {
                        ...
                        if (is_a($bean, 'Contact') || is_a($bean, 'Lead')) {
                            ...
                            $result = $accountBean->retrieve_by_string_fields(['name' => $full_company_name, 'deleted' => 0]);
                            if (!isset($result->id)) {
                                // Try to trim the full company name down, see if we get some other matches
                                $vCardTrimStrings = ['/ltd\.*/i' => '',
                                    '/llc\.*/i' => '',
                                    '/gmbh\.*/i' => '',
                                    '/inc\.*/i' => '',
                                    '/\.com/i' => '',
                                ...
                                $result = $accountBean->retrieve_by_string_fields(['name' => $short_company_name, 'deleted' => 0]);
                            }
