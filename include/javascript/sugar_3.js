...
function addRequireFieldToComposeField(formname,parentname,name,type,msg){if(typeof validate[formname]!='undefined'){for(var i=0;i<validate[formname].length;i++){if(validate[formname][i][nameIndex]==parentname&&validate[formname][i][jstypeIndex]=='composefield'){validate[formname][i][compareToIndex].push(new Array(name,type,'required',msg));}}}}
...
break;case'composefield':if(typeof validate[formname][i][compareToIndex]!='undefined'){if(!validate[formname][i][requiredIndex]&&trim(form[validate[formname][i][nameIndex]].value)==''){add_error_style(formname,validate[formname][i][nameIndex],invalidTxt+' '+validate[formname][i][msgIndex]);isError=true;}if(trim(form[validate[formname][i][nameIndex]].value)==''||validate[formname][i][requiredIndex]){
for(var j=0;j<validate[formname][i][compareToIndex].length;j++){switch(validate[formname][i][compareToIndex][j][2]){case'required':if(typeof form[validate[formname][i][compareToIndex][j][0]]=='undefined'||trim(form[validate[formname][i][compareToIndex][j][0]].value)==""){add_error_style(formname,validate[formname][i][compareToIndex][j][0],invalidTxt+' '+validate[formname][i][compareToIndex][j][3]);isError=true;}
break;}}}}
...
if(typeof(uniquecheck[formname])!='undefined'){
if (typeof(module_sugar_grp1)=='undefined'){module_sugar_grp1='Home';}var module=module_sugar_grp1;var id=$("input[name=record]").attr("value");if(typeof id == 'undefined'){id=$(form).closest('tr').find('[type=checkbox]').attr( "value" )}var jsonrules='{"rules":[';for(var i=0;i<uniquecheck[formname].length;i++){var ruleName=uniquecheck[formname][i][nameIndex];if(i>0)
