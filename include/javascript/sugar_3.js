...
if(typeof window[formname+"_tabs"]!="undefined"){if($(".validation-message")){$("#EditView_tabs li a").each(function(){if($(this).css("background-color")=="rgb(255, 0, 0)"){$(this).css("background-color","");}})
counter=0;$(".validation-message").each(function(){CurrentValidationTabId=$(this).closest(".tab-pane-NOBOOTSTRAPTOGGLER").attr("id").split('-').slice(-1);$CurrentValidationTabA=$("#EditView_tabs a#tab"+CurrentValidationTabId);$CurrentValidationTabA.attr("style","background-color:red;").closest("li");if(counter==0){$CurrentValidationTabA.click();$CurrentValidationTabA.addClass("active");}
counter++;});}}
...
lastSubmitTime=_date.getTime();oldStartsWith=startsWith;clear_all_errors();inputsWithErrors=new Array();for(var i=0;i<validate[formname].length;i++){if(validate[formname][i][nameIndex].indexOf(startsWith)==0){if(typeof form[validate[formname][i][nameIndex]]!='undefined'&&form[validate[formname][i][nameIndex]]instanceof HTMLElement&&typeof form[validate[formname][i][nameIndex]].value!='undefined'){var bail=false;if(!validate[formname][i][requiredIndex]&&trim(form[validate[formname][i][nameIndex]].value)==''&&(typeof(validate[formname][i][jstypeIndex])!='undefined'&&validate[formname][i][jstypeIndex]!='binarydep'&&validate[formname][i][jstypeIndex]!='callback')){continue;}
...
break;case'int':case'range':if(!isInteger(trim(form[validate[formname][i][nameIndex]].value))){isError=true;add_error_style(formname,validate[formname][i][nameIndex],invalidTxt+" "+validate[formname][i][msgIndex]);}
...
else{if($('#massupdate_form input#mass_duplicate').length==1){return confirm(SUGAR.language.get('app_strings','LBL_MASS_DUPLICATE_UPDATE_CONFIRMATION_NUM')+sugarListView.get_num_selected_string()+SUGAR.language.get('app_strings','NTC_DELETE_SELECTED_RECORDS'));}else{return confirm(SUGAR.language.get('app_strings','NTC_UPDATE_CONFIRMATION_NUM')+sugarListView.get_num_selected_string()+SUGAR.language.get('app_strings','NTC_DELETE_SELECTED_RECORDS'));}}}
