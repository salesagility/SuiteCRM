...
composeBox.setBody(data);self.emailComposeView=composeBox.controls.modal.body.find('.compose-view').EmailsComposeView();var targetCount=0;var targetList='';var populateModuleName='';var populateEmailAddress='';var populateModule='';var populateModuleRecord='';var dataEmailName=$(source).attr('data-module-name');var dataEmailAddress=$(source).attr('data-email-address');$('.email-compose-view-to-list').each(function(){populateModuleName=$(this).attr('data-record-name');if(dataEmailName!==''){populateModuleName=dataEmailName;}
populateEmailAddress=$(this).attr('data-record-email');if(dataEmailAddress!==''){populateEmailAddress=dataEmailAddress;}
populateModule=$(this).attr('data-record-module');populateModuleRecord=$(this).attr('data-record-id');if(targetCount>0){targetList=targetList+',';}
if(populateModuleName===''){populateModuleName=populateEmailAddress;}
targetList=targetList+populateModuleName+' <'+populateEmailAddress+'>';targetCount++;});if(targetCount>0){if(populateEmailAddress!==''){$(self.emailComposeView).find('#to_addrs_names').val(targetList);}
