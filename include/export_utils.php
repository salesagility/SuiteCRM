 ...
 * @param $type string type the bean-type to export
 * @param array $records records an array of records if coming directly from a query
 * @param bool $members
 * @param bool $sample
    ...
    if ($populate) {
        ...
        while ($val = $db->fetchByAssoc($result, false)) {
            ...
            foreach ($val as $key => $value) {
                ...
                if (isset($focus->field_name_map[$fieldNameMapKey])
                    && isset($focus->field_name_map[$fieldNameMapKey]['exportable']) &&
                    ...
                    switch ($fieldType) {
                        ...
                        case 'multienum':
                            ...
                            if (isset($focus->field_name_map[$fields_array[$key]]['options'])
                                ...
                                $options = $focus->field_name_map[$fields_array[$key]]['options'];
                                foreach ($valueArray as $multikey => $multivalue) {
                                    if (isset($app_list_strings[$options][$multivalue])) {
                                        $valueArray[$multikey] = $app_list_strings[$options][$multivalue];
                                    }
                                }
                            }
        ...
        foreach ($records as $record) {
            foreach ($record as $recordKey => $recordValue) {
                if (preg_match('/{relate\s+from=""([^"]+)""\s+to=""([^"]+)""}/', $recordValue, $matches)) {
                    ...
                    if (isset($relatedBean->field_defs['name']['source'])
                        && $relatedBean->field_defs['name']['source'] == 'non-db') {
                        //$relatedLabel = 'NULL AS related_label, NULL AS related_label1';
                        if (!isset($relatedBean->field_defs['first_name']['source'])
                            || $relatedBean->field_defs['first_name']['source'] != 'non-db' &&
                            !isset($relatedBean->field_defs['last_name']['source'])
                            || $relatedBean->field_defs['last_name']['source'] != 'non-db') {
                            $relatedLabel =
                                "$relatedTable.last_name AS related_label, $relatedTable.first_name AS related_label1";
                        }
                    }
                    ...
                    if (isset($currentBean->field_defs[$relatedField])) {
                        $relatedTableJoin =
                            "LEFT JOIN $relatedTable ON $relatedTable.id = {$currentTable}_cstm.$relatedField";
                    }

                    //-- $relatedTable.id AS related_id,
                    //-- {$currentTable}_cstm.id_c AS current_id_c,
                    //-- {$relatedTable}_cstm.id_c AS related_id_c,
                    $selects[] = "(SELECT $currentTable.id AS current_id,'$currentModule' AS current_module," .
                        "'$currentField' AS current_field,'$relatedModule' AS related_module,'$relatedField' " .
                        "AS related_field,$relatedFieldSelect,$relatedLabel FROM $currentTable JOIN " .
                        "{$currentTable}_cstm ON {$currentTable}_cstm.id_c=$currentTable.id $relatedTableCustomJoin " .
                        "$relatedTableJoin WHERE $currentTable.id='{$record['id']}')";
                }
            }
        }
        ...
        foreach ($selects as $select) {
            ...
            if ($i == $selectsCount || strlen($queryTemp) > $queryMaxLength) {
                ...
                while ($val = $db->fetchByAssoc($result, false)) {
                    $current_module = $val['current_module'];
                    $current_id = $val['current_id'];
                    $related_module = $val['related_module'];
                    $related_field = $val['related_field'];
                    $customRelateFields[$current_module][$current_id][$related_module][$related_field]
                        = trim($val['related_label'] . ' ' . $val['related_label1']);
                }
                $query = $select;
            } else {
                $query = $queryTemp;
 ...
 * @param $customRelateFields mixed string CSV line
 * @return mixed
 */
...
function generateSearchWhere($module, $query)
{
    ...
    if (file_exists('modules/' . $module . '/SearchForm.html')) {
        if (file_exists('modules/' . $module . '/metadata/SearchFields.php')) {
            ...
            // Currently, massupdate will not generate the where sql. It will use the sql stored in the SESSION.
            ...
            //expects the results in an array, not just a string. So rather than fixing the caller,
            // I felt it would be best for the function to return the results in a standard format.
    ...
    while ($counter < $rowsToReturn) {
        ...
        foreach ($field_array as $field_name) {
            ...
            switch ($type) {
                ...
                case "name":
                    ...
                    if ($field['name'] == 'first_name') {
                        ...
                        if ($focus->module_dir == 'Accounts') {
                            ...
                            $returnContent .= '"'
                                . $sugar_demodata['meeting_seed_data_names'][mt_rand(0, $count)] . '",';
                ...
                case "text":
                    //return random text
                    $returnContent .= '"Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Maecenas porttitor ' .
                ...
                case "varchar":
                    ...
                    if ($field['name'] == 'first_name') {
                        ...
                        $count = count($sugar_demodata['first_name_array']) - 1;
                        $returnContent .= '"' . $sugar_demodata['last_name_array'][mt_rand(0, $count)] . '_'
                            . mt_rand(1, 111) . '",';
...
function get_field_order_mapping($name = '', $reorderArr = '', $exclude = true)
{
    ...
    $field_order_array['accounts'] = array(
        'name' => 'Name',
        'id' => 'ID',
        'website' => 'Website',
    ...
    $field_order_array['contacts'] = array(
        ...
        'email_address' => 'Email Address',
        'email_addresses_non_primary' => 'Non Primary E-mails for Import',
    ...
    $field_order_array['leads'] = array(
        ...
        'modified_user_id' => 'Modified By ID',
        'deleted' => 'Deleted'
    );
    $field_order_array['opportunities'] = array(
    ...
    $field_order_array['bugs'] = array(
        ...
        'found_in_release_name' => 'Found In Release Name',
        'fixed_in_release_name' => 'Fixed In Release',
        'product_category' => 'Category',
        'source' => 'Source',
        'portal_viewable' => 'Portal Viewable',
        'system_id' => 'System ID',
        'assigned_user_id' => 'Assigned User ID',
        'assigned_user_name' => 'Assigned User Name',
        'team_name' => 'Teams',
    ...
    $field_order_array['meetings'] = array(
        'name' => 'Subject',
        'id' => 'ID',
        'description' => 'Description',
        'status' => 'Status',
    ...
    $field_order_array['prospects'] = array(
        'first_name' => 'First Name',
        'last_name' => 'Last Name',
        'id' => 'ID',
        'salutation' => 'Salutation',
    ...
    $fields_to_exclude['cases'] = array(
        ...
        'account_name_mod',
        'modified_user_name',
        'modified_user_name_owner',
        'modified_user_name_mod'
    );
