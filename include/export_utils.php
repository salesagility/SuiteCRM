...
function export($type, $records = null, $members = false, $sample = false)
{
    ...
    foreach ($fields_array as $key => $dbname) {
        ...
        if (isset($focus->field_name_map[$key], $focus->field_name_map[$key]['exportable'])
    ...
    if ($populate) {
        ...
        while ($val = $db->fetchByAssoc($result, false)) {
            ...
            if ($members) {
                ...
                unset($val['ea_deleted'], $val['ear_deleted'], $val['primary_address']);
            }
            $pre_id = $val['id'];
            ...
            foreach ($val as $key => $value) {
                ...
                if (isset($focus->field_name_map[$fieldNameMapKey], $focus->field_name_map[$fieldNameMapKey]['exportable']) &&
            ...
            $valueArray = unencodeMultiEnum($value);
                        ...
                        if (isset($focus->field_name_map[$fields_array[$key]]['options'], $app_list_strings[$focus->field_name_map[$fields_array[$key]]['options']])) {
            ...
            $value = implode(',', $valueArray);
                        ...
                        break;
        case 'enum':
            if (isset($focus->field_name_map[$fields_array[$key]]['options'], $app_list_strings[$focus->field_name_map[$fields_array[$key]]['options']], $app_list_strings[$focus->field_name_map[$fields_array[$key]]['options']][$value])
            ) {
 ...
 * @param mixed $customRelateFields
 *
...
function get_field_order_mapping($name = '', $reorderArr = '', $exclude = true)
{
    ...
    if (!empty($name) && !empty($reorderArr) && is_array($reorderArr)) {
        ...
        if (!isset($field_order_array[strtolower($name)]) && isset($_REQUEST['module'])) {
            $exemptModuleList = ['ProspectLists'];
