...
class TimeDate
{
        ...
        if (empty(self::$timedate)) {
            if (ini_get('date.timezone') == '') {
                ...
                if (isset($GLOBALS['log'])) {
                    $GLOBALS['log']->warn(
                        "Configuration variable date.timezone is not set, guessed timezone $tz. " .
                        "Please set date.timezone=\"$tz\" in php.ini!"
                    );
                }
    ...
    )
    {
        if (is_bool($user) || func_num_args() > 1) {
            ...
            $GLOBALS['log']->fatal(
                'TimeDate::get_time_format(): Deprecated API used, please update you code - ' .
                'get_time_format() now has one argument of type User'
            );
            if (func_num_args() > 1) {
                $user = func_get_arg(1);
            } else {
    ...
    public function asUserType(DateTime $date, $type, User $user = null)
    {
        switch ($type) {
            case "date":
                return $this->asUserDate($date, $user);
                break;
            case 'time':
    ...
    public function fromDbFormat($date, $format)
    {
        try {
            ...
            $GLOBALS['log']->error(
                "fromDbFormat: Conversion of $date from DB format $format failed: {$e->getMessage()}"
            );
