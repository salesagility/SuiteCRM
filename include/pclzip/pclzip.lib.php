...
//  function create($p_filelist, $p_add_dir="", $p_remove_dir="")
    function create($p_filelist /*, options */)
    {
        ...
        if (is_array($p_filelist)) {
            // ----- Call the create fct
            $v_result = $this->privCreate($p_filelist, $p_result_list, $v_add_path, $v_remove_path, $v_remove_all_path,
                $v_options);
            ...
            if (is_string($p_filelist)) {
                ...
                // ----- Call the create fct
                $v_result = $this->privCreate($v_list, $p_result_list, $v_add_path, $v_remove_path, $v_remove_all_path,
                    $v_options);
            } // ----- Invalid variable
            else {
                // ----- Error log
...
//  function add($p_filelist, $p_add_dir="", $p_remove_dir="")
    ...
    function privCreate($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
    {
        ...
        $v_result = $this->privAddList($p_list, $p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir,
            $p_options);
    ...
    function privAddList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
    {
        ...
        // ----- Add the files
        $v_header_list = array();
        if (($v_result = $this->privAddFileList($p_list, $v_header_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir,
                $p_options)) != 1
        ) {
            // ----- Return
            //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, $v_result);
    ...
    function privAddFileList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
    {
        ...
        for ($j = 0; ($j < count($p_list)) && ($v_result == 1); $j++) {
            ...
            if ((is_file($p_filename)) || ((is_dir($p_filename)) && !$p_remove_all_dir)) {
                // ----- Add the file
                if (($v_result = $this->privAddFile($p_filename, $v_header, $p_add_dir, $p_remove_dir,
                        $p_remove_all_dir, $p_options)) != 1
                ) {
                    // ----- Return status
                    //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, $v_result);
                    return $v_result;
...
//          $p_hitem = @readdir($p_hdir); // '..' directory
                    while (($p_hitem = @readdir($p_hdir)) !== false) {
                        ...
                        if (@is_file($v_path . $p_hitem)) {
                            ...
                            // ----- Add the file
                            if (($v_result = $this->privAddFile($v_path . $p_hitem, $v_header, $p_add_dir,
                                    $p_remove_dir, $p_remove_all_dir, $p_options)) != 1
                            ) {
                                // ----- Return status
                                //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, $v_result);
                                return $v_result;
...
// --------------------------------------------------------------------------------
    ...
    function add($p_filelist /* options */)
    {
        ...
        if (is_array($p_filelist)) {
            // ----- Call the create fct
            $v_result = $this->privAdd($p_filelist, $p_result_list, $v_add_path, $v_remove_path, $v_remove_all_path,
                $v_options);
        } // ----- Look if the $p_filelist is a string
        else {
            if (is_string($p_filelist)) {
                ...
                // ----- Call the create fct
                $v_result = $this->privAdd($v_list, $p_result_list, $v_add_path, $v_remove_path, $v_remove_all_path,
                    $v_options);
            } // ----- Invalid variable
            else {
                // ----- Error log
    ...
    function privAdd($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
    {
        ...
        if ((!is_file($this->zipname)) || (filesize($this->zipname) == 0)) {
            ...
            // ----- Do a create
            $v_result = $this->privCreate($p_list, $p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir,
                $p_options);
    ...
    function privReadEndCentralDir(&$p_central_dir)
    {
        ...
        if (mb_strlen($v_binary_data, 'iso-8859-1') != 18) {
            ...
            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT,
                "Invalid End of Central Dir Record size : " . mb_strlen($v_binary_data, 'iso-8859-1'));
    ...
    function privReadCentralFileHeader(&$p_header)
    {
        ...
        if (mb_strlen($v_binary_data, 'iso-8859-1') != 42) {
            ...
            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT,
                "Invalid block size : " . mb_strlen($v_binary_data, 'iso-8859-1'));
