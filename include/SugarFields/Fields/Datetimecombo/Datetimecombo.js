 ...
 */function Datetimecombo(datetime,field,timeformat,tabindex,showCheckbox,checked,allowEmptyHM,defaultValue=""){this.defaultValue=defaultValue;this.datetime=datetime;this.allowEmptyHM=allowEmptyHM;if(typeof this.datetime=="undefined"||datetime==''||trim(datetime).length<10){this.datetime='';var d=new Date();var month=d.getMonth();var date=d.getDate();var year=d.getYear();var hours=d.getHours();var minutes=d.getMinutes();}
...
this.meridiem=this.hasMeridiem?trim(this.datetime.substring(16)):'';this.datetime=this.datetime.substr(0,10);this.showCheckbox=showCheckbox;this.checked=parseInt(checked);YAHOO.util.Selector.query('input#'+this.fieldname+'_date')[0].value=this.datetime;}
...
var minuteSteps=15;if(typeof SUGAR.config.datetime_combo_minute_interval!=='undefined'){minuteSteps=parseInt(SUGAR.config.datetime_combo_minute_interval);}
if(this.defaultValue.includes('now')){minuteSteps=1;}
for(i=0;i<60;i+=minuteSteps){text+='\n<option value="'+(i<10?'0':'')+i+'" '+(this.mins==i?"SELECTED":"")+'>'+(i<10?'0':'')+i+'</option>';}
