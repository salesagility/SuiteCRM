            ...
            if (isset($primary_id) || isset($secondary_ids)) {
                ...
                if (isset($GLOBALS['beanList'][$this->related_module])) {
                    ...
                    if (file_exists($GLOBALS['beanFiles'][$class])) {
                        ...
                        if (isset($mod->name)) {
                            $this->bean->{$this->value_name} = array_merge($this->bean->{$this->value_name},
                                array('primary' => array('id' => $primary_id, 'name' => $mod->name)));
                        }
                        $secondaries = array();
        ...
        if (isset($this->displayParams['collection_field_list'])) {
            ...
            foreach ($this->displayParams['collection_field_list'] as $k => $v) {
                ...
                if ($collection_field_vardef['type'] == 'relate') {
                    ...
                    $javascript = $tph->createQuickSearchCode(array($collection_field_vardef['name'] => $collection_field_vardef),
                        array($v), $this->form_name);
                    $javascript = str_replace('<script language="javascript">' . "if(typeof sqs_objects == 'undefined'){var sqs_objects = new Array;}sqs_objects['{$collection_field_vardef['name']}']=",
                        "", $javascript);
                ...
                $this->displayParams['collection_field_list'][$name]['field'] = $sfh->displaySmarty('displayParams.collection_field_list',
                    $collection_field_vardef, $viewtype, $v['displayParams'], 1);
...
FRA;
    ...
    function createQuickSearchCode($returnAsJavascript = true)
    {
        ...
        for ($i = 0; $i < $this->numFields; $i++) {
            ...
            if (!$this->skipModuleQuickSearch && preg_match('/(Campaigns|Teams|Users|Accounts)/si',
                    $this->related_module, $matches)
            ) {
                if ($matches[0] == 'Users') {
                    ...
                    if ($matches[0] == 'Campaigns') {
                        ...
                        if ($matches[0] == 'Users') {
                            ...
                            if ($matches[0] == 'Accounts') {
                                ...
                                $sqs_objects[$name1] = $qsd->getQSAccount($nameKey, $idKey, $billingKey, $shippingKey,
                                    $additionalFields);
                            } else {
                ...
                $sqs_objects[$name1]['populate_list'] = array(
                    "{$this->vardef['name']}_collection_{$i}",
                    "id_{$this->vardef['name']}_collection_{$i}"
                );
                ...
                if (isset($this->displayParams['field_to_name_array'])) {
                    ...
                    if (isset($field['field_list']) && isset($field['populate_list'])) {
                        $sqs_objects[$name1]['primary_populate_list'] = array_merge($sqs_objects[$name1]['populate_list'],
                            $field['field_list']);
                        $sqs_objects[$name1]['primary_field_list'] = array_merge($sqs_objects[$name1]['field_list'],
                            $field['populate_list']);
                    } else {
    ...
    function createPopupCode()
    {
        ...
        if (isset($this->field_to_name_array) && !empty($this->field_to_name_array)) {
            ...
            $this->displayParams['popupData'] = '{literal}' . str_replace(array('{{', '}}'), array('{ {', '} }'),
                    $this->json->encode($popup_request_data)) . '{/literal}';
        }
    }
