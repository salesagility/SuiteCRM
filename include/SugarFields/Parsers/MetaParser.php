...
class MetaParser
{
    ...
    {
        preg_match_all("'(<[ ]*?)(textarea|input|select)[^>]*?name=[\'\"]([^\'\"]*?)(\[\])?(_basic)?[\'\"]([^>]*?>)'si",
...
//Find all blocks that may need to be replaced with Smarty syntax
    ...
    function findAssignedVariableName($name, $filePath)
    {
        ...
        if (preg_match("/xtpl->assign[\(][\"\']" . $uname . "[\"\'][\s]*?,[\s]*?[\$]focus->(.*?)[\)]/si", $phpContents,
            $matches)) {
            return $matches[1];
        }
...
?>";
                ...
                foreach ($panels[$name] as $rowKey => $row) {
                    foreach ($row as $colKey => $column) {
                        if (is_array($column) && !empty($column['name'])) {
                            ...
                            $existingLocation[$column['name']] = array(
                                "panel" => $name,
                                "row" => $rowKey,
                                "col" => $colKey
                            );
    ...
    function mergeTemplateMeta($templateMeta, $moduleDir, $masterCopy)
    {
        ...
        if (isset($masterTemplateMeta['javascript'])) {
            ...
            $masterTemplateMeta['javascript'] = preg_replace('/src\s*=\s*[\'\"].*?(modules\/|include\/)([^\.]*?\.js)([^\'\"]*?)[\'\"]/i',
                'src="@sq . getJSPath(@sq${1}${2}@sq) . @sq"', $masterTemplateMeta['javascript']);
        }
