...
class ListViewDisplay
{
    ...
    function shouldProcess($moduleDir)
    {
        ...
        if (!empty($GLOBALS['sugar_config']['save_query']) &&
    ...
    function buildSelectLink($id = 'select_link', $total = 0, $pageTotal = 0, $location = "top")
    {
        ...
        if (!empty($GLOBALS['sugar_config']['disable_count_query']) &&
            $GLOBALS['sugar_config']['disable_count_query'] === true && $total > $pageTotal
        ) {
            $this->show_plus = true;
    ...
    protected function buildActionsLink($id = 'actions_link', $location = 'top')
    {
        ...
        if (isset($this->templateMeta['form']['actions'])) {
            ...
            if (ACLController::checkAccess($this->seed->module_dir, 'delete', true) && $this->delete) {
                $menuItems[] = $this->show_action_dropdown_as_delete ?
                    $this->buildDeleteLink($location) : $this->buildBulkActionButton($location);
            }
            ...
            if ((ACLController::checkAccess($this->seed->module_dir, 'edit',
                        true
                    ) && ACLController::checkAccess($this->seed->module_dir, 'massupdate',
                        true
                    )
    ...
    protected function buildMergeLink(array $modules_array = null, $loc = 'top')
    {
        ...
        if ($user_merge === 'on' && isset($admin->settings['system_mailmerge_on']) &&
            $admin->settings['system_mailmerge_on'] && !empty($modules_array[$module_dir])
        ) {
            return "<a href='javascript:void(0)'  " .
...
EOF;
    ...
    public function getMultiSelectData()
    {
        ...
        $select_entire_list = ($massUpdateRun) ? 0 : (isset($_POST['select_entire_list']) ?
            $_POST['select_entire_list'] : (isset($_REQUEST['select_entire_list']) ?
     ...
     * Similar vardef field values do not override field values from displayColumns,
     * only necessary and missing ones are added
     */
        ...
        foreach ($this->displayColumns as $columnName => $def) {
            ...
            if (empty($this->displayColumns[$columnName]['type'])) {
                if (!empty($seedDef['type'])) {
                    $this->displayColumns[$columnName]['type'] = (!empty($seedDef['custom_type'])) ?
                        $seedDef['custom_type'] : $seedDef['type'];
                } else {
            ...
            if ($this->displayColumns[$columnName]['type'] === "relate" &&
                !empty($seedDef['link']) && empty($this->displayColumns[$columnName]['module'])
            ) {
                $link = $seedDef['link'];
                if (!empty($this->lvd->seed->field_defs[$link]) &&
                    !empty($this->lvd->seed->field_defs[$seedDef['link']]['module'])
                ) {
            ...
            if (isset($seedDef)) {
                ...
                // making sure the seedDef doesn't override anything explicitly set in the displayColumns array.
            ...
            if (!isset($this->displayColumns[$columnName]['id']) &&
                isset($this->displayColumns[$columnName]['id_name'])
            ) {
