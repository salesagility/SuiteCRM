...
class MssqlManager extends DBManager
{
    ...
    {
        return $this->getOne("select IDENT_CURRENT('{$table}') + IDENT_INCR ( '{$table}' ) as 'Auto_increment'");
    }
    ...
    protected function _appendN($sql)
    {
        ...
        if (!empty($matches)) {
            foreach ($matches[0] as $value) {
                ...
                if (
                    !empty($value)
                    && !is_numeric(trim(str_replace(["'", ','], '', $value)))
                    && !preg_match('/^\'[\,]\'$/', $value)
                ) {
    ...
    private function getAliasFromSQL($sql, $alias)
    {
        ...
        foreach ($mstr_sql_array as $token) {
            if (strpos($token, $alias)) {
                ...
                return $this->addPatternToSQL($token, $sin_array);
