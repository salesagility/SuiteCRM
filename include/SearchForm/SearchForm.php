...
class SearchForm
{
    ...
    function __construct($module, &$seedBean, $tpl = null)
    {
        ...
        $this->tabs = array(
            array(
                'title' => $app_strings['LNK_BASIC_FILTER'],
                'link' => $module . '|basic_search',
                'key' => $module . '|basic_search'
            ),
            array(
                'title' => $app_strings['LNK_ADVANCED_FILTER'],
        ...
        if (file_exists('modules/' . $this->module . '/index.php')) {
            $this->tabs[] = array(
                'title' => $app_strings['LNK_SAVED_VIEWS'],
                ...
                if ($field_value != '') {
                    foreach ($parms['db_field'] as $db_field) {
                        ...
                        if ($type == 'datetime' || $type == 'datetimecombo') {
                            ...
                            $field_value = array(
                                $this->bean->db->convert($dates["start"], "datetime"),
    ...
    function displaySavedViews($listViewDefs, $lv, $header = true, $return = false)
    {
        ...
        $savedSearch = new SavedSearch($listViewDefs[$this->module], $lv->data['pageData']['ordering']['orderBy'],
            $lv->data['pageData']['ordering']['sortOrder']);

        if ($header) {
            $this->displayWithHeaders('saved_views', $this->displayBasic(false, true),
                $this->displayAdvanced(false, true), $savedSearch->getForm($this->module));
    ...
    function displayHeader($view)
    {
        ...
        if (is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])) {
            $header_text = "<a href='index.php?action=index&module=DynamicLayout&from_action=SearchForm&from_module=" . $_REQUEST['module'] . "'>" . SugarThemeRegistry::current()->getImage("EditLayout",
                    "border='0' align='bottom'", null, null, '.gif', 'Edit Layout') . "</a>";
        }
    ...
    function displayAdvanced($header = true, $return = false, $listViewDefs = '', $lv = '')
    {
        ...
        if (!empty($listViewDefs) && !empty($lv)) {
            ...
            $this->xtpl->assign('ADVANCED_SEARCH_IMG',
                SugarThemeRegistry::current()->getImageURL('advanced_search.gif'));
