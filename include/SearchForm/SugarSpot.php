...
class SugarSpot
{
    ...
    protected function _performSearch($query, $modules, $offset = -1, $limit = 20)
    {
        ...
        $limit = !empty($GLOBALS['sugar_config']['max_spotresults_initial']) ?
            $GLOBALS['sugar_config']['max_spotresults_initial'] : 5;
        if ($offset !== -1) {
            $limit = !empty($GLOBALS['sugar_config']['max_spotresults_more']) ?
                $GLOBALS['sugar_config']['max_spotresults_more'] : 20;
        }
        ...
        foreach ($modules as $moduleName) {
            ...
            foreach ($searchFields[$moduleName] as $k => $v) {
                ...
                if (!empty($GLOBALS['dictionary'][$class]['unified_search'])) {
                    ...
                    if (empty($GLOBALS['dictionary'][$class]['fields'][$k]['unified_search'])) {
                            ...
                            foreach ($searchFields[$moduleName][$k]['db_field'] as $field) {
                                if (!empty($GLOBALS['dictionary'][$class]['fields'][$field]['unified_search'])) {
                                    if (isset($GLOBALS['dictionary'][$class]['fields'][$field]['type'])) {
                                        if (!$this->filterSearchType($GLOBALS['dictionary'][$class]['fields'][$field]['type'],
                                            $query
                                        )
                                        ) {
                        ...
                        if (!$keep && (isset($v['force_unifiedsearch']) === false ||
                                $v['force_unifiedsearch'] !== true)
                        ) {
                    ...
                    if (empty($GLOBALS['dictionary'][$class]['fields'][$k])) {
                        //If module did not have unified_search defined, then check the exception for an email search
                        if (strpos($k, 'email') === false || !$searchEmail) {
                            unset($searchFields[$moduleName][$k]);
                        }
                    } else {
            ...
            foreach ($seed->field_defs as $k => $v) {
                if (isset($seed->field_defs[$k]['type']) && ($seed->field_defs[$k]['type'] === 'name') &&
                    !isset($return_fields[$k])
                ) {
            ...
            if (!isset($return_fields['name'])) {
                ...
                foreach ($searchFields[$moduleName] as $k => $v) {
                    if (strpos($k,
                            'name'
                        ) != -1 && isset($seed->field_defs[$k]) && !isset($seed->field_defs[$k]['source'])
                    ) {
                        $return_fields[$k] = $seed->field_defs[$k];
                        break;
                    }
            ...
            $searchForm->setup(array($moduleName => array()), $searchFields, '',
            ...
            if (count($where_clauses) > 1) {
                ...
                $ret_array_start = $seed->create_new_list_query('', '', $return_fields, array(), 0, '', true, $seed,
                    true
                );
                ...
                foreach ($where_clauses as $n => $clause) {
                    ...
                    $ret_array = $seed->create_new_list_query('', $clause, $allfields, array(), 0, '', true, $seed,
                        true
                    );
                ...
                $ret_array = $seed->create_new_list_query('', $where_clauses[0], $return_fields, array(), 0, '', true,
                    $seed, true
                );
    ...
    protected function _searchKeys($item1, $key, $patterns)
    {
        ...
        if ($module_exists !== false && $pattern_exists !== false) {
            $GLOBALS['matching_keys'] = array_merge(array(array('NAME' => $key, 'ID' => $key, 'VALUE' => $item1)),
                $GLOBALS['matching_keys']
            );
        } else {
     ...
     * filterSearchType
     *
     * This is a private function to determine if the search type field should be filtered
     * based on the query string value
     *
