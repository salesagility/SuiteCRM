...
class SearchForm
{
    ...
    public function __construct($seed, $module, $action = 'index', $options = [])
    {
        ...
        $this->module = $module;
        $this->action = $action;
        $this->tabs = [['title' => $GLOBALS['app_strings']['LNK_BASIC_FILTER'],
    ...
    public function setup($searchdefs, $searchFields = [], $tpl = 'SubpanelSearchFormGeneric.tpl', $displayView = 'basic_search', $listViewDefs = [])
    {
        ...
        if ($this->showAdvanced) {
            $this->nbTabs++;
            $this->tabs[] = ['title' => $GLOBALS['app_strings']['LNK_ADVANCED_FILTER'],
    ...
    public function display($header = true)
    {
        ...
        $totalWidth = 0;
        if (isset($this->searchdefs['templateMeta']['widths'], $this->searchdefs['templateMeta']['maxColumns'])
        ) {
        ...
        $searchFormInPopup = !in_array($this->module, isset($sugar_config['enable_legacy_search']) ? $sugar_config['enable_legacy_search'] : []);
    ...
    public function _build_field_defs()
    {
        ...
        if ($this->seed) {
            ...
            foreach ($this->seed->toArray() as $name => $value) {
                ...
                if (isset($this->fieldDefs[$fvName]['options'], $GLOBALS['app_list_strings'][$this->fieldDefs[$fvName]['options']])) {
                    // fill in enums
        ...
        foreach ((array) $this->searchFields as $field => $parms) {
            ...
            if (!empty($parms['enable_range_search']) && empty($type)) {
                if (preg_match('/^start_range_(.*?)$/', $field, $match)) {
                    ...
                    if (isset($this->searchFields[$start_field]['value'], $this->searchFields[$end_field]['value'])) {
            ...
            if (isset($parms['value']) && $parms['value'] != '') {
                ...
                if (is_array($parms['value'])) {
                    ...
                    if (!empty($this->seed->field_name_map[$field]['isMultiSelect']) && $this->seed->field_name_map[$field]['isMultiSelect']) {
                        ...
                        if (isset($this->seed->field_name_map[$field], $this->seed->field_name_map[$field]['source'])
                             &&
                            $this->seed->field_name_map[$field]['source'] == 'non-db'
                        ) {
                ...
                if ($field_value != '' || $operator == 'isnull') {
                    ...
                    foreach ($parms['db_field'] as $db_field) {
                                    ...
                                    if ($type == 'relate' && $customField && !empty($this->seed->field_name_map[$field]['module'])) {
                                        ...
                                        if (!$customField) {
                                            ...
                                            // Relationship fields get the name directly from the field_name_map
                                                ...
                                                if (!(isset($this->seed->field_name_map[$db_field], $this->seed->field_name_map[$db_field]['source']) && $this->seed->field_name_map[$db_field]['source'] == 'non-db')) {
                        ...
                        if (empty($type) && isset($parms['db_field'], $parms['db_field'][0], $this->seed->field_defs[$parms['db_field'][0]]['type'])) {
                            ...
                            case 'like':
                                if ($type == 'bool' && $field_value == 0) {
                                    ...
                                    if ($UnifiedSearch && strpos($field_value, ' ') !== false && strpos($db_field, $parms['db_field'][count($parms['db_field']) - 1]) !== false) {
                                        ...
                                        if (isset($this->seed->field_name_map, $this->seed->field_name_map[$db_field])) {
    ...
    {
        $paths = isset($this->options['locator_class_params']) ? $this->options['locator_class_params'][0] : [];
