...
class Sugarpdf extends TCPDF
{
    ...
    function preDisplay()
    {
        ...
        $this->SetProtection($protection,
            blowfishDecode(blowfishGetKey('sugarpdf_pdf_user_password'), PDF_USER_PASSWORD),
            blowfishDecode(blowfishGetKey('sugarpdf_pdf_owner_password'), PDF_OWNER_PASSWORD));
    ...
    public function writeCellTable($item, $options = null)
    {
        ...
        if (!isset($options['isheader']) || $options['isheader'] == true) {
            ...
            foreach ($header as $v) {
                $this->MultiCell($options["width"][$v], $h, $v, $headerOptions['border'], $headerOptions['align'],
                    $headerOptions['fillstate'], 0, '', '', true, $options['stretch'][$v], $headerOptions['ishtml']);
            }
        ...
        foreach ($item as $k => $line) {
            ...
            foreach ($line as $kk => $cell) {
                ...
                if ($firstrow) {
                    $this->MultiCell($options["width"][$kk], $h, $value, $cellOptions['border'], $cellOptions['align'],
                        $cellOptions['fillstate'], 0, '', '', true, $options['stretch'][$kk], $cellOptions['ishtml'],
                        true, 0, false);
                } else {
                    if ($firstcell) {
                        // add page only once (for the first cell)
                        $this->MultiCell($options["width"][$kk], $h, $value, $cellOptions['border'],
                            $cellOptions['align'], $cellOptions['fillstate'], 0, '', '', true, 0,
                            $cellOptions['ishtml'], true, 0, true);
                        $firstcell = false;
                    } else {
                        $this->MultiCell($options["width"][$kk], $h, $value, $cellOptions['border'],
                            $cellOptions['align'], $cellOptions['fillstate'], 0, '', '', true, 0,
                            $cellOptions['ishtml'], true, 0, false);
                    }
                }
                        ...
                        if (ceil($this->GetStringWidth($wordBlock) / $wmax) > 1) {
                            if ($first) {
                                ...
                                if ($run && $lastNum == ceil($this->GetStringWidth($wordBlock) / $wmax)) {
                                    ...
                                    $first = true;
                                    $lines += ceil($this->GetStringWidth(substr($wordBlock, 0,
                                            (strlen($wordBlock) - strlen(" " . $words[$i])))) / $wmax);
                                    $i--;
                                    $lastNum = 0;
                                    $run = false;
                                }
