...
class SugarEmailAddress extends SugarBean
{
    ...
    {
        // STIC Custom - JCH - 20220929 - Avoid email overwriting
        // STIC#874
        // STIC#956
        // Avoid overwriting email values if current bean is not the GUI bean 
        if(!empty($_REQUEST['record']) 
            && !empty($bean->id) 
            && !empty($_REQUEST['module']) 
            && !empty($bean->module_name) 
            && $_REQUEST['record'] != $bean->id
            && $_REQUEST['module'] == $bean->module_name){
                $GLOBALS['log']->stic("[PR_956]: module={$bean->module_name} | email1={$bean->email1} | id={$bean->id} | \$_REQUEST['record']={$_REQUEST['record']}");
                $stackTrace = new \Exception;
                $GLOBALS['log']->stic("[PR_956] : [Stack_Trace] ", $stackTrace->getTraceAsString());
                // return;
        }
        // END STIC Custom
    ...
    private function needsToParseLegacyAddresses($bean)
    {
        if (
            ...
            // STIC-custom - JCH - 20220314 - Checking if email1 is not empty too (mandatory for email inline editing)
