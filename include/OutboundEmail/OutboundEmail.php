...
class OutboundEmail
{
    ...
    {
        $query =
            "SELECT id FROM outbound_email WHERE user_id = '{$user_id}' AND type = 'system-override' ORDER BY name";
    ...
    public function doesUserOverrideAccountRequireCredentials($user_id)
    {
        ...
        if ($userOverideAccount == null ||
            empty($userOverideAccount->mail_smtpuser) ||
            empty($userOverideAccount->mail_smtpuser)) {
    ...
    public function save()
    {
        ...
        if ($this->new_with_id) {
            ...
            $q = "UPDATE outbound_email SET " . implode(', ', $updvalues) .
