...
class aSubPanel
{
    ...
    function aSubPanel(
        ...
        self::__construct($name, $instance_properties, $parent_bean, $reload, $original_only, $search_query,
            $collections);
    }
    ...
    function load_sub_subpanels()
    {
        ...
        if (empty ($this->sub_subpanels)) {
            ...
            foreach ($panels as $panel => $properties) {
                if (array_key_exists($properties ['module'], $modListHeader) or array_key_exists($properties ['module'],
                        $modules_exempt_from_availability_check)
                ) {
                    $this->sub_subpanels [$panel] = new aSubPanel ($panel, $properties, $this->parent_bean, false,
                        false, $this->search_query, $this->base_collection_list);
                }
            }
    ...
    function get_buttons()
    {
        ...
        if ($mod_name == 'Activities' || $mod_name == 'History') {
            global $modListHeader;
            global $modules_exempt_from_availability_check;
            if (isset ($modListHeader) && (!(array_key_exists('Emails', $modListHeader) or array_key_exists('Emails',
                        $modules_exempt_from_availability_check)))
            ) {
...
class SubPanelDefinitions
{
    ...
    function load_subpanel($name, $reload = false, $original_only = false, $search_query = '', $collections = array())
    {
        ...
        $subpanel = new aSubPanel ($name, $this->layout_defs ['subpanel_setup'] [strtolower($name)], $this->_focus,
            $reload, $original_only, $search_query, $collections);
    ...
    function get_available_tabs($FromGetModuleSubpanels = false)
    {
        ...
        if (isset ($this->layout_defs ['subpanel_setup'])) // bug 17434 - belts-and-braces - check that we have some subpanels first
        {
            ...
            foreach ($this->layout_defs ['subpanel_setup'] as $key => $values_array) {
                ...
                if ($hidden_panels && is_array($hidden_panels) && (in_array($module,
                            $hidden_panels) || array_key_exists($module, $hidden_panels))
                ) {
                ...
                if (!isset($values_array ['module'])) {
                    $GLOBALS['log']->debug("SubPanelDefinitions->get_available_tabs(): no module defined in subpaneldefs for '$key' =>" . var_export($values_array,
                            true) . " - ingoring subpanel defintion");
                    continue;
                }

                //check permissions.
                $exempt = array_key_exists($values_array ['module'], $modules_exempt_from_availability_check);
                $ok = $exempt || ((!ACLController::moduleSupportsACL($values_array ['module']) || ACLController::checkAccess($values_array ['module'],
                            'list', true)));
