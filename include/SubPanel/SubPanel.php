...
//		}
        ...
        $ListView->smartyTemplateAssign("RETURN_URL",
        ...
        $ListView->smartyTemplateAssign("EDIT_INLINE_PNG",
            SugarThemeRegistry::current()->getImage('edit_inline', 'align="absmiddle"  border="0"', null, null, '.gif',
                $app_strings['LNK_EDIT']));
        $ListView->smartyTemplateAssign("DELETE_INLINE_PNG",
            SugarThemeRegistry::current()->getImage('delete_inline', 'align="absmiddle" border="0"', null, null, '.gif',
                $app_strings['LBL_DELETE_INLINE']));
        $ListView->smartyTemplateAssign("REMOVE_INLINE_PNG",
            SugarThemeRegistry::current()->getImage('delete_inline', 'align="absmiddle" border="0"', null, null, '.gif',
        ...
        if (is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])) {
            ...
            $header_text = "&nbsp;<a href='index.php?action=index&module=DynamicLayout&from_action=$file_name&from_module=$mod_name&mod_lang="
                . $_REQUEST['module'] . "'>" . SugarThemeRegistry::current()->getImage("EditLayout",
    ...
    function getSearchForm()
    {
        ...
        if ($subpanel_searchMetaData = $this->get_searchdefs($module)) {
            ...
            $searchForm->setup($subpanel_searchMetaData, $searchMetaData['searchFields'],
                'SubpanelSearchFormGeneric.tpl', 'basic_search');
    ...
    function get_searchdefs($module)
    {
        ...
        if (isset($subpanel_defs['searchdefs'])) {
            ...
            $searchdefs[$module]['templateMeta'] = Array(
                'maxColumns' => 3,
                'maxColumnsBasic' => 4,
...
//  	$GLOBALS['log']->debug('SubPanel.php->saveSubPanelDefOverride(): '.$name);
        ...
        $fp = sugar_fopen('custom/Extension/modules/' . $panel->parent_bean->module_dir . "/Ext/Layoutdefs/$extname.php",
            'w');
