...
abstract class DashletGenericChart extends Dashlet
{
    ...
    public function saveOptions(
        ...
        foreach ($this->_searchFields as $name => $params) {
            ...
            if ($widgetDef['type'] == 'date') {
                ...
                $options[$widgetDef['name']] = $timedate->swap_formats($req['type_' . $widgetDef['name']],
                    $timedate->get_date_format(), $timedate->dbDayFormat);
            } elseif ($widgetDef['type'] == 'time') {
                // special case time types
                $options[$widgetDef['name']] = $timedate->swap_formats($req['type_' . $widgetDef['name']],
                    $timedate->get_time_format(), $timedate->dbTimeFormat);
            } elseif ($widgetDef['type'] == 'datepicker') {
                // special case datepicker types
                $options[$widgetDef['name']] = $timedate->swap_formats($req[$widgetDef['name']],
                    $timedate->get_date_format(), $timedate->dbDayFormat);
    ...
    public function displayOptions()
    {
        ...
        if (is_array($this->_searchFields)) {
            foreach ($this->_searchFields as $name => $params) {
                if (!empty($name)) {
                    ...
                    $currentSearchFields[$name]['label'] = translate($widgetDef['vname'],
                        $this->getSeedBean()->module_dir);
        ...
        if ($this->isAutoRefreshable()) {
            ...
            $this->getConfigureSmartyInstance()->assign('autoRefresh',
