...
$GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide stic_Centers subpanel', $currentTabs);

$modulesToHideNotEnabled = array_diff($modulesToHide, $currentTabs);
$GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide stic_Centers subpanel', $modulesToHideNotEnabled);
if (count($modulesToHideNotEnabled)> 0){
    // This function needs to be customized if the file is copied.
    function sticCentersPrepareElementToHide(&$item, $key)
    {
        $item = strtolower($item);
    }
    
    array_walk($modulesToHideNotEnabled, 'sticCentersPrepareElementToHide');
    ...
    $GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide stic_Centers subpanel', $unserialized);
