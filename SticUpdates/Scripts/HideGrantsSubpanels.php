 ...
 **/
 ...
 $GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide stic_Grants subpanel starting');
...
$GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide stic_Grants subpanel', $currentTabs);

$modulesToHideNotEnabled = array_diff($modulesToHide, $currentTabs);
$GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide stic_Grants subpanel', $modulesToHideNotEnabled);
if (count($modulesToHideNotEnabled)> 0){
    // This function needs to be customized if the file is copied.
    function sticGrantsPrepareElementToHide(&$item, $key)
    {
        $item = strtolower($item);
    }
    
    array_walk($modulesToHideNotEnabled, 'sticGrantsPrepareElementToHide');
    ...
    $GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide stic_Grants subpanel', $unserialized);
