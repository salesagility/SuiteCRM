...
if ((!isset($_REQUEST['isProfile']) && empty($_REQUEST['id'])) || empty($_REQUEST['type']) || !isset($_SESSION['authenticated_user_id'])) {
    ...
    // STIC#1086 - Add the download of files related to the import validation process 
    elseif (isset($_REQUEST['isTempFile']) && $file_type == "stic_import_validation") {
        $local_location = "upload://stic_Import_Validation/{$_REQUEST['tempName']}";    
    }
    // END STIC-Custom
    else {
