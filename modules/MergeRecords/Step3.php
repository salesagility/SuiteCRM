    ...
    if (show_field($field_array)
        ...
        switch ($field_check) {
            ...
            case ('text') :
                ...
                break;
            case ('enum') :
                $xtpl->assign("SELECT_OPTIONS", get_select_options_with_id($app_list_strings[$field_array['options']],
            ...
            case ('link') :
                ...
                $popup_data = array(
                    ...
                    'form_name' => 'EditView',
                    'field_to_name_array' => array('id' => $tempId, 'name' => $tempName,),
                );
        ...
        foreach ($merge_ids_array as $id) {
            ...
            switch ($field_check) {
                ...
                case ('multienum') :
                    if ($mergeBeanArray[$id]->$tempName != '' and isset($field_array['options']) and isset($app_list_strings[$field_array['options']][$mergeBeanArray[$id]->$tempName])) {
                        display_field_value(str_replace("^", "",
            ...
            if ($field_check == 'relate' or $field_check == 'link') {
                ...
                $json_data['popup_fields'] = Array(
                    $tempName => $mergeBeanArray[$id]->$tempName,
                    $tempId => $mergeBeanArray[$id]->$tempId,
                );
            } else {
                if ($field_check == 'teamset') {
                    $json_data['field_value'] = TeamSetManager::getCommaDelimitedTeams($mergeBeanArray[$id]->team_set_id,
                        $mergeBeanArray[$id]->team_id, true);
...
if ($diff_field_count > 0) {
    $xtpl->assign("DIFF_HEADER",
        "<tr height='20'><td colspan=2><strong>{$mod_strings['LBL_DIFF_COL_VALUES']}</strong></td>" . implode(' ',
            $header_cols) . "</tr>");
    $xtpl->assign("SIMILAR_HEADER",
        "<tr height='20'><td colspan=20><strong>{$mod_strings['LBL_SAME_COL_VALUES']}</strong></td></tr>");
    $xtpl->assign("GROUP_PARTITION", "<tr height=3><td colspan=20' class='listViewHRS1'></td></tr>");
} else {
    $xtpl->assign("SIMILAR_HEADER",
        "<tr height='20'><td colspan=2><strong>{$mod_strings['LBL_SAME_COL_VALUES']}</strong></td>" . implode(' ',
            $header_cols) . "</tr>");
}
