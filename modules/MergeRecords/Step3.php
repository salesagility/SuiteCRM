...
$filter_for_valid_related_attributes = [['type' => 'link']];
$filter_for_invalid_related_attributes = [['type' => 'link', 'link_type' => 'one']];

//following attributes will be ignored from the merge process.
$invalid_attribute_by_name = ['date_entered' => 'date_entered', 'date_modified' => 'date_modified', 'modified_user_id' => 'modified_user_id', 'created_by' => 'created_by', 'deleted' => 'deleted'];
    ...
    if (show_field($field_array)
        ...
        switch ($field_check) {
            ...
            case 'link':
                ...
                $popup_data = ['call_back_function' => 'set_return', 'form_name' => 'EditView', 'field_to_name_array' => ['id' => $tempId, 'name' => $tempName]];
        ...
        foreach ($merge_ids_array as $id) {
            ...
            $json_data = ['field_name' => $tempName, 'field_type' => $field_check];
            ...
            if ($field_check == 'relate' or $field_check == 'link') {
                $temp_array = [];
                $tempId = $field_array['id_name'];
                $json_data['popup_fields'] = [$tempName => $mergeBeanArray[$id]->{$tempName}, $tempId => $mergeBeanArray[$id]->{$tempId}];
            } else {
