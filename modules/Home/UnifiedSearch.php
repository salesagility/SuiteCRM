...
if (!$luceneSearch) {
    if (!empty($sugar_config['aod']['enable_aod'])) {
        echo "<a href='index.php?action=UnifiedSearch&module=Home&query_string=" . urlencode($queryString) . "'>" . translate("LBL_USE_AOD_SEARCH",
                "AOD_Index") . "</a>";
    }
...
echo "<a href='index.php?action=UnifiedSearch&module=Home&search_fallback=1&query_string=" . urlencode($queryString) . "'>" . translate("LBL_USE_VANILLA_SEARCH",
        "AOD_Index") . "</a>";
...
?>
    <form name='UnifiedSearchAdvancedMain' action='index.php' method='POST' class="search_form">
        ...
        <input id='searchFieldMain' class='searchField' type='text' size='80' name='query_string'
               placeholder='<?php echo translate("LBL_SEARCH_QUERY_PLACEHOLDER", "AOD_Index"); ?>'
               value='<?php echo $queryString; ?>'>
...
function getPaginateHTML($queryString, $start, $amount, $total)
{
    ...
    <td class="paginationChangeButtons" align="right" nowrap="nowrap" width="1%">
        <form action='index.php'>
            ...
            <button type="submit" id="listViewStartButton_top" name="listViewStartButton" title="Start"
                    class="button" <?php echo $first ? 'disabled="disabled"' : '' ?>>
                <img src="<?php echo $startImage; ?>" alt="Start" align="absmiddle" border="0">
            </button>
            <button type="submit" id="listViewPrevButton_top" name="listViewPrevButton" class="button"
                    title="Previous" <?php echo $first ? 'disabled="disabled"' : '' ?>>
                <img src="<?php echo $prevImage; ?>" alt="Previous" align="absmiddle" border="0">
            </button>
            <span class="pageNumbers">(<?php echo $total ? $start + 1 : 0; ?> - <?php echo min($start + $amount,
                    $total); ?> of <?php echo $total; ?>)</span>
            <button type="submit" id="listViewNextButton_top" name="listViewNextButton" title="Next"
                    class="button" <?php echo $last ? 'disabled="disabled"' : '' ?>>
                <img src="<?php echo $nextImage; ?>" alt="Next" align="absmiddle" border="0">
            </button>
            <button type="submit" id="listViewEndButton_top" name="listViewEndButton" title="End"
                    class="button" <?php echo $last ? 'disabled="disabled"' : '' ?>>
