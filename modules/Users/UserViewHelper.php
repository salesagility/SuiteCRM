 ...
 * There are a lot of fields on those views that do not map directly to being used on the metadata based UI, so they
 * are handled here.
 */
class UserViewHelper
{
    ...
    protected function assignUserTypes()
    {
        ...
        if ((!empty($this->bean->is_group) && $this->bean->is_group)
    ...
    protected function setupButtonsAndTabs()
    {
        ...
        if (!$this->bean->is_group) {
            ...
            $buttons_header[] = "<input type='button' class='button' id='reset_user_preferences_header' " .
                "onclick='if(confirm(\"{$reset_pref_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' .
                $user_preference_url . "&reset_preferences=true\";' value='" . translate(
                    'LBL_RESET_PREFERENCES',
                    'Users'
                ) . "' />";
            $buttons_header[] = "<input type='button' class='button' id='reset_homepage_header' " .
                "onclick='if(confirm(\"{$reset_home_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' .
                $the_query_string . "&reset_homepage=true\";' value='" . translate(
                    'LBL_RESET_HOMEPAGE',
                    'Users'
                ) . "' />";

            $buttons_footer[] = "<input type='button' class='button' id='reset_user_preferences_footer' " .
                "onclick='if(confirm(\"{$reset_pref_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' .
                $user_preference_url . "&reset_preferences=true\";' value='" . translate(
                    'LBL_RESET_PREFERENCES',
                    'Users'
                ) . "' />";
            $buttons_footer[] = "<input type='button' class='button' id='reset_homepage_footer' " .
                "onclick='if(confirm(\"{$reset_home_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' .
                $the_query_string . "&reset_homepage=true\";' value='" . translate(
                    'LBL_RESET_HOMEPAGE',
                    'Users'
     ...
     *
     * This function handles setting up the user type dropdown field.  It determines which user types are available for
     * the current user. At the end of the function two Smarty variables (USER_TYPE_DROPDOWN and USER_TYPE_READONLY)
     * are assigned.
     *
     */
    public function setupUserTypeDropdown()
    {
        ...
        foreach ($availableUserTypes as $currType) {
            if ($setSelected && $currType == $userType) {
                $userTypeDropdown .= '<option value="' . $currType . '" SELECTED>'
                    . $userTypes[$currType]['label'] . '</option>';
            } else {
                $userTypeDropdown .= '<option value="' . $currType . '">'
                    . $userTypes[$currType]['label'] . '</option>';
            }
        }
        ...
        $this->ss->assign(
            'USER_TYPE_READONLY',
            $userTypes[$userType]['label']
            . "<input type='hidden' id='UserType' value='{$userType}'><div id='UserTypeDesc'>&nbsp;</div>"
        );
    }
    ...
    protected function setupPasswordTab()
    {
        ...
        $this->ss->assign(
            'PWDSETTINGS',
            isset($GLOBALS['sugar_config']['passwordsetting']) ? $GLOBALS['sugar_config']['passwordsetting'] : array()
        );
        ...
        if (((isset($enable_syst_generate_pwd) && !$enable_syst_generate_pwd && $this->usertype != 'GROUP')
                || $this->usertype == 'PORTAL_ONLY') && empty($this->bean->id)) {
        ...
        if ((($current_user->id == $this->bean->id) || $this->usertype == 'PORTAL_ONLY'
                || (($this->usertype == 'REGULAR' || $this->usertype == 'Administrator'
                        || (isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true'
                            && $this->usertype != 'GROUP')) && !$enable_syst_generate_pwd))
    ...
    protected function setupEmailSettings()
    {
        ...
        $this->ss->assign(
            ...
            get_select_options_with_id(
                $app_list_strings['notifymail_sendtype'],
                $this->bean->getPreference('mail_sendtype')
            )
        );
        ...
        $this->ss->assign(
            "NEW_EMAIL",
            '<span id="email_span">' . getEmailAddressWidget(
                $this->bean,
                "email1",
                $this->bean->email1,
                $this->viewType
        ...
        if ($this->viewType == 'EditView') {
            $this->ss->assign(
                'EMAIL_LINK_TYPE',
                get_select_options_with_id($app_list_strings['dom_email_link_type'], $raw_email_link_type)
            );
        } else {
        ...
        if ($this->viewType == 'EditView') {
            $this->ss->assign(
                'EDITOR_TYPE',
                get_select_options_with_id($app_list_strings['dom_editor_type'], $rawEditorType)
            );
        } else {
    ...
    {
        global $locale, $app_list_strings, $sugar_config;
        ...
        if ($this->bean->receive_notifications || (!isset($this->bean->id)
                && $admin->settings['notify_send_by_default'])) {
        ...
        $ical_url = $sugar_config['site_url']
            . "/ical_server.php?type=ics&key=<span id=\"ical_pub_key_span\">$publish_key</span>";
        if (!empty($this->bean->email1)) {
        ...
        $this->ss->assign(
            ...
            . "/vcal_server.php/type=vfb&key=<span id=\"search_pub_key_span\">$publish_key</span>&email=%NAME%@%SERVER%"
        );
    ...
    protected function setupAdvancedTabNavSettings()
    {
        ...
        $chooser->args['title'] = translate('LBL_EDIT_TABS', 'Users') . smarty_function_sugar_help(
            array("text" => translate('LBL_CHOOSE_WHICH', 'Users')),
            $ss
            );
    ...
    protected function setupAdvancedTabLocaleSettings()
    {
        ...
        $nameOptions = get_select_options_with_id(
            $locale->getUsableLocaleNameOptions($sugar_config['name_formats']),
            $nformat
        );
        ...
        if (isset($cur_id)) {
            ...
            $this->ss->assign(
                "CURRENCY_DISPLAY",
                $currencyDisplay->getDefaultISO4217() . ' ' . $currencyDisplay->getDefaultCurrencySymbol()
            );
        }
        ...
        $this->ss->assign(
            "NUM_GRP_SEP",
            (empty($num_grp_sep) ? $GLOBALS['sugar_config']['default_number_grouping_seperator'] : $num_grp_sep)
        );
        $this->ss->assign(
            "DEC_SEP",
            (empty($dec_sep) ? $GLOBALS['sugar_config']['default_decimal_seperator'] : $dec_sep)
        );
