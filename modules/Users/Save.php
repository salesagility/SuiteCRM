...
// Bug 43241 - Changed $focus->id to $focus->user_name to make sure that
// a system generated password is made when converting employee to user
...
if (!$current_user->is_admin && !$GLOBALS['current_user']->isAdminForModule('Users')
    ...
    if ($current_user->id != $focus->id
        ...
        $GLOBALS['log']->fatal(
            "SECURITY:Non-Admin " . $current_user->id . " attempted to change settings for user:" . $focus->id
        );
...
// copy the group or portal user name over.  We renamed the field in order to ensure auto-complete
...
if (!$focus->verify_data()) {
    ...
    if ((isset($_POST['old_password']) || $focus->portal_only) &&
        ...
        if (!$focus->change_password($_POST['old_password'], $_POST['new_password'])) {
            if ((isset($_POST['page']) && $_POST['page'] == 'EditView')) {
                header(
                    "Location: index.php?action=EditView&module=Users&record="
                    . $_POST['record'] . "&error_password=" . urlencode(
                        $focus->error_string
                    )
                );
                exit;
            ...
            if ((isset($_POST['page']) && $_POST['page'] == 'Change')) {
                header(
                    "Location: index.php?action=ChangePassword&module=Users&record="
                    . $_POST['record'] . "&error_password=" . urlencode(
                        $focus->error_string
                    )
                );
    ...
    if (
        ...
        $emailSettings = array();
        $emailSettings['emailCheckInterval']
            = isset($_REQUEST['emailCheckInterval']) ? $_REQUEST['emailCheckInterval'] : null;
        ...
        $emailSettings['defaultOutboundCharset']
            = isset($_REQUEST['default_charset']) ? $_REQUEST['default_charset'] : null;
