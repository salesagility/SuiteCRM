...
function password_confirmation() {
  ...
  if (confirm_pwd.value != new_pwd) confirm_pwd.style.borderColor = "red";
  else confirm_pwd.style.borderColor = "";
  if (confirm_pwd.value != new_pwd.substring(0, confirm_pwd.value.length)) document.getElementById("comfirm_pwd_match").style.display = "inline";
  else document.getElementById("comfirm_pwd_match").style.display = "none";
  if (
    new_pwd != "" ||
    confirm_pwd.value != "" ||
    old_pwd != "" ||
    (document.getElementById("page") && document.getElementById("page").value == "Change")
  )
    document.getElementById("password_change").value = "true";
  else document.getElementById("password_change").value = "false";
}
...
function set_password(form, rules) {
  if (form.password_change.value == "true") {
    ...
    if (form.new_password.value == form.confirm_pwd.value) return true;
    else {
      ...
      for (i = 0; i < document.forms.length; i++) {
        for (j = 0; j < document.forms[i].elements.length; j++) {
          ...
          if (
            ...
            !field.disabled &&
            (field.name == "first_name" || field.name == "name" || field.name == "user_name" || field.name == "document_name")
          ) {
            field.focus();
            if (field.type == "text") {
              field.select();
