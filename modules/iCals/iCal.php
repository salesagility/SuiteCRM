...
class iCal extends vCal
{
    ...
    protected function createSugarIcal(&$user_bean, &$start_date_time, &$end_date_time, $dtstamp)
    {
        ...
        $where = "project_task.assigned_user_id='{$user_bean->id}' " .
            ...
            "AND (project_task.date_start IS NULL OR " . CalendarActivity::get_occurs_within_where_clause('project_task',
                '', $start_date_time, $end_date_time, 'date_start', 'month') . ")";
        ...
        if ($taskAsVTODO) {
            ...
            $where = "tasks.assigned_user_id='{$user_bean->id}' " .
                ...
                "AND (tasks.date_start IS NULL OR " . CalendarActivity::get_occurs_within_where_clause('tasks', '',
                    $start_date_time, $end_date_time, 'date_start', 'month') . ")";
            $seedTask = new Task();
