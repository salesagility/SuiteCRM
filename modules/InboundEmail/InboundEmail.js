...
var moveCenter=function(sel){var $elem=$(sel);var $window=$(window);$elem.css('left',(($window.width()-$elem.width())/ 2)+'px');$elem.css('top',(($window.height()-$elem.height())/ 2)+'px');};function getEncryptedPassword(login,password,mailbox){var words=new Array(login,password,mailbox);for(i=0;i<3;i++){word=words[i];if(word.indexOf('&')>0){fragment1=word.substr(0,word.indexOf('&'));fragment2=word.substr(word.indexOf('&')+1,word.length);newWord=fragment1+'::amp::'+fragment2;words[i]=newWord;word=newWord;fragment1='';fragment2='';}
