...
class InboundEmail extends SugarBean
{
    ...
    */
     ...
     * Email constructor.
     *
     * @param null|ImapHandlerInterface $imapHandler
     * @param null|MailMimeParser $mailParser
     *
    ...
    /**
     * @param null|ImapHandlerInterface $imap
     *
     * @throws ImapHandlerException
     *
     ...
     * set.
     *
     * @param array|string $uids Sequence of UIDs, comma separated string or array
    ...
    public function getTeamSetIdForTeams($teamIds)
    {
        ...
        return $teamSet->addTeams($teamIds);
    }
    ...
    public function findOptimumSettings(
        ...
        foreach ($serviceArr as $k => $serviceTest) {
            ...
            if (($errors = $this->getImap()->getLastError()) || ($alerts = $this->getImap()->getAlerts())) {
                ...
                if (in_array($errors, $acceptableWarnings, true)) { // false positive
     ...
     * found
     * @return    bool        false if NO DUPE IS FOUND
     */
    ...
    public function getGroupsWithSelectOptions($groups = [])
    {
        ...
        return get_select_options_with_id_separate_key($groups, $groups, $this->group_id);
    }
    ...
    public function buildBreadCrumbs($parts, $subtype, $breadcrumb = '0')
    {
        ...
        foreach ($parts as $k => $part) {
            ...
            if ($part->type == 1 && (strtoupper($part->subtype) == 'RELATED' || strtoupper($part->subtype) == 'ALTERNATIVE' || strtoupper($part->subtype) == 'MIXED')) {
                return $this->buildBreadCrumbs($part->parts, $subtype, $thisBc);
            }
            if (strtolower($part->subtype) == strtolower($subtype)) { // found the subtype we want, return the breadcrumb value
                return $thisBc;
            }
        }
    }
     ...
     * @param mixed $stackedBreadcrumbs
     *
    ...
    {
        return md5(print_r($header, true));
    }
    ...
    public function checkFilterDomain($email)
    {
        ...
        if (strpos($from, $filterDomain) !== false) {
     ...
     *
     * @return bool|string   Case ID or FALSE if not found
     */
     ...
     * @return    bool false if none returned
     */
...
eoq;
     ...
     * @param int $options Bitmask for options parameter to the imap_open function
     *
     * @return bool|resource  Connection resource on success, FALSE on failure
     */
     ...
     * @param null|string $mailbox Mailbox (if applied to protocol)
     */
