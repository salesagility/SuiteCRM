        ...
        foreach ($mailboxes as $mbox) {
            ...
            if ($connectToMailServer) {
                ...
                if (is_array($newMsgs)) {
                    ...
                    $isGroupFolderExists = (!empty($groupFolderId) ? $sugarFolder->retrieve($groupFolderId) : false);
                    $users = array();
...
function pollMonitoredInboxesAOP()
{
    ...
    while ($inboundEmailRow = $aopInboundEmail->db->fetchByAssoc($sqlQueryResult)) {
        ...
        foreach ($mailboxes as $mbox) {
            ...
            $connectToMailServer = ($aopInboundEmailX->connectMailserver() == 'true' ? true : false);
            if (!$connectToMailServer) {
            ...
            if (is_array($newMsgs)) {
                ...
                foreach ($newMsgs as $k => $msgNo) {
                    ...
                    $GLOBALS['log']->debug('***** On message [ ' . ($k+1) . ' of ' . $total . ' ] *****');
                } // foreach
                // update Inbound Account with last robin
            } // if
