...
class jjwg_Maps extends jjwg_Maps_sugar
{
        ...
        if (!empty($rev) && count($rev) > 0) {
            ...
            foreach ($rev as $name => $value) {
                ...
                if (substr($name, 0, 13) == 'address_type_') {
                    ...
                    if (in_array($value,
                        array('billing', 'shipping', 'primary', 'alt', 'flex_relate', 'custom', 'address'))) {
            ...
            $int_settings = array(
                'geocoding_limit',
                'google_geocoding_limit',
                'map_markers_limit',
                'map_default_distance',
                'export_addresses_limit',
                'map_clusterer_grid_size',
                'map_clusterer_max_zoom'
            );
    ...
    function saveConfiguration($data = array())
    {
        ...
        if (!empty($data) && count($data) > 0) {
            ...
            $int_settings = array(
                ...
                'map_clusterer_grid_size',
                'map_clusterer_max_zoom',
                'address_cache_get_enabled',
                'address_cache_save_enabled',
                'logic_hooks_enabled',
                'allow_approximate_location_type'
            );
            foreach ($int_settings as $setting) {
            ...
            // Float/Language Settings
            if (isset($data['map_default_unit_type']) && in_array(trim($data['map_default_unit_type']),
                    array('mi', 'km'))
            ) {
            ...
            if (isset($data['map_duplicate_marker_adjustment']) && is_numeric(trim($data['map_duplicate_marker_adjustment']))) {
                $admin->saveSetting($category, 'map_duplicate_marker_adjustment',
                    (float)trim($data['map_duplicate_marker_adjustment']));
            }
            ...
            if (isset($data['map_default_center_latitude']) && is_numeric(trim($data['map_default_center_latitude']))) {
                $admin->saveSetting($category, 'map_default_center_latitude',
                    (float)trim($data['map_default_center_latitude']));
            }
            ...
            if (isset($data['map_default_center_longitude']) && is_numeric(trim($data['map_default_center_longitude']))) {
                $admin->saveSetting($category, 'map_default_center_longitude',
                    (float)trim($data['map_default_center_longitude']));
            }

            // Set Geocoding API URL or Proxy URL
            if (substr($data['geocoding_api_url'], 0, 4) != 'http' && substr($data['geocoding_api_url'], 0,
                    2) != '//'
            ) {
    ...
    function defineMapsAddress($object_name, $display)
    {
        ...
        if ($object_name == 'Account') {
            ...
            $address = $this->defineMapsFormattedAddress($display,
                $this->settings['geocode_modules_to_address_type']['Accounts']);

        } elseif ($object_name == 'Contact') {

            $address = $this->defineMapsFormattedAddress($display,
                $this->settings['geocode_modules_to_address_type']['Contacts']);

        } elseif ($object_name == 'Lead') {

            $address = $this->defineMapsFormattedAddress($display,
                $this->settings['geocode_modules_to_address_type']['Leads']);

        } elseif ($object_name == 'Prospect') {

            $address = $this->defineMapsFormattedAddress($display,
                $this->settings['geocode_modules_to_address_type']['Prospects']);

        } elseif ($object_name == 'User') {

            $address = $this->defineMapsFormattedAddress($display,
                $this->settings['geocode_modules_to_address_type']['Users']);
            ...
            if (!empty($fields)) {
                $address = $this->defineMapsFormattedAddress($fields,
                    $this->settings['geocode_modules_to_address_type']['Opportunities']);
            }
            ...
            if (!empty($fields)) {
                $address = $this->defineMapsFormattedAddress($fields,
                    $this->settings['geocode_modules_to_address_type']['Cases']);
            }
            ...
            if (!empty($fields)) {
                $address = $this->defineMapsFormattedAddress($fields,
                    $this->settings['geocode_modules_to_address_type']['Project']);
            }
            ...
            if (in_array($parent_type, array_keys($this->settings['valid_geocode_modules']))
                ...
                $GLOBALS['log']->debug(__METHOD__ . ' Meeting $parent->fetched_row: ' . print_r($parent->fetched_row,
                        true));
