...
class jjwg_Maps extends jjwg_Maps_sugar
{
    ...
    public $settings = [
         ...
         * @var array
         */
        'geocode_modules_to_address_type' => [
            'Accounts' => 'billing',
            'Contacts' => 'primary',
            'Leads' => 'primary',
        ...
        if (!empty($rev) && count($rev) > 0) {
            foreach ($rev as $name => $value) {
                ...
                if (substr($name, 0, 13) == 'address_type_') {
                    ...
                    if (in_array($value, ['billing', 'shipping', 'primary', 'alt', 'flex_relate', 'custom', 'address'])) {
            ...
            // Integer Settings
            $int_settings = ['geocoding_limit', 'google_geocoding_limit',
            ...
            if (isset($rev['map_default_unit_type']) && in_array($rev['map_default_unit_type'], ['mi', 'km'])) {
    ...
    public function saveConfiguration($data = [])
    {
        ...
        if (!empty($data) && count($data) > 0) {
            ...
            // Float/Language Settings
            if (isset($data['map_default_unit_type']) && in_array(trim($data['map_default_unit_type']), ['mi', 'km'])) {
     ...
     */
    public function updateGeocodeInfoByAssocQuery($table_name, $display, $aInfo = [])
    {
        $GLOBALS['log']->info(__METHOD__ . ' START');
     ...
     * @param type $aInfo   array
     */
    public function updateGeocodeInfoByBeanQuery(&$bean, $aInfo = [])
    {
        $GLOBALS['log']->info(__METHOD__ . ' START');
    ...
    public function getGeocodeAddressesResult($table_name, $limit = 0, $id = '')
    {
        ...
        return $this->db->limitQuery($query, 0, $limit);
    }
     ...
     *
     * @param mixed $address
     * @param mixed $return_full_array
     * @param mixed $allow_approximate
     */
    ...
    public function defineMapsAddress($object_name, $display)
    {
        ...
        } elseif (in_array($object_name, ['aCase', 'Case'])) {
    ...
    public function defineMapsFormattedAddress($display, $type = 'billing')
    {
        ...
        if (!in_array($type, ['billing', 'shipping', 'primary', 'alt', 'custom', 'address'])) {
            $type = 'billing';
        }
        $GLOBALS['log']->debug(__METHOD__ . ' $type: ' . print_r($type, true));
        $address_fields = ['billing_address_street', 'billing_address_city', 'billing_address_state', 'billing_address_postalcode', 'billing_address_country'];
        $address_parts = [];
        switch ($type) {
            case 'billing':
                $address_fields = ['billing_address_street', 'billing_address_city', 'billing_address_state', 'billing_address_postalcode', 'billing_address_country'];
                ...
                break;
            case 'shipping':
                $address_fields = ['shipping_address_street', 'shipping_address_city', 'shipping_address_state', 'shipping_address_postalcode', 'shipping_address_country'];

                break;
            case 'primary':
                $address_fields = ['primary_address_street', 'primary_address_city', 'primary_address_state', 'primary_address_postalcode', 'primary_address_country'];

                break;
            case 'alt':
                $address_fields = ['alt_address_street', 'alt_address_city', 'alt_address_state', 'alt_address_postalcode', 'alt_address_country'];

                break;
            case 'address':
                $address_fields = ['address_street', 'address_city', 'address_state', 'address_postalcode', 'address_country'];
