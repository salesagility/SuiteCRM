...
class actionSendEmail extends actionBase
{
    ...
    {
        return ['modules/AOW_Actions/actions/actionSendEmail.js'];
    }
    ...
    public function run_action(SugarBean $bean, $params = [], $in_save = false)
    {
        ...
        if (isset($params['individual_email']) && $params['individual_email']) {
            foreach ($emails['to'] as $email_to) {
                $emailTemp = new EmailTemplate();
                $emailTemp->retrieve($params['email_template']);
                $template_override = isset($emails['template_override'][$email_to]) ? $emails['template_override'][$email_to] : [];
                $this->parse_template($bean, $emailTemp, $template_override);
                if (!$this->sendEmail([$email_to], $emailTemp->subject, $emailTemp->body_html, $emailTemp->body, $bean, $emails['cc'], $emails['bcc'], $attachments)) {
                    $ret = false;
    ...
    public function parse_template(SugarBean $bean, &$template, $object_override = [])
    {
        global $sugar_config;
            ...
            if ($bean_arr['type'] == 'relate') {
                ...
                if ($bean_arr['type'] == 'link') {
                    ...
                    if (isset($bean_arr['module']) && $bean_arr['module'] != '' && !isset($object_arr[$bean_arr['module']]) && $bean_arr['module'] != 'EmailAddress') {
                        $linkedBeans = $bean->get_linked_beans($bean_arr['name'], $bean_arr['module'], [], 0, 1);
                        if ($linkedBeans) {
    ...
    public function sendEmail($emailTo, $emailSubject, $emailBody, $altemailBody, SugarBean $relatedBean = null, $emailCc = [], $emailBcc = [], $attachments = [])
    {
        ...
        if (isset($params['email_target_type']) && !is_array($params['email_target_type'])) {
            ...
            switch ($params['email_target_type']) {
                case 'Email Address':
                    ...
                    break;
                case 'Specify User':
                    $params['email'] = [$params['email_user_id']];

                    break;
                case 'Related Field':
                    $params['email'] = [$params['email_target']];

                    break;
            }
            $params['email_target_type'] = [$params['email_target_type']];
            $params['email_to_type'] = ['to'];
        }
        //end backward compatible
        if (isset($params['email_target_type'])) {
            foreach ($params['email_target_type'] as $key => $field) {
                switch ($field) {
                    ...
                    case 'Specify User':
                        ...
                        if (trim($user_email) != '') {
                            ...
                            $emails['template_override'][$user_email] = ['Users' => $user->id];
                        }
                            ...
                            foreach ($linkedBeans as $linkedBean) {
                                if (!empty($linkedBean)) {
                                    ...
                                    if (trim($rel_email) != '') {
                                        ...
                                        $emails['template_override'][$rel_email] = [$linkedBean->module_dir => $linkedBean->id];
                                    }
                                }
                            }
                        }
