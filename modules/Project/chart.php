...
//Returns an array containing the years, months and weeks between two dates
	public function year_quarter($start_date, $end_date)
	{
		...
		{
			$count++;
      $aResult[$dt->format('Y')][ceil($dt->format('m')/3)][$count] = mb_substr($GLOBALS['app_list_strings']['dom_cal_month_short'][$dt->format('n')],0, 3);
		}
...
//Returns an array containing the years, months and weeks between two dates
	public function year_week($start_date, $end_date)
	{
		...
		{
			$count++;
      $aResult[$dt->format('Y')][$GLOBALS['app_list_strings']['dom_cal_month_short'][$dt->format('n')]][] = $dt->format('W');
    }
...
//Returns an array containing the years, months and days between two dates
  public function year_month($start_date, $end_date)
  {
    ...
    foreach ( $period as $dt )
    {        	
      $aResult[$dt->format('Y')][mb_substr($GLOBALS['app_list_strings']['dom_cal_month_short'][$dt->format('n')],0, 3)][$dt->format('j')] = mb_substr($GLOBALS['app_list_strings']['dom_cal_day_short'][$dt->format('w')+1],0, 1);
    }
