        ...
        if (!file_exists($cache_file_name) || $refresh == true) {
            ...
            if (!empty($months)) {
                foreach ($months as $month) {
                    ...
                    $fileContents .= '          <dataRow title="' . $month . '" endLabel="' . currency_format_number($monthArr[$month]['row_total'], ['currency_symbol' => true]) . '">' . "\n";
                    arsort($salesStages);
                    foreach ($salesStages as $outcome => $outcome_translation) {
                        if (isset($monthArr[$month][$outcome])) {
                            $fileContents .= '               <bar id="' . $outcome . '" totalSize="' . array_sum($monthArr[$month][$outcome]['total']) . '" altText="' . $month . ': ' . format_number(array_sum($monthArr[$month][$outcome]['opp_count']), 0, 0) . ' ' . $current_module_strings['LBL_OPPS_WORTH'] . ' ' . currency_format_number(array_sum($monthArr[$month][$outcome]['total']), ['currency_symbol' => true]) . $current_module_strings['LBL_OPP_THOUSANDS'] . ' ' . $current_module_strings['LBL_OPPS_OUTCOME'] . ' ' . $outcome_translation . '" url="index.php?module=Opportunities&action=index&date_closed=' . $month . '&sales_stage=' . urlencode($outcome) . '&query=true&searchFormTab=advanced_search"/>' . "\n";
                        }
                    }
            ...
            $total = round($total, 2);
            $title = '<graphData title="' . $current_module_strings['LBL_TOTAL_PIPELINE'] . currency_format_number($total, ['currency_symbol' => true]) . $app_strings['LBL_THOUSANDS_SYMBOL'] . '">' . "\n";
