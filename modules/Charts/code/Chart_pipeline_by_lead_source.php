    ...
    public function gen_xml(
        ...
        if (!file_exists($cache_file_name) || $refresh == true) {
            ...
            foreach ($legends as $lead_source_key => $translation) {
                ...
                $fileContents .= '          <wedge title="' . $translation . '" kDelim="' . $kDelim . '" value="' . $leadSourceArr[$lead_source_key]['sum'] . '" color="' . $color . '" labelText="' . currency_format_number($leadSourceArr[$lead_source_key]['sum'], ['currency_symbol' => true]) . '" url="index.php?module=Opportunities&action=index&lead_source=' . urlencode($lead_source_key) . '&query=true&searchFormTab=advanced_search" altText="' . format_number($leadSourceArr[$lead_source_key]['opp_count'], 0, 0) . ' ' . $current_module_strings['LBL_OPPS_IN_LEAD_SOURCE'] . ' ' . $translation . '"/>' . "\n";
                if (isset($leadSourceArr[$lead_source_key])) {
            ...
            $total = round($total, 2);
            $title = $current_module_strings['LBL_TOTAL_PIPELINE'] . currency_format_number($total, ['currency_symbol' => true]) . $app_strings['LBL_THOUSANDS_SYMBOL'];
