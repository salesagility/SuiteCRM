...
<script src="include/javascript/jquery/jquery-ui-min.js"></script>
    <script src="custom/include/javascript/datetimepicker/jquery-ui-timepicker-addon.js"></script>
    <script src="custom/include/javascript/rating/rating.min.js"></script>
  <script>
...
function displayQuestion($survey, $question){
    ...
    <?php
    $options = array();
    foreach($question->get_linked_beans('surveyquestions_surveyquestionoptions','SurveyQuestionOptions','sort_order') as $option){
            $optionArr =  array();
    ...
    switch($question->type){
        ...
        case "Textbox":
            echo "<textarea class=\"form-control\" id='question".$question->id."' name='question[".$question->id."]'></textarea>";
            break;
        case "Checkbox":
            echo "<div class='checkbox'><label>";
            echo "<input id='question".$question->id."' name='question[".$question->id
                 ."]' type='checkbox'/>";
            echo "</label></div>";
            break;
        case "Radio":
            foreach($options as $option){
                ...
                echo "<label>";
                echo "<input  id='question".$question->id."' name='question[".$question->id."]' value='".$option['id']."' type='radio'/>";
                echo $option['name'];
                echo "</label>";
                echo "</div>";
            }
        ...
        case "Multiselect":
            ...
            $name = $question->type == 'Multiselect' ? "question[".$question->id."][]" : "question[".$question->id."]";
...
function displayTextField($question){
    echo "<input class=\"form-control\" id='question".$question->id."' name='question[".$question->id."]' type='text'/>";
}
function displayScaleField($question){
    ...
    echo "</tr><tr>";
    for($x = 1; $x <= $scaleMax; $x++){
        echo "<td><input id='question".$question->id."' name='question[".$question->id."]' value='".$x ."' type='radio'/></td>";
    }
    echo "</tr></table>";
}
function displayRatingField($question){
    ...
    for($x = 1; $x <= $ratingMax; $x++){
        echo "<input class='rating' id='question".$question->id."' name='question[".$question->id."]' value='" .$x ."' type='radio'/>";
    }
    echo "</div>";
}
function displayMatrixField($survey,$question,$options){
    ...
    foreach($options as $option){
        ...
        foreach($matrixOptions as $x => $matrixOption){
            echo "<td style='width:25%'><input  id='question".$question->id."' name='question[".$question->id."]["
                 .$option['id']
                 ."]' 
value='".$x."' type='radio'/></td>";
        }
        echo "</tr>";
    }
    echo "</table>";
...
function displayDateTimeField($question){
    ...
    echo "<input class=\"form-control datetimefield\" id='question".$question->id."' name='question[".$question->id
         ."]' type='text'/>";
...
function displayDateField($question){
    echo "<div class=\"input-group\">";
    echo "<input class=\"form-control datefield\" id='question".$question->id."' name='question[".$question->id
         ."]' type='text'/>";
