...
class stic_Import_ValidationViewStep3 extends stic_Import_ValidationView
{
    ...
    public function display()
    {
        ...
        for ($field_count = 0; $field_count < $ret_field_count; $field_count++) {
            ...
            foreach ($fields as $fieldname => $properties) {
                ...
                if (isset($properties['type'])
                        && isset($mod_strings['LBL_STIC_IMPORT_VALIDATION_FIELDDEF_' . strtoupper($properties['type'])])) {
                    $fieldtype = ' [' . $mod_strings['LBL_STIC_IMPORT_VALIDATION_FIELDDEF_' . strtoupper($properties['type'])] . '] ';
                }
            ...
            if (!empty($defaultField)) {
                ...
                $defaultFieldHTML = stic_getControl(
        ...
        // STIC-Code MHP - Add the values for the input of type select where to choose the column where to generate the ID
        $columnsName = $rows[0];
        array_unshift($columnsName, $mod_strings['LBL_CREATE_COLUMN']);
        $this->ss->assign("COLUMNSNAME", $columnsName);
        // END STIC-Code
        ...
        $content = $this->ss->fetch($this->getCustomFilePathIfExists('modules/stic_Import_Validation/tpls/step3.tpl'));
        $this->ss->assign("CONTENT", $content);
        $this->ss->display($this->getCustomFilePathIfExists('modules/stic_Import_Validation/tpls/wizardWrapper.tpl'));
    }
     ...
     * It first checks to see if an equivalent custom source map exists in custom/modules/stic_Import_Validations/maps directory
     ...
     *
     * If no such custom file is found, the method then checks the modules/stic_Import_Validations/maps directory for a source
     * mapping file.
     *
     * Lastly, if a source mapping file is still not located, it checks in
     * custom/modules/stic_Import_Validation/maps/ImportMapOther.php file exists, it uses the ImportMapOther class.
     *
     * @see display()
    ...
    protected function getMappingClassName($source)
    {
        ...
        if (file_exists("custom/modules/stic_Import_Validation/maps/{$customName}.php")) {
            require_once("custom/modules/stic_Import_Validation/maps/{$customName}.php");
            return $customName;
        } else {
            if (file_exists("custom/modules/stic_Import_Validation/maps/{$name}.php")) {
                require_once("custom/modules/stic_Import_Validation/maps/{$name}.php");
            } else {
                if (file_exists("modules/stic_Import_Validation/maps/{$name}.php")) {
                    require_once("modules/stic_Import_Validation/maps/{$name}.php");
                } else {
                    if (file_exists('custom/modules/stic_Import_Validation/maps/ImportMapOther.php')) {
...
if( document.getElementById('gonext') )
{
    document.getElementById('gonext').onclick = function(){
        ...
        if( ImportView.validateMappings() )
        {
            ...
            // STIC-Code MHP - Add a notice of missing required fields
            alert("{$mod_strings['LBL_REQUIRED_FIELDS']}");
            return false;
        }
    }
}
    ...
    for ( i = 0; i < column0.childNodes.length; i++ ) {
        if ( column0.childNodes[i].name == 'colnum_0' ) {
            ...
            column0.childNodes[i].onchange = function(){
                ...
                YAHOO.util.Connect.asyncRequest('GET', 'index.php?module=stic_Import_Validation&action=stic_getControl&import_module='+module+'&field_name='+fieldname,
                    {
                        success: function(o)
                        {
