...
require_once('modules/stic_Import_Validation/ImportFieldSanitize.php');
...
class Importer
{
    ...
    protected function importValidationRow($row)
    {
        ...
        for ($fieldNum = 0; $fieldNum < $_REQUEST['columncount']; $fieldNum++) {
            ...
            $fieldTranslated = translate((isset($fieldDef['vname'])?$fieldDef['vname']:$fieldDef['name']), $focus->module_dir);
            //  STIC-Code MHP - Delete the last character if is ":"
            if (substr($fieldTranslated, -1) != ':') {
                $fieldTranslated .= ':';
            }
            //  END STIC-Code
            $defaultRowValue = '';
            ...
            if (!empty($_REQUEST[$field])) {
                ...
                if (empty($rowValue)) {
                    ...
                    // STIC-Code MHP - Save the default value set by user in the import source since _currentRow contains the values that are written to the error file
                    $row[$fieldNum] = $defaultRowValue;
                    $this->importSource->setCurrentRow($row);
                    // END STIC-Code MHP -
            ...
            if (array_key_exists($field, $focus->get_import_required_fields()) && empty($rowValue) && $rowValue!='0') {
                $this->importSource->writeRecordAndErrors($mod_strings['LBL_REQUIRED_VALUE'], $fieldTranslated, 'NULL');
                $do_save = false;
            }
            ...
            if ($focus->object_name == "Contact" && $field == 'sync_contact') {
                ...
                if (true == $rowValue || 'true' == strtolower($rowValue)) {
                    ...
                    if (!$returnValue) {
                        $this->importSource->writeRecordAndErrors($mod_strings['LBL_ERROR_SYNC_USERS'], $fieldTranslated, $bad_names);
                        $do_save = 0;
                    } else {
            ...
            if ($field == 'email1' || $field == 'email2') {
                ...
                if ($returnValue === false) {
                    $do_save=0;
                    $this->importSource->writeRecordAndErrors($mod_strings['LBL_ERROR_INVALID_EMAIL'], $fieldTranslated, $rowValue);
                } else {
            ...
            if (!empty($rowValue)) {
                ...
                // STIC-Code MHP - Validate STIC fields such as identification number or bank_account
                if (isset($fieldDef['customValidationImport'])) {

                    // Check if a custom function file exists
                    $path = "modules/{$_REQUEST['module']}/ImporterValidationFunctions.php";
                    ...
                    // Perform validation depending on the name of the field
                    switch ($fieldDef['name']) 
                    {
                        case 'stic_identification_number_c':
                            $returnValue = identificationNumberValidation($focus->module_dir, $rowValue, $row);
                            break;
            
                        case 'bank_account':
                            $returnValue = bankAccountValidation($rowValue);
                            break;
                    ...
                    if ($returnValue != $rowValue) {
                        $this->importSource->writeRecordAndErrors($mod_strings[$returnValue], $fieldTranslated, $rowValue);
                        $rowValue = false;
                    } 
                }
                // END STIC-Code
                if ($rowValue === false) {
                    ...
                    $do_save = false;
                    // STIC-Code MHP - Save the value in $focus since it will help us to detect duplicate values in the file
                    $focus->$field = $row[$fieldNum];                 
                    continue;
                }
            }
        ...
        if (isset($_REQUEST['enabled_dupes']) && $_REQUEST['enabled_dupes'] != "") {
            ...
            for ($i=0; $i < count($enabled_dupes); $i++) { 
                $name = explode('::',$enabled_dupes[$i])[1];
                $fieldNameTranslate = translate($focus->field_name_map[$name]["vname"], $focus->module_dir);
                if (substr($fieldNameTranslate, -1) == ':') {
                    $fieldNameTranslate = substr($fieldNameTranslate, 0, -1);
                }                
            ...
            if ($idc->isADuplicateRecord($enabled_dupes)) {
                ...
                // Being a duplicate record, we get the ID of the duplicate record in BBDD
                $id = $this->getIDForBBDD($enabled_dupes, $focus);
                // Get the first duplicate record
                $duplicateField = $idc->_dupedFields[0];
                $this->importSource->writeRecordAndErrors($mod_strings['LBL_ERROR_DUPLICATE_RECORD'], $msg, $focus->$duplicateField, $id);
                // END STIC-Code 
                return;
            }
        }
            ...
            if (isset($dbrow['id']) && $dbrow['id'] != -1) {
                ...
                if (isset($dbrow['deleted']) && $dbrow['deleted'] == 1) {
                    ...
                    if (! $this->isUpdateOnly) {
                        $this->importSource->writeRecordAndErrors($mod_strings['LBL_ID_EXISTS_ALREADY'], 'ID', $focus->id);
                        $this->_undoCreatedBeans(ImportFieldSanitize::$createdBeans);
                        return;
                    }
                    ...
                    if ($clonedBean === false) {
                        $this->importSource->writeRecordAndErrors($mod_strings['LBL_RECORD_CANNOT_BE_UPDATED'], 'ID', $focus->id);
                        $this->_undoCreatedBeans(ImportFieldSanitize::$createdBeans);
                        return;
                    }
                    $focus = $clonedBean;
        ...
        if ($do_save) {
            ...
            $this->importSource->writeRecordAndErrors();
        } else {
            $this->_undoCreatedBeans(ImportFieldSanitize::$createdBeans);
        }
    ...
    /**
     * STIC-Code MHP
     * Returns the ID if one is found. Otherwise, it returns an empty string.
     * @param array $enabled_dupes Array containing the filters used to find duplicates 
     * @param object $focus Record being validated
     * @return string 
     */
    protected function getIDForBBDD($enabled_dupes, $focus)
    {
        $emailFilter = false;
        ...
        for ($i=0; $i < count($enabled_dupes); $i++) { 
            $name = explode('::',$enabled_dupes[$i])[1];
            $fields[$i]['name'] = $name;
            $fields[$i]['value'] = $focus->$name;
            $fields[$i]['custom'] = strpos($name, '_c') ?  true : false;
            $emailFilter = (($name == 'email1') && (isset($focus->$name)) && !empty($focus->$name)) ? true : $emailFilter;
        }

        // Create the query with the main table
        $query = "SELECT m.id FROM {$focus->table_name} as m";

        // Check if the custom table exists, and if it does, do a JOIN with it
        global $db;
        $cstmTableExists = $db->getOne("SELECT count(*) FROM information_schema.TABLES where table_schema=database() and TABLE_NAME='{$focus->table_name}_cstm'");
        if ($cstmTableExists) {
            $query .= "
            JOIN {$focus->table_name}_cstm as c ON m.id = c.id_c";
        }

        // If the email filter has been added, we do the JOIN with the corresponding tables.
        if ($emailFilter) {
            $emailCap = strtoupper($focus->email1);
            $query .= "
        ...
        $id = $db->getOne($query);
        // Set empty string in case of getting a false
        if (!$id) $id = '';
    ...
    protected function sanitizeFieldValueByType($rowValue, $fieldDef, $defaultRowValue, $focus, $fieldTranslated)
    {
        ...
        switch ($fieldtype) {
            ...
            case 'decimal':     
                ...
                $floatNumber = str_replace(',', '.', $rowValue);

                if (!floatval($floatNumber)) {
                    $this->importSource->writeRecordAndErrors($mod_strings['LBL_ERROR_INVALID_'.strtoupper($fieldtype)], $fieldTranslated, $rowValue);
                    return false;
                }
                break;
            // END STIC-Code
    ...
    protected function saveImportBean($focus, $newRecord)
    {
        ...
        */
        ...
        // STIC-Code MHP - Do not execute the action to save the records
        // $focus->save(false);
