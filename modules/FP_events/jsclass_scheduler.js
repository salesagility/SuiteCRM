...
SugarWidgetListView.prototype.display = function() {
  ...
  for (var i = 0; i < GLOBAL_REGISTRY["result_list"].length; i++) {
    ...
    html +=
      '<input type="button" id="invitees_add_' +
      (i + 1) +
      '" class="button" onclick="this.disabled=true;SugarWidgetSchedulerAttendees.form_add_attendee(' +
      i +
      ');" value="' +
      GLOBAL_REGISTRY["meeting_strings"]["LBL_ADD_BUTTON"] +
      '"/ style="visibility: ' +
      hidden +
      '"/>';
    html += "</td>";
...
SugarWidgetSchedulerSearch.prototype.display = function() {
  ...
  html += "<tr>";
  html +=
    '<td scope="col" nowrap><label for="search_first_name">' +
    GLOBAL_REGISTRY["meeting_strings"]["LBL_FIRST_NAME"] +
    ':</label>&nbsp;&nbsp;<input  name="search_first_name" id="search_first_name" value="" type="text" size="10"></td>';
  html +=
    '<td scope="col" nowrap><label for="search_last_name">' +
    GLOBAL_REGISTRY["meeting_strings"]["LBL_LAST_NAME"] +
    ':</label>&nbsp;&nbsp;<input  name="search_last_name" id="search_last_name" value="" type="text" size="10"></td>';
  html +=
    '<td scope="col" nowrap><label for="search_email">' +
    GLOBAL_REGISTRY["meeting_strings"]["LBL_EMAIL"] +
    ':</label>&nbsp;&nbsp;<input name="search_email" id="search_email" type="text" value="" size="15"></td>';
  ...
  html +=
    '<button type="button" id="create_invitee_as_contact" onclick="SugarWidgetSchedulerSearch.showCreateForm(\'Contacts\');">' +
    GLOBAL_REGISTRY["meeting_strings"]["LBL_CREATE_CONTACT"] +
    "</button> ";
  ...
  html +=
    ...
    GLOBAL_REGISTRY["meeting_strings"]["LBL_LAST_NAME"] +
    ': <span class="required">*</span></td><td valign="top"><input name="last_name" type="text" size="19"></td>';
  html += "</tr>";
  html += "<tr>";
  ...
  html +=
    '<button type="button" id="cancel-create-invitee-btn" onclick="SugarWidgetSchedulerSearch.hideCreateForm();">' + GLOBAL_REGISTRY["meeting_strings"]["LBL_CANCEL_CREATE_INVITEE"] + "</button> ";
  html += "</form>";
  html += "</div>";
  html += "</div>";
  this.parentNode.innerHTML += html;
...
SugarWidgetScheduler.update_time = function() {
  var form_name;
  if (typeof document.EditView != "undefined") form_name = "EditView";
  else if (typeof document.CalendarEditView != "undefined") form_name = "CalendarEditView";
  else return;
  if (typeof document.forms[form_name].date_start == "undefined") return;
...
SugarWidgetSchedulerAttendees.prototype.init = function() {
  ...
  GLOBAL_REGISTRY.focus.fields.datetime_end = new Date(
    dtstart.getFullYear(),
    dtstart.getMonth(),
    dtstart.getDate(),
    dtstart.getHours() + parseInt(GLOBAL_REGISTRY.focus.fields.duration_hours),
    dtstart.getMinutes() + parseInt(GLOBAL_REGISTRY.focus.fields.duration_minutes),
    0
  );
  var has_start = false;
  var has_end = false;
...
SugarWidgetSchedulerAttendees.prototype.display = function() {
  ...
  html += "</tr>";
  html += "</table>";
  if (this.parentNode.childNodes.length < 1) this.parentNode.innerHTML += '<div class="schedulerDiv">' + html + "</div>";
  else this.parentNode.childNodes[0].innerHTML = html;
  var thetable = "schedulerTable";
  if (typeof GLOBAL_REGISTRY == "undefined") {
    return;
  }
  if (
    (typeof GLOBAL_REGISTRY.focus.users_arr == "undefined" || GLOBAL_REGISTRY.focus.users_arr.length == 0) &&
    document.forms[form_name].record.value == "" &&
    typeof GLOBAL_REGISTRY.FIRST_REMOVE == "undefined"
  ) {
...
SugarWidgetScheduleRow.prototype.display = function() {
  SUGAR.util.doWhen(
    "document.getElementById('" + this.thetableid + "') != null",
    function() {
      var tr;
      this.thetable = document.getElementById(this.thetableid);
      if (typeof this.element != "undefined") {
        if (this.element.parentNode != null) this.thetable.deleteRow(this.element.rowIndex);
      ...
      tr.appendChild(td);
      td.scope = "row";
      var img = '<img align="absmiddle" src="index.php?entryPoint=getImage&themeName=' + SUGAR.themes.theme_name + "&imageName=" + this.focus_bean.module + 's.gif"/>&nbsp;';
      td.innerHTML = img;
      td.innerHTML = td.innerHTML;
      if (this.focus_bean.fields.full_name) td.innerHTML += " " + this.focus_bean.fields.full_name;
      ...
      td.innerHTML =
        '<a title="' +
        GLOBAL_REGISTRY["meeting_strings"]["LBL_REMOVE"] +
        '" class="listViewTdToolsS1" style="text-decoration:none;" ' +
        "href=\"javascript:SugarWidgetScheduleRow.deleteRow('" +
        this.focus_bean.fields.id +
        "');\">&nbsp;" +
        '<img src="index.php?entryPoint=getImage&themeName=' +
        SUGAR.themes.theme_name +
        '&imageName=delete_inline.gif" ' +
        'align="absmiddle" alt="' +
        GLOBAL_REGISTRY["meeting_strings"]["LBL_REMOVE"] +
        '" border="0"> ' +
        GLOBAL_REGISTRY["meeting_strings"]["LBL_REMOVE"] +
