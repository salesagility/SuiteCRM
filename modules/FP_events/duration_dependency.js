...
function DurationDependency(start_field, end_field, duration_field, format) {
  ...
  if (format_parts.length == 3) this.time_format += "" + this.time_format[2];
...
DurationDependency.prototype.set_duration_handler = function() {
  if (this.duration_field == null) return;
...
DurationDependency.prototype.set_date = function(date, field) {
  ...
  if (this.has_meridiem) {
    var meridiem = "am";
    if (hour * 60 + minute >= 12 * 60) meridiem = "pm";
    if (hour == 0) hour = 12;
  ...
  var date = "";
  for (var i = 0; i < 3; i++) {
    if (i > 0) date += this.date_delimiter;
    if (i == this.year_pos) date += year;
    if (i == this.month_pos) date += month;
    if (i == this.day_pos) date += day;
  }
