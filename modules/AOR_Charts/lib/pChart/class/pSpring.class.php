    ...
    function drawSpring($Object, $Settings = "")
    {
        ...
        if ($DrawQuietZone) {
            foreach ($this->Data as $Key => $Settings) {
                ...
                $this->pChartObject->drawFilledCircle($X, $Y, $FreeZone,
        ...
        foreach ($this->Data as $Key => $Settings) {
            ...
            if ($Shape == NODE_SHAPE_CIRCLE) {
                ...
                $Offset = $Size / 2;
                $Size = $Size / 2;
                $this->pChartObject->drawFilledRectangle($X - $Offset, $Y - $Offset, $X + $Offset, $Y + $Offset,
                    $Color);
            }
            ...
            foreach ($this->Data as $Key => $Settings) {
                ...
                if (isset($Settings["Vectors"]) && $Settings["Type"] != NODE_TYPE_CENTRAL) {
                    foreach ($Settings["Vectors"] as $ID => $Vector) {
                        ...
                        $Color = $Type == "A" ? array("FillR" => 255, "FillG" => 0, "FillB" => 0) : array(
                            "FillR" => 0,
                            "FillG" => 255,
                            "FillB" => 0
                        );
