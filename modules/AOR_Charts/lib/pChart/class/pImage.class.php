...
class pImage extends pDraw
{
    ...
    function pImage($XSize, $YSize, $DataSet = null, $TransparentBackground = false)
    {
        ...
        if ($this->TransparentBackground) {
            ...
            imagefilledrectangle($this->Picture, 0, 0, $XSize, $YSize,
        ...
        if ($this->ImageMapStorageMode == IMAGE_MAP_STORAGE_SESSION) {
            ...
            if ($Handle) {
                while (($Buffer = fgets($Handle, 4096)) !== false) {
                    $Fields = preg_split("/" . IMAGE_MAP_DELIMITER . "/",
    ...
    function drawAreaMirror($X, $Y, $Width, $Height, $Format = "")
    {
        ...
        for ($i = 1; $i <= $Height; $i++) {
            if ($Y + ($i - 1) < $this->YSize && $Y - $i > 0) {
                imagecopymerge($Picture, $this->Picture, $X, $Y + ($i - 1), $X, $Y - $i, $Width, 1,
