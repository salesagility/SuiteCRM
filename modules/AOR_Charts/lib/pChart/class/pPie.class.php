...
class pPie
{
    ...
    function draw2DPie($X, $Y, $Format = "")
    {
        ...
        foreach ($Values as $Key => $Value) {
            ...
            if ($RecordImageMap && !$Shadow) {
                $this->pChartObject->addToImageMap("POLY", $this->arraySerialize($Plots),
                    $this->pChartObject->toHTMLColor($Palette[$ID]["R"], $Palette[$ID]["G"], $Palette[$ID]["B"]),
                    $Data["Series"][$Data["Abscissa"]]["Data"][$Key], $Value);
            }
    ...
    function draw3DPie($X, $Y, $Format = "")
    {
        ...
        foreach ($Slices as $SliceID => $Plots) {
            ...
            if ($Visible[$SliceID]["Start"] && isset($Plots[2])) /* Empty error handling */ {
                $this->pChartObject->drawLine($Plots[2], $Plots[3], $Plots[2], $Plots[3] - $SliceHeight,
        ...
        foreach ($Slices as $SliceID => $Plots) {
            ...
            if ($Visible[$SliceID]["End"]) {
                $this->pChartObject->drawLine($Plots[count($Plots) - 2], $Plots[count($Plots) - 1],
                    $Plots[count($Plots) - 2], $Plots[count($Plots) - 1] - $SliceHeight,
                    array("R" => $Settings["R"], "G" => $Settings["G"], "B" => $Settings["B"]));
        ...
        foreach ($Slices as $SliceID => $Plots) {
            ...
            if ($RecordImageMap && !$Shadow) {
                $this->pChartObject->addToImageMap("POLY", $this->arraySerialize($Top),
                    $this->pChartObject->toHTMLColor($Settings["R"], $Settings["G"], $Settings["B"]),
                    $Data["Series"][$Data["Abscissa"]]["Data"][count($Slices) - $SliceID - 1], $Values[$SliceID]);
            }
        }
    ...
    function drawPieLegend($X, $Y, $Format = "")
    {
        ...
        foreach ($Data["Series"][$Data["Abscissa"]]["Data"] as $Key => $Value) {
            $BoxArray = $this->pChartObject->getTextBox($vX + $BoxSize + 4, $vY + $BoxSize / 2, $FontName, $FontSize, 0,
                $Value);
        ...
        foreach ($Data["Series"][$Data["Abscissa"]]["Data"] as $Key => $Value) {
            ...
            $this->pChartObject->drawFilledRectangle($X + 1, $Y + 1, $X + $BoxSize + 1, $Y + $BoxSize + 1,
                array("R" => 0, "G" => 0, "B" => 0, "Alpha" => 20));
            $this->pChartObject->drawFilledRectangle($X, $Y, $X + $BoxSize, $Y + $BoxSize,
    ...
    function draw2DRing($X, $Y, $Format = "")
    {
        ...
        foreach ($Values as $Key => $Value) {
            ...
            $this->pChartObject->drawLine($Boundaries[0]["X1"], $Boundaries[0]["Y1"], $Boundaries[0]["X2"],
                $Boundaries[0]["Y2"], $BorderColor);
            $this->pChartObject->drawLine($Boundaries[1]["X1"], $Boundaries[1]["Y1"], $Boundaries[1]["X2"],
                $Boundaries[1]["Y2"], $BorderColor);
        ...
        if ($WriteValues && !$Shadow) {
            ...
            foreach ($Values as $Key => $Value) {
                ...
                $this->pChartObject->drawText($Xc, $Yc, $Display,
                    array("Align" => $Align, "R" => $ValueR, "G" => $ValueG, "B" => $ValueB));
                $Offset = $EndAngle;
            }
        }
    ...
    function draw3DRing($X, $Y, $Format = "")
    {
        ...
        foreach ($Slices as $SliceID => $Plots) {
            ...
            if ($StartAngle >= 270 || $StartAngle <= 90) {
                $this->pChartObject->drawLine($Plots["OutX1"], $Plots["OutY1"], $Plots["OutX1"],
                    $Plots["OutY1"] - $SliceHeight, $Settings);
            }
            if ($StartAngle >= 270 || $StartAngle <= 90) {
                $this->pChartObject->drawLine($Plots["OutX2"], $Plots["OutY2"], $Plots["OutX2"],
                    $Plots["OutY2"] - $SliceHeight, $Settings);
            }

            $this->pChartObject->drawLine($Plots["InX1"], $Plots["InY1"], $Plots["InX1"], $Plots["InY1"] - $SliceHeight,
                $Settings);
            $this->pChartObject->drawLine($Plots["InX2"], $Plots["InY2"], $Plots["InX2"], $Plots["InY2"] - $SliceHeight,
                $Settings);
        }
        ...
        foreach ($Slices as $SliceID => $Plots) {
            ...
            if ($RecordImageMap) {
                $this->pChartObject->addToImageMap("POLY", $this->arraySerialize($Plots["TopPoly"]),
                    $this->pChartObject->toHTMLColor($Settings["R"], $Settings["G"], $Settings["B"]),
            ...
            $this->pChartObject->drawLine($Plots["InX1"], $Plots["InY1"] - $SliceHeight, $Plots["OutX2"],
                $Plots["OutY2"] - $SliceHeight, $Settings);
            $this->pChartObject->drawLine($Plots["InX2"], $Plots["InY2"] - $SliceHeight, $Plots["OutX1"],
                $Plots["OutY1"] - $SliceHeight, $Settings);
        }
        ...
        if ($DrawLabels) {
            ...
            foreach ($Values as $Key => $Value) {
                ...
                if ($LabelStacked) {
                    $this->writePieLabel($Xc, $Yc - $SliceHeight, $Label, $Angle, $Settings, true, $X, $Y,
                        $OuterRadius);
                } else {
