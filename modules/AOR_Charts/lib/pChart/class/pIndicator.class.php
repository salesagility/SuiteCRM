    ...
    function draw($X, $Y, $Width, $Height, $Format = "")
    {
        ...
        foreach ($IndicatorSections as $Key => $Settings) {
            ...
            if ($CaptionPosition == INDICATOR_CAPTION_INSIDE) {
                $TxtPos = $this->pChartObject->getTextBox($X1, $Y + $Height + $TextPadding, $CaptionFontName,
                    $CaptionFontSize, 0, $Caption);
                $YOffset = ($TxtPos[0]["Y"] - $TxtPos[2]["Y"]) + $TextPadding;

                if ($CaptionLayout == INDICATOR_CAPTION_EXTENDED) {
                    $TxtPos = $this->pChartObject->getTextBox($X1, $Y + $Height + $TextPadding, $CaptionFontName,
                        $CaptionFontSize, 0, $SubCaption);
            ...
            if ($CaptionLayout == INDICATOR_CAPTION_DEFAULT) {
                ...
                $this->pChartObject->drawText($X1 + $XOffset, $Y + $Height - $YOffset + $TextPadding, $Caption,
                    $CaptionColor);
                $this->pChartObject->drawText($X1 + $XOffset,
                    $Y + $Height - $YOffset + $CaptionHeight + $TextPadding * 2, $SubCaption, $SubCaptionColor);
            }
                ...
                foreach ($IndicatorSections as $Key => $Settings) {
                    if ($Value >= $Settings["Start"] && $Value <= $Settings["End"]) {
                        ...
                        if ($ValueDisplay == INDICATOR_VALUE_BUBBLE) {
                            $TxtPos = $this->pChartObject->getTextBox($X1, $Y, $ValueFontName, $ValueFontSize, 0,
                                $Value . $Unit);
                            ...
                            $this->pChartObject->drawFilledCircle($X1, $Y, $Radius,
                            ...
                            $this->pChartObject->drawLabelBox(floor($X1), floor($Y) + 2,
                                "Value - " . $Settings["Caption"], $Caption);
                        }
                    }
                    $X1 = $X2 + $SectionsMargin;
                }
