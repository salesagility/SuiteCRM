...
class AbstractRelationship
{
        ...
        'from_studio',
        'readonly',
        // a readonly relationship cannot be Built by subclasses of AbstractRelationships
        'deleted',
        // a deleted relationship will not be built, and if it had been built previously the built relationship will be removed
        'relationship_only',
        // means that we won't build any UI components for this relationship - required while the Subpanel code is restricted to one subpanel only from any module, and probably useful afterwards also for developers to build relationships for new code - it's a feature!
        ...
        // optional
        'lhs_subpanel',
        // subpanel FROM the lhs_module to display on the rhs_module detail view
        'rhs_subpanel',
        // subpanel FROM the rhs_module to display on the lhs_module detail view
        ...
        'reverse'
    ); // enough information to rebuild this relationship
...
protected $definition;
    ...
    function getRelationshipMetaData($relationshipType, $checkExisting = true)
    {
        ...
        if (isset ($this->definition ['relationship_role_column_value'])) {
            $properties ['fields'] [] = array(
                'name' => $this->definition ['relationship_role_column'],
                'type' => 'varchar'
            );
        }
        ...
        switch ($relationshipType) {
            case MB_ONETOONE:
                ...
                $properties ['indices'] [] = array(
                    'name' => $indexBase . '_ida1',
                    'type' => 'index',
                    'fields' => array($rel_properties ['join_key_lhs'])
                );
                $properties ['indices'] [] = array(
                    'name' => $indexBase . '_idb2',
                    'type' => 'index',
                    'fields' => array($rel_properties ['join_key_rhs'])
                );
                break;
            case MB_ONETOMANY :
    ...
    protected function getSubpanelDefinition(
        $relationshipName,
        $sourceModule,
        $subpanelName,
        $titleKeyName = '',
    ...
    protected function getLinkFieldDefinition(
        $sourceModule,
        $relationshipName,
        $right_side = false,
        $vname = "",
