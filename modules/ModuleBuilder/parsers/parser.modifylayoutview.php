...
//          $GLOBALS['log']->debug($origdefs);
    ...
    function getAvailableFields()
    {
        ...
        $GLOBALS['log']->debug(get_class($this) . "->getAvailableFields(): _getModelFields returns: " . implode(",",
...
//        $GLOBALS['log']->debug("Original viewdefs = ".print_r($origViewDefs,true));
        ...
        foreach ($this->_fieldDefs as $field => $def) {
            if ((!empty($def['studio']) && $def['studio'] == 'visible')
                || (empty($def['studio']) && (empty($def ['source']) || $def ['source'] == 'db' || $def ['source'] == 'custom_fields') && $def ['type'] != 'id' && strcmp($field,
                        'deleted') != 0 && (empty($def ['dbType']) || $def ['dbType'] != 'id') && (empty($def ['dbtype']) || $def ['dbtype'] != 'id'))
            ) {
