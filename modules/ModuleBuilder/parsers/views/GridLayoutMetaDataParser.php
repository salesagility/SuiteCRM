...
class GridLayoutMetaDataParser extends AbstractMetaDataParser implements MetaDataParserInterface
{
    ...
    public function writeWorkingFile($populate = true)
    {
        ...
        $this->implementation->save([self::$variableMap[$this->_view] => $viewdefs]);
    }

    /**
     * Deploy the layout.
     *
     * @param bool $populate If true (default), then update the layout first with new layout information from the $_REQUEST array
     */
    public function handleSave($populate = true)
    {
        ...
        $viewdefs = $this->_viewdefs;
        $viewdefs['panels'] = $this->_convertToCanonicalForm($this->_viewdefs['panels'], $this->_fielddefs);
        $this->implementation->deploy([self::$variableMap[$this->_view] => $viewdefs]);
    }
                ...
                foreach ($row as $colID => $fieldName) {
                    if (isset($this->_fielddefs[$fieldName])) {
                        ...
                        if (isset($this->_originalViewDef[$fieldName]) && is_array($this->_originalViewDef[$fieldName])) {
                            ...
                            $viewDefinitions[$panelID][$rowID][$colID] = [
                                'name' => $fieldName,
                                'label' => $fieldName
                            ];
                        }
        ...
        foreach ($this->_fielddefs as $key => $def) {
            if (GridLayoutMetaDataParser::validField($def, $this->_view) || isset($this->_originalViewDef[$key])) {
                ...
                if (isset($this->_originalViewDef[$key]) && is_array($this->_originalViewDef[$key]) && isset($this->_originalViewDef[$key]['label'])) {
                    $availableFields[$key] = ['name' => $key, 'label' => $this->_originalViewDef[$key]['label']];
                } else {
                    $availableFields[$key] = [
                        'name' => $key,
     ...
     * @param bool|string $panelID Identifier of the panel to add the field to; empty or false if we should use the first panel
     *
     * @return bool
     */
     ...
     *
     * @return bool True if the field was removed; false otherwise
     */
     ...
     *
     * @return int First unique panel ID suffix
     */
     ...
     *
     * @param array|string $source it can be a string which contain just a name of field
     *                                  or an array with field attributes including name
     * @param array $fieldDefinitions stores field defs from request
     *
     * @return array|string definition of new row item
     */
    ...
    protected function _populateFromRequest(&$fieldDefinitions)
    {
        ...
        $this->_viewdefs['panels'] = []; // because the new field properties should replace the old fields, not be merged
