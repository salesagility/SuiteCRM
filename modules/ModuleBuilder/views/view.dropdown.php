...
class ViewDropdown extends SugarView
{
    function display()
    {
        ...
        $smarty->assign('deleteImage', SugarThemeRegistry::current()->getImage('delete_inline', '', null, null, '.gif',
            $mod_strings['LBL_MB_DELETE']));
        $smarty->assign('editImage',
            SugarThemeRegistry::current()->getImage('edit_inline', '', null, null, '.gif', $mod_strings['LBL_EDIT'],
                true, '.gif'));
...
//		$my_list_strings = $GLOBALS['app_list_strings'];
        ...
        if (!empty($_REQUEST['view_package']) && $_REQUEST['view_package'] != 'studio') {
            ...
            $my_list_strings = array_merge($my_list_strings,
                $module->mblanguage->appListStrings[$selected_lang . '.lang.php']);
        ...
        if (!empty($_REQUEST['dropdown_name']) && !$new) {
            ...
            $smarty->assign('dropdown_name',
                (!empty($vardef['options']) ? $vardef['options'] : $_REQUEST['dropdown_name']));
