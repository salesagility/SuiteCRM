...
class MBModule
{
    ...
    public $always_implement = ['assignable' => 'Assignable', 'security_groups' => 'Security Groups', 'acl' => 'Access Controls', 'studio' => 'Studio Support', 'audit' => 'Audit Table'];
    public $iTemplate = [
    ...
    public function setModStrings($language, $mod_strings = [])
    {
    ...
    public function setAppListStrings($language, $app_list_strings = [])
    {
        // set $language = 'en_us' as default
        if (!$language) {
            $language = 'en_us';
    ...
    public function copyMetaRecursive($from, $to, $overwrite = false)
    {
        ...
        if (is_dir($from)) {
            $findArray = ['<module_name>', '<_module_name>', '<MODULE_NAME>', '<object_name>', '<_object_name>', '<OBJECT_NAME>'];
            $replaceArray = [$this->key_name, strtolower($this->key_name), strtoupper($this->key_name),
    ...
    {
        $name = $this->key_name;
        $installDefs['copy'][] = ['from' => '<basepath>/SugarModules/modules/' . $name, 'to' => 'modules/' . $name];
        $installDefs['beans'][] = ['module' => $name, 'class' => $name, 'path' => 'modules/' . $name . '/' . $name . '.php', 'tab' => $this->config['has_tab']];
    ...
    public function getNodes()
    {
        ...
        foreach ($psubs as $sub) {
            ...
            $lSubs[] = ['name' => $subLabel, 'type' => 'list', 'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=ListView&view_module=' . $this->name . '&view_package=' . $this->package . '&subpanel=' . $sub . '&subpanelLabel=' . $subLabel . '&local=1'];
        }

        $searchSubs = [];
        $searchSubs[] = ['name' => translate('LBL_BASIC_SEARCH'), 'type' => 'list', 'action' => "module=ModuleBuilder&MB=true&action=editLayout&view=basic_search&view_module={$this->name}&view_package={$this->package}"];
        $searchSubs[] = ['name' => translate('LBL_ADVANCED_SEARCH'), 'type' => 'list', 'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=advanced_search&view_module=' . $this->name . '&view_package=' . $this->package];
        $dashlets = [];
        $dashlets[] = ['name' => translate('LBL_DASHLETLISTVIEW'), 'type' => 'dashlet', 'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=dashlet&view_module=' . $this->name . '&view_package=' . $this->package];
        $dashlets[] = ['name' => translate('LBL_DASHLETSEARCHVIEW'), 'type' => 'dashletsearch', 'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=dashletsearch&view_module=' . $this->name . '&view_package=' . $this->package];

        $popups = [];
        $popups[] = ['name' => translate('LBL_POPUPLISTVIEW'), 'type' => 'popuplistview', 'action' => 'module=ModuleBuilder&action=editLayout&view=popuplist&view_module=' . $this->name . '&view_package=' . $this->package];
        $popups[] = ['name' => translate('LBL_POPUPSEARCH'), 'type' => 'popupsearch', 'action' => 'module=ModuleBuilder&action=editLayout&view=popupsearch&view_module=' . $this->name . '&view_package=' . $this->package];

        $layouts = [
            ['name' => translate('LBL_EDITVIEW'), 'type' => 'edit', 'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=' . MB_EDITVIEW . '&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_DETAILVIEW'), 'type' => 'detail', 'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=' . MB_DETAILVIEW . '&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_LISTVIEW'), 'type' => 'list', 'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=' . MB_LISTVIEW . '&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_QUICKCREATE'), 'type' => MB_QUICKCREATE,  'action' => 'module=ModuleBuilder&MB=true&action=editLayout&view=' . MB_QUICKCREATE . '&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_DASHLET'), 'type' => 'Folder', 'children' => $dashlets, 'action' => 'module=ModuleBuilder&MB=true&action=wizard&view=dashlet&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_POPUP'), 'type' => 'Folder', 'children' => $popups, 'action' => 'module=ModuleBuilder&MB=true&action=wizard&view=popup&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_SEARCH_FORMS'), 'action' => 'module=ModuleBuilder&MB=true&action=wizard&view=search&view_module=' . $this->name . '&view_package=' . $this->package, 'type' => 'folder', 'children' => $searchSubs]
        ];

        $children = [
            ['name' => translate('LBL_FIELDS'), 'action' => 'module=ModuleBuilder&action=modulefields&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_LABELS'), 'action' => 'module=ModuleBuilder&action=modulelabels&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_RELATIONSHIPS'), 'action' => 'module=ModuleBuilder&action=relationships&view_module=' . $this->name . '&view_package=' . $this->package],
            ['name' => translate('LBL_LAYOUTS'), 'type' => 'Folder', 'action' => "module=ModuleBuilder&action=wizard&view_module={$this->name}&view_package={$this->package}&MB=1", 'children' => $layouts],
        ];

        if (count($lSubs) > 0) {
            $children[] = ['name' => translate('LBL_AVAILABLE_SUBPANELS'), 'type' => 'folder', 'children' => $lSubs];
        }

        $nodes = ['name' => $this->name, 'children' => $children, 'action' => 'module=ModuleBuilder&action=module&view_module=' . $this->name . '&view_package=' . $this->package];
    ...
    public function removeFieldFromLayouts($fieldName)
    {
        ...
        $views = ['editview', 'detailview', 'listview', 'basic_search', 'advanced_search', 'dashlet', 'popuplist'];
