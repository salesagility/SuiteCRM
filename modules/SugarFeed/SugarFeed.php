...
class SugarFeed extends Basic
{
    ...
    function get_list_view_data()
    {
        ...
        if (ACLController::moduleSupportsACL($data['RELATED_MODULE'])) {
            ...
            if (
                !ACLController::checkAccess($data['RELATED_MODULE'], 'view',
                    $data['CREATED_BY'] == $GLOBALS['current_user']->id, 'module', $in_group)
                && !ACLController::checkAccess($data['RELATED_MODULE'], 'list',
                    $data['CREATED_BY'] == $GLOBALS['current_user']->id, 'module', $in_group)
    ...
    function fetchReplies($data)
    {
        ...
        $replies = $seedBean->get_list('date_entered',
            "related_module = 'SugarFeed' AND related_id = '" . $data['ID'] . "'");
