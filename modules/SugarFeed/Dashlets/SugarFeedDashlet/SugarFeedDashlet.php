...
class SugarFeedDashlet extends DashletGeneric
{
    ...
    function process($lvsParams = array(), $id = null)
    {
        ...
        if (!empty($this->displayTpl)) {
            ...
            if (is_admin($GLOBALS['current_user'])) {
                ...
                if (count($owner_modules) > 0
                    ...
                    $module_limiter = " ((sugarfeed.related_module IN ('" . implode("','", $regular_modules) . "') "
                        . ") ";
                    if (count($owner_modules) > 0) {
                        $module_limiter .= "OR (sugarfeed.related_module IN('" . implode("','",
                                $owner_modules) . "') AND sugarfeed.assigned_user_id = '" . $current_user->id . "' "
                            . ") ";
                    }
                    $module_limiter .= ")";
                }
            ...
            foreach ($this->lvs->data['data'] as $row => $data) {
                ...
                $this->lvs->data['data'][$row]['NAME'] = str_replace("{this.CREATED_BY}",
                    get_assigned_user_name($this->lvs->data['data'][$row]['CREATED_BY']), $data['NAME']);
                ...
                if (count($modStringMatches) == 3 && $modStringMatches[1] == 'SugarFeed' && !empty($data['RELATED_MODULE'])) {
                    ...
                    if (strpos($modString,
                            '{0}') === false || !isset($GLOBALS['app_list_strings']['moduleListSingular'][$data['RELATED_MODULE']])
                    ) {
                        continue;
                    }
                    ...
                    $this->lvs->data['data'][$row]['NAME'] = preg_replace('/' . $modStringMatches[0] . '/',
                        strtolower($modString), $this->lvs->data['data'][$row]['NAME']);
                }
    ...
    function display()
    {
        ...
        foreach ($alt_modules[1] as $alt) {
            ...
            $altString = 'alt="' . translate('LBL_VIEW',
                    'SugarFeed') . ' ' . $GLOBALS['app_list_strings']['moduleListSingular'][$alt] . '"';
    ...
    function getPostForm()
    {
        ...
        $moreimg = SugarThemeRegistry::current()->getImage('advanced_search',
            'onclick="toggleDisplay(\'more_' . $this->id . '\'); toggleDisplay(\'more_img_' . $this->id . '\'); toggleDisplay(\'less_img_' . $this->id . '\');"',
            null, null, '.gif', translate('LBL_SHOW_MORE_OPTIONS', 'SugarFeed'));
        $lessimg = SugarThemeRegistry::current()->getImage('basic_search',
            'onclick="toggleDisplay(\'more_' . $this->id . '\'); toggleDisplay(\'more_img_' . $this->id . '\'); toggleDisplay(\'less_img_' . $this->id . '\');"',
            null, null, '.gif', translate('LBL_HIDE_OPTIONS', 'SugarFeed'));
        $ss = new Sugar_Smarty();
