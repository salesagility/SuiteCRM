...
SUGAR.Studio.prototype.publish = function() {
  ...
  var cObj = YAHOO.util.Connect.asyncRequest(
    "GET",
    "index.php?to_pdf=1&module=Studio&action=Publish",
    { success: SUGAR.Studio.prototype.published, failure: SUGAR.Studio.prototype.published },
    null
  );
};
...
var studiopopup = (function() {
  return {
    display: function() {
      ...
      var cObj = YAHOO.util.Connect.asyncRequest(
        "GET",
        "index.php?to_pdf=1&module=Studio&action=wizard&wizard=EditCustomFieldsWizard&option=CreateCustomFields&popup=true",
        { success: studiopopup.render, failure: studiopopup.render },
        null
      );
    },
    destroy: function() {
...
function submitCustomFieldForm(isPopup) {
  ...
  if (isPopup) {
    var callback = { success: popupSave, failure: popupSave, argument: "" };
...
var populateCustomFieldCallback = { success: populateCustomField, failure: populateCustomField, argument: 1 };
var COBJ = false;
function changeTypeData(type) {
  ...
  COBJ = YAHOO.util.Connect.asyncRequest(
    "GET",
    "index.php?module=Studio&popup=true&action=index&&ajax=editcustomfield&to_pdf=true&type=" + type,
    populateCustomFieldCallback,
    null
  );
}
