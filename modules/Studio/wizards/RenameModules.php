...
class RenameModules
{
    ...
    private function changeModuleDashletStrings($moduleName, $replacementLabels, $dashletsFiles)
    {
        ...
        foreach ($dashletsFiles as $dashletName => $dashletData) {
            if (isset($dashletData['module']) && $dashletData['module'] == $moduleName && file_exists($dashletData['meta'])) {
                ...
                if ($modStringKey !== false) {
                    $replacedString = str_replace(html_entity_decode_utf8($replacementLabels['prev_plural'],
                        ENT_QUOTES), $replacementLabels['plural'], $dashletTitle);
                    if ($replacedString == $dashletTitle) {
                        $replacedString = str_replace(html_entity_decode_utf8($replacementLabels['prev_singular'],
                            ENT_QUOTES), $replacementLabels['singular'], $replacedString);
                    }
        ...
        if (count($replacementStrings) > 0) {
            $GLOBALS['log']->debug("Writing out labels for dashlet changes for module $moduleName, labels: " . var_export($replacementStrings,
                    true));
    ...
    private function renameModuleSubpanel($moduleName, $beanName)
    {
        ...
        foreach ($subpanelDefs as $subpanelName => $subpanelMetaData) {
            ...
            foreach ($this->changedModules as $changedModuleName => $renameFields) {
                if (!(isset($subpanelMetaData['type']) && $subpanelMetaData['type'] == 'collection') //Dont bother with collections
                    ...
                    $replacedString = str_replace(html_entity_decode_utf8($renameFields['prev_plural'], ENT_QUOTES),
                        $renameFields['plural'], $oldStringValue);
                    if ($replacedString == $oldStringValue) {
                        // continue to replace singular only if nothing been replaced yet
                        $replacedString = str_replace(html_entity_decode_utf8($renameFields['prev_singular'],
                            ENT_QUOTES), $renameFields['singular'], $replacedString);
                    }
        ...
        //Now we can write out the replaced language strings for each module
        if (count($replacementStrings) > 0) {
            $GLOBALS['log']->debug("Writing out labels for subpanel changes for module $moduleName, labels: " . var_export($replacementStrings,
                    true));
            ParserLabel::addLabels($this->selectedLanguage, $replacementStrings, $moduleName);
    ...
    private function renameModuleRelatedLinks($moduleName, $moduleClass)
    {
        ...
        foreach ($arrayToRename as $link => $linkEntry) {
            ...
            if (strlen($renameFields['prev_plural']) > strlen($renameFields['prev_singular']) && strpos($oldStringValue,
                    $renameFields['prev_plural']) !== false
            ) {
                $key = 'plural';
            } else {
                $key = 'singular';
            }
            $replacedString = str_replace(html_entity_decode_utf8($renameFields['prev_' . $key], ENT_QUOTES),
                $renameFields[$key], $oldStringValue);
            $replacementStrings[$replaceKey] = $replacedString;
        }

        //Now we can write out the replaced language strings for each module
        if (count($replacementStrings) > 0) {
            $GLOBALS['log']->debug("Writing out labels for link changes for module $moduleName, labels: " . var_export($replacementStrings,
                    true));
            ParserLabel::addLabels($this->selectedLanguage, $replacementStrings, $moduleName);
            $this->renamedModules[$moduleName] = true;
        }
    ...
    private function changeModuleModStrings($moduleName, $replacementLabels)
    {
        ...
        foreach ($labelKeysToReplace as $entry) {
            ...
            if (isset($currentModuleStrings[$formattedLanguageKey])) {
                ...
                $replacedLabels[$formattedLanguageKey] = $this->replaceSingleLabel($oldStringValue, $replacementLabels,
                    $entry);
                if (isset($entry['case']) && $entry['case'] == 'both') {
                    $replacedLabels[$formattedLanguageKey] = $this->replaceSingleLabel($replacedLabels[$formattedLanguageKey],
                        $replacementLabels, $entry, 'strtolower');
                }
            }
        }
    ...
    {
        $unformatedKey = str_replace('###MODULE_SINGULAR###', strtoupper($replacementStrings['key_singular']),
            $unformatedKey);
        ...
        foreach (array_keys($this->changedModules) as $moduleName) {
            foreach ($typeDisplayList as $typeDisplay) {
                if (isset($app_list_strings[$typeDisplay]) && isset($app_list_strings[$typeDisplay][$moduleName])) {
                    ...
                    DropDownHelper::saveDropDown($this->createModuleListSingularPackage($newParams,
                        array($moduleName => $this->changedModules[$moduleName])));
                }
            }
        }
    ...
    protected function display()
    {
        ...
        $title = getClassicModuleTitle($mod_strings['LBL_MODULE_NAME'], array(
            "<a href='index.php?module=Administration&action=index'>" . $mod_strings['LBL_MODULE_NAME'] . "</a>",
            $mod_strings['LBL_RENAME_TABS']
        ), false);
        ...
        $buttons[] = array(
            'text' => $mod_strings['LBL_BTN_SAVE'],
            'actionScript' => "onclick='if(check_form(\"editdropdown\")){document.editdropdown.submit();}'"
        );
