...
<tr><td class='mbLBL'>{sugar_translate module="DynamicFields" label="COLUMN_TITLE_INLINE_EDIT_TEXT"}:</td><td><input type="checkbox" name="inline_edit" value="1" {if !empty($vardef.inline_edit) }CHECKED{/if} {if $hideLevel > 5}disabled{/if}/>{if $hideLevel > 5}<input type="hidden" name="audited" value="{$vardef.inline_edit}">{/if}</td></tr>
