...
class Account extends Company
{
    ...
    public function clear_account_case_relationship($account_id = '', $case_id = '')
    {
        ...
        $query = "UPDATE cases SET account_name = '', account_id = '' WHERE account_id "
            . "= '$account_id' AND deleted = 0 " . $where;
    ...
    public function fill_in_additional_detail_fields()
    {
        ...
        //rrs bug: 28184 - instead of removing this code altogether
        // just adding this check to ensure that if the parent_name
        //is empty then go ahead and fill it.
        if (empty($this->parent_name) && !empty($this->id)) {
            $query = "SELECT a1.name from accounts a1, accounts a2 where a1.id = a2.parent_id "
                . "and a2.id = '$this->id' and a1.deleted=0";
    ...
    public function create_export_query($order_by, $where, $relate_link_join = '')
    {
        ...
        $query .= ' LEFT JOIN  email_addr_bean_rel on accounts.id = email_addr_bean_rel.bean_id and ' .
            'email_addr_bean_rel.bean_module=\'Accounts\' and email_addr_bean_rel.deleted=0 and ' .
