...
class AccountFormBase
{
    ...
    public function buildTableForm($rows, $mod = 'Accounts')
    {
        ...
        if ($action != 'ShowDuplicates') {
            $form = "<form action='index.php' method='post' id='dupAccounts'  " .
                "name='dupAccounts'><input type='hidden' name='selectedAccount' value=''>";
            $form .= '<table width="100%"><tr><td>' . $mod_strings['MSG_DUPLICATE']
                . '</td></tr><tr><td height="20"></td></tr></table>';
            unset($_POST['selectedAccount']);
        } else {
            $form = '<table width="100%"><tr><td>' . $mod_strings['MSG_SHOW_DUPLICATES']
                . '</td></tr><tr><td height="20"></td></tr></table>';
        }
        ...
        $form .= "<table width='100%' cellpadding='0' cellspacing='0' class='list view' border='0'>" .
            "<tr class='pagination'><td colspan='$cols'>" .
            "<table width='100%' cellspacing='0' cellpadding='0' border='0'><tr><td>";
        // handle buttons
        if ($action == 'ShowDuplicates') {
            $form .= "<input type='hidden' name='selectedAccount' id='selectedAccount' value=''>" .
                "<input title='${app_strings['LBL_SAVE_BUTTON_TITLE']}' " .
                "accessKey='${app_strings['LBL_SAVE_BUTTON_KEY']}' class='button' " .
                "onclick=\"this.form.action.value='Save';\" type='submit' name='button' value='  " .
                "${app_strings['LBL_SAVE_BUTTON_LABEL']}  '>\n";

            if (!empty($_REQUEST['return_module'])
                && !empty($_REQUEST['return_action'])
                && !empty($_REQUEST['return_id'])) {
                $form .= "<input title='${app_strings['LBL_CANCEL_BUTTON_TITLE']}' "
                    . "accessKey='${app_strings['LBL_CANCEL_BUTTON_KEY']}' class='button' "
                    . "onclick=\"this.form.module.value='" . $_REQUEST['return_module'] . "';this.form.action.value='"
                    . $_REQUEST['return_action'] . "';this.form.record.value='"
                    . $_REQUEST['return_id'] . "'\" type='submit' name='button' value='  "
                    . "${app_strings['LBL_CANCEL_BUTTON_LABEL']}  '>";
            } elseif (!empty($_POST['return_module']) && !empty($_POST['return_action'])) {
                $form .= "<input title='${app_strings['LBL_CANCEL_BUTTON_TITLE']}' " .
                    "accessKey='${app_strings['LBL_CANCEL_BUTTON_KEY']}' class='button' " .
                    "onclick=\"this.form.module.value='" . $_POST['return_module'] . "';this.form.action.value='"
                    . $_POST['return_action'] . "';\" type='submit' name='button' value='  " .
                    "${app_strings['LBL_CANCEL_BUTTON_LABEL']}  '>";
            } else {
                $form .= "<input title='${app_strings['LBL_CANCEL_BUTTON_TITLE']}' " .
                    "accessKey='${app_strings['LBL_CANCEL_BUTTON_KEY']}' class='button' " .
                    "onclick=\"this.form.action.value='ListView';\" type='submit' type='submit' " .
                    "name='button' value='  ${app_strings['LBL_CANCEL_BUTTON_LABEL']}  '>";
            }
        } else {
            $form .= "<input type='submit' class='button' name='ContinueAccount' " .
                "value='${mod_strings['LNK_NEW_ACCOUNT']}'>\n";
        }
        ...
        foreach ($rows as $row) {
            ...
            if ($action != 'ShowDuplicates') {
                $form .= "<td width='1%' nowrap><a href='javascript:void(0)' " .
                    "onclick='document.dupAccounts.selectedAccount.value=\"${row['id']}\"; " .
                    "document.dupAccounts.submit(); '>[${app_strings['LBL_SELECT_BUTTON_LABEL']}]</a>" .
                    "&nbsp;&nbsp;</td>\n";
            }
            foreach ($row as $key => $value) {
                if ($key != 'id') {
                    if (isset($_POST['popup']) && $_POST['popup'] == true) {
                        $form .= "<td scope='row'><a  href='javascript:void(0)' " .
                            "onclick=\"window.opener.location='index.php?module=Accounts&action=DetailView&record=" .
                            "${row['id']}'\">$value</a></td>\n";
                    } else
                        $form .= "<td><a target='_blank' href='index.php?module=Accounts&action=DetailView&record=" .
                            "${row['id']}'>$value</a></td>\n";
        ...
        $form .= "<tr class='pagination'><td colspan='$cols'><table width='100%' cellspacing='0' cellpadding='0' " .
            "border='0'><tr><td>";
        ...
        if ($action == 'ShowDuplicates') {
            ...
            if (!empty($_REQUEST['return_module'])
                && !empty($_REQUEST['return_action'])
                && !empty($_REQUEST['return_id'])) {
                $form .= "<input title='${app_strings['LBL_CANCEL_BUTTON_TITLE']}' class='button' " .
                    "onclick=\"this.form.module.value='" . $_REQUEST['return_module'] . "';this.form.action.value='"
                    . $_REQUEST['return_action'] . "';this.form.record.value='" . $_REQUEST['return_id']
                ...
                $form .= "<input title='${app_strings['LBL_CANCEL_BUTTON_TITLE']}'  class='button' " .
                    "onclick=\"this.form.action.value='ListView';\" type='submit' name='button' value='  " .
                    "${app_strings['LBL_CANCEL_BUTTON_LABEL']}  '>";
            }
        } else {
            $form .= "<input type='submit' class='button' name='ContinueAccount' " .
                "value='${mod_strings['LNK_NEW_ACCOUNT']}'></form>\n";
        }
    ...
    public function getForm($prefix, $mod = '', $form = '')
    {
        ...
        $the_form .= <<<EOQ
		<form name="${prefix}AccountSave" onSubmit="return check_form('${prefix}AccountSave');"
...
EOQ;
        ...
        $the_form .= <<<EOQ
		<p><input title="$lbl_save_button_title" accessKey="$lbl_save_button_key" class="button"
		    type="submit" name="button" value="  $lbl_save_button_label  " ></p>
		</form>
...
EOQ;
        $form .= "$lbl_account_name&nbsp;<span class='required'>$lbl_required_symbol</span><br>" .
    ...
    public function getWideFormBody($prefix, $mod = '', $formname = '', $contact = '')
    {
		...
		<input type='hidden' name='${prefix}billing_address_city' value='{$contact->primary_address_city}'>
		<input type='hidden' name='${prefix}billing_address_state'   value='{$contact->primary_address_state}'>
		<input type='hidden' name='${prefix}billing_address_postalcode'   value='{$contact->primary_address_postalcode}'>
		<input type='hidden' name='${prefix}billing_address_country'  value='{$contact->primary_address_country}'>
		<input type='hidden' name='${prefix}shipping_address_street' value='{$contact->alt_address_street}'>
		<input type='hidden' name='${prefix}shipping_address_city' value='{$contact->alt_address_city}'>
		<input type='hidden' name='${prefix}shipping_address_state'   value='{$contact->alt_address_state}'>
		<input type='hidden' name='${prefix}shipping_address_postalcode'   value='{$contact->alt_address_postalcode}'>
		<input type='hidden' name='${prefix}shipping_address_country'  value='{$contact->alt_address_country}'>
