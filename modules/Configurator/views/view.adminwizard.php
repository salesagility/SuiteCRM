...
class ViewAdminwizard extends SugarView
{
    ...
    public function display()
    {
        ...
        $this->ss->assign('CSS',
            ...
            SugarThemeRegistry::current()->getCSSURL('wizard.css') . '" />');
        ...
        $this->ss->assign('notify_allow_default_outbound_on',
            (!empty($focus->settings['notify_allow_default_outbound']) &&
                $focus->settings['notify_allow_default_outbound'] == 2) ? 'CHECKED' : '');
        ...
        if (isset($installation_scenarios)) {
            ...
            for ($i = 0; $i < count($installation_scenarios); $i++) {
                $installation_scenarios[$i]['moduleOverview'] = '( ' . implode(', ',
                        $installation_scenarios[$i]['modules']) . ')';
            }
