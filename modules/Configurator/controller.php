...
class ConfiguratorController extends SugarController
{
    ...
    function action_addFont()
    {
        ...
        if (empty($_FILES['pdf_metric_file']['name'])) {
            $this->errors[] = translate('ERR_MISSING_REQUIRED_FIELDS') . ' ' . translate('LBL_PDF_METRIC_FILE',
                    'Configurator'
                );
            $this->view = 'addFontView';
        ...
        if (empty($_FILES['pdf_font_file']['name'])) {
            $this->errors[] = translate('ERR_MISSING_REQUIRED_FIELDS') . ' ' . translate('LBL_PDF_FONT_FILE',
                    'Configurator'
                );
            $this->view = 'addFontView';
        ...
        if (($path_info_metric['extension'] !== 'afm' && $path_info_metric['extension'] !== 'ufm') ||
            ($path_info['extension'] !== 'ttf' && $path_info['extension'] !== 'otf' &&
                $path_info['extension'] !== 'pfb')
        ) {
    ...
    function action_saveadminwizard()
    {
        ...
        if ($current_user->isAdmin()) {
            ...
            $fileContents = "<?php \n" . '$GLOBALS["tabStructure"] =' . var_export($GLOBALS['tabStructure'],
                    true
                ) . ';';
            ...
            foreach ($_POST['modules'] as $moduleName => $enabled) {
                ...
                foreach ($subPanel->layout_defs['subpanel_setup'] as $subPanelDef) {
                    ...
                    foreach ($subPanelDef['collection_list'] as $v) {
                        if (!empty($v['get_subpanel_data']) &&
                            $v['get_subpanel_data'] === 'function:get_emails_by_assign_or_link'
                        ) {
                            $isValid = true;
                            break 2;
                        }
