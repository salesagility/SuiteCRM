    ...
    class Tracker extends SugarBean
    {
        ...
        function get_recently_viewed($user_id, $modules = '')
        {
            ...
            if (empty($_SESSION['breadCrumbs'])) {
                ...
                $GLOBALS['log']->info(string_format($GLOBALS['app_strings']['LBL_BREADCRUMBSTACK_CREATED'],
                    array($user_id)));
            } else {
                ...
                if (!empty($modules)) {
                    ...
                    $module_query = is_array($modules) ? ' AND module_name IN (\'' . implode("','",
                            $modules) . '\')' : ' AND module_name = \'' . $modules . '\'';
                } else {
