...
class SugarMerge
{
    ...
    function __construct($new_path = '', $original_path = '', $custom_path = 'custom')
    {
        ...
        $this->original_path = empty($original_path) || preg_match('/[\/]$/',
            $original_path) ? $original_path : $original_path . '/';
        $this->custom_path = empty($custom_path) || preg_match('/[\/]$/',
    ...
    function mergeFile($module, $file, $save = true, $logHistory = true)
    {
        ...
        if (isset($this->mergeMapping[$file]) && file_exists("{$path}{$file}") && file_exists("{$custom_path}{$file}") && file_exists("{$new_path}{$file}")) {
            ...
            return $this->mergeMapping[$file]->merge($module, "{$path}{$file}", "{$new_path}{$file}",
                "{$custom_path}{$file}", $save);
        }
