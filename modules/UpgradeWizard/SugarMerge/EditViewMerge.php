...
class EditViewMerge
{
     ...
     *
     * @return bool - success or failure of the save
     */
     ...
     * @param bool $save - boolean on if it should save the results to the custom file or not
     *
     * @return bool - if the merged file was saved if false is passed in for the save parameter it always returns true
     */
    ...
    protected function mergeFields()
    {
        foreach ($this->customFields as $field => $data) {
            ...
            if (isset($this->newFields[$field], $this->originalFields[$field])) {
            ...
            unset($this->originalFields[$field], $this->customFields[$field], $this->newFields[$field]);
        }

        // These are fields that were removed by the customer
        foreach ($this->originalFields as $field => $data) {
            unset($this->originalFields[$field], $this->newFields[$field]);
        }
            ...
            foreach ($panel as $row_id => $rows) {
                foreach ($rows as $col_id => $col) {
                    ...
                    if (is_string($field_name)) {
                        ...
                        $fields[$field_name] = ['data' => $col, 'loc' => ['panel' => (string) ($panel_id), 'row' => (string) ($row_id), 'col' => (string) ($col_id)]];
                    }
                }
            }
        }
