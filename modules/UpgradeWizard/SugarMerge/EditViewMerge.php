...
class EditViewMerge
{
                ...
                foreach ($rows as $col_id => $col) {
                    ...
                    if (is_string($field_name)) {
                        ...
                        $fields[$field_name] = array(
                            'data' => $col,
                            'loc' => array('panel' => "{$panel_id}", 'row' => "{$row_id}", 'col' => "{$col_id}")
                        );
                    }
                }
            }
        ...
        foreach ($this->customFields as $field => $data) {
            ...
            if (isset($this->newFields[$field]) && isset($this->originalFields[$field])) {
                ...
                if ($do_merge) {
                    ...
                    $this->mergedFields[$field] = array(
                        'data' => $this->mergeField($this->originalFields[$field]['data'],
                            $this->newFields[$field]['data'], $this->customFields[$field]['data']),
                        'loc' => $loc
                    );
                }
                ...
                if (!isset($this->newFields[$field])) {
                    ...
                    $this->mergedFields[$field] = array(
                        'data' => $this->mergeField('', $this->newFields[$field]['data'],
    ...
    protected function mergeTemplateMeta()
    {
        ...
        if (isset($this->customData[$this->module][$this->viewDefs][$this->templateMetaName]) && strcmp(strtolower($this->module),
                'calls') != 0 && strcmp(strtolower($this->module), 'meetings') != 0
        ) {
    ...
    {
        return write_array_to_file("viewdefs['$this->module']['$this->viewDefs']",
            $this->newData[$this->module][$this->viewDefs], $to);
    }
}
