...
class SearchMerge extends ListViewMerge
{
    ...
    protected function loadData($module, $original_file, $new_file, $custom_file)
    {
        ...
        $this->originalData = [$module => [$this->viewDefs => $this->originalData[$module]]];
        $this->customData = [$module => [$this->viewDefs => $this->customData[$module]]];
        $this->newData = [$module => [$this->viewDefs => $this->newData[$module]]];
    }
    ...
    protected function getFields(&$panels, $multiple = true)
    {
        ...
        foreach ($panels as $panel_id => $panel) {
            foreach ($panel as $col_id => $col) {
                ...
                $fields[$field_name . $panel_id] = ['data' => $col, 'loc' => ['row' => $col_id, 'panel' => $panel_id]];
            }
        }
