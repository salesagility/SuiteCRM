    ...
    protected function getFields(&$panels, $multiple = true)
    {
        ...
        foreach ($panels as $panel_id => $panel) {
            ...
            foreach ($panel as $col_id => $col) {
                $field_name = $col_id;
                $fields[$field_name . $panel_id] = array(
                    'data' => $col,
                    'loc' => array('row' => $col_id, 'panel' => $col_id)
                );
            }
    ...
    protected function mergeFields()
    {
        ...
        foreach ($this->customFields as $field => $data) {
            ...
            if (!empty($GLOBALS['dictionary'][$objectName]['fields'][$fieldName])) {
                $data['data'] = array_merge(ListLayoutMetaDataParser::createViewDefsByFieldDefs($GLOBALS['dictionary'][$objectName]['fields'][$fieldName]),
                    $data['data']);
            }
            ...
            if (isset($this->newFields[$field]) && isset($this->originalFields[$field])) {
                ...
                $this->mergedFields[$field] = array(
                    'data' => $this->mergeField($this->originalFields[$field]['data'], $this->newFields[$field]['data'],
