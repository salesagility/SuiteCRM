...
class MyMeetingsDashlet extends DashletGeneric
{
    ...
    function process($lvsParams = array(), $id = null)
    {
        ...
        // grab meeting status
        if (!empty($keys)) {
            $query = "SELECT meeting_id, accept_status FROM meetings_users WHERE deleted = 0 AND user_id = '" . $current_user->id . "' AND meeting_id IN ('" . implode("','",
                    $keys) . "')";
                ...
                if ($this->lvs->data['data'][$rowNum]['ACCEPT_STATUS'] == '') {
                    ...
                    $this->lvs->data['data'][$rowNum]['SET_ACCEPT_LINKS'] = "<div id=\"accept" . $this->id . "\" class=\"acceptMeeting\"><a title=\"" .
                        ...
                        SugarThemeRegistry::current()->getImage("tentative_inline", "border='0'", null, null, '.gif',
                            $app_list_strings['dom_meeting_accept_options']['tentative']) .
