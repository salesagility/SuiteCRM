...
SugarWidgetSchedulerSearch.prototype.display=function(){var create_invitees=document.createElement("div");create_invitees.setAttribute('id','create-invitees');create_invitees.setAttribute('style','margin-bottom: 10px;');var h3=document.createElement("h3");h3.setAttribute('id','create-invitees-title');h3.innerHTML=GLOBAL_REGISTRY['meeting_strings']['LBL_ADD_INVITEE'];create_invitees.appendChild(h3);var create_invitees_buttons=document.createElement("div");create_invitees_buttons.setAttribute('id','create-invitees-buttons');var invitableModules=['Contacts','Leads','Users'];invitableModules.forEach(function(moduleName){var button=document.createElement("button");button.setAttribute('class','create_invitee_button');button.setAttribute('type','button');button.setAttribute('onclick',"SugarWidgetSchedulerAttendees.openSelectorPopup('"+moduleName+"')");button.innerHTML=SUGAR.language.translate('app','LBL_'+moduleName.toUpperCase());create_invitees_buttons.appendChild(button);});create_invitees.appendChild(create_invitees_buttons);this.parentNode.appendChild(create_invitees);this.list_view=new SugarWidgetListView();};SugarWidgetSchedulerSearch.showCreateForm=function(module){document.getElementById('create-invitee-edit').style.display='';document.getElementById('create-invitees-buttons').style.display='none';document.getElementById('list_div_win').style.display='none';document.forms['createInviteeForm'].elements['inviteeModule'].value=module;document.getElementById('empty-search-message').style.display='none';if(typeof document.createInviteeForm.first_name!='undefined'&&typeof document.schedulerwidget.search_first_name!='undefined')
...
$.each(arguments,function(index,value){SUGAR.util.evalScript(value[0]);var container=result.body;containers.push(container);});containers.sort(SugarWidgetScheduler.sortByStartdate);containers.sort(SugarWidgetScheduler.sortByType);$dialog.html(containers);});}
...
SugarWidgetSchedulerAttendees.form_add_attendees_from_json_data=function(data){data.list.forEach(function(element){if(typeof(GLOBAL_REGISTRY.focus.users_arr_hash[element.fields.id])=='undefined'){GLOBAL_REGISTRY.focus.users_arr.push(element);}})
GLOBAL_REGISTRY.scheduler_attendees_obj.display();}
SugarWidgetSchedulerAttendees.form_add_attendees_from_selection_list=function(data){var conditions=[];var groupBy="or";if(typeof data.name_to_value_array!=='undefined'){conditions.push({"name":"id","op":"like_custom","value":data.name_to_value_array['subpanel_id']});var groupBy="and";}else if(typeof data.selection_list!=='undefined'){for(var property in data.selection_list){if(data.selection_list.hasOwnProperty(property)){conditions.push({"name":"id","op":"like_custom","value":data.selection_list[property]});}}}
var query={"modules":["Users","Contacts","Leads"],"group":groupBy,"field_list":['id','full_name','email1','phone_work'],"conditions":conditions};var requestId=global_rpcClient.call_method('query',query);global_request_registry[requestId]=[SugarWidgetSchedulerAttendees,'form_add_attendees_from_json_data'];}
SugarWidgetSchedulerAttendees.openSelectorPopup=function(module){var url="index.php%3Fmodule%3DAccounts%26action%3DSubPanelViewer%26subpanel%3Dcontacts%26record%3Db1f53412-87c1-38df-02c1-5a4cbf2d75bb%26sugar_body_only%3D1";var data={"call_back_function":"SugarWidgetSchedulerAttendees.form_add_attendees_from_selection_list","form_name":"DetailView","field_to_name_array":{"id":"subpanel_id"},"passthru_data":{"child_field":"contacts","return_url":url,"link_field_name":"contacts","module_name":"contacts","refresh_page":0}};open_popup(module,600,400,"",true,true,data,"MultiSelect",true);}
