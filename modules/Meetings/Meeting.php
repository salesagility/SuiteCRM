...
//		$this->fill_in_additional_detail_fields();
    ...
    function get_list_view_data()
    {
        ...
        if ($this->status == "Planned") {
            ...
            if ($this->ACLAccess('edit')) {
                $meeting_fields['SET_COMPLETE'] = $setCompleteUrl . SugarThemeRegistry::current()->getImage("close_inline",
                        " border='0'", null, null, '.gif', translate('LBL_CLOSEINLINE')) . "</a>";
            } else {
    ...
    function fill_in_additional_detail_fields()
    {
        ...
        global $timedate;
        $this->time_meridiem = $timedate->AMPMMenu('', $this->time_start,
            'onchange="SugarWidgetScheduler.update_time();"');
        $hours_arr = array();
        $num_of_hours = 13;
        $start_at = 1;
    ...
    function set_notification_body($xtpl, &$meeting)
    {
        ...
        $xtpl->assign("MEETING_STATUS",
            (isset($meeting->status) ? $app_list_strings['meeting_status_dom'][$meeting->status] : ""));
