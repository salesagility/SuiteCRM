 ...
 */
require_once('modules/UT_OrderSupplier/UT_OrderSupplier_sugar.php');
#[\AllowDynamicProperties]
class UT_OrderSupplier extends UT_OrderSupplier_sugar
{
    ...
    public function save($check_notify = false)
    {
        ...
        if (empty($this->id) || $this->new_with_id
            ...
            if ($sugar_config['dbconfig']['db_type'] == 'mssql') {
                $this->number = $this->db->getOne("SELECT MAX(CAST(number as INT))+1 FROM ut_ordersupplier");
            } else {
                $this->number = $this->db->getOne("SELECT MAX(CAST(number as UNSIGNED))+1 FROM ut_ordersupplier");
            }
