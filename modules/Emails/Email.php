...
class Email extends Basic
{
    ...
    /**
     * @var string prefix to use when importing inlinge images in emails
     */
    public $imagePrefix;
    ...
    protected static $validFieldNames = [
        'email_address',
        'to',
        'from',
        'from_name',
     ...
     * @var int
     */
    protected $lastSaveAndStoreInSentError;

    /**
     * @var NonGmailSentFolderHandler
     */
    protected $nonGmailSentFolderHandler;

    /**
     * @var Email
     */
    protected $tempEmailAtSend;
    ...
    public function decodeDuringSend($htmlData)
    {
        ...
        return str_replace('sugarGreaterThan', '&gt;', $htmlData);
    }
    ...
    public function email2Send($request)
    {
        ...
        if ($request['composeType'] == 'reply' || $request['composeType'] == 'replyCase') {
            if (isset($request['ieId'], $request['mbox'])) {
    ...
    public function quoteHtmlEmail($text)
    {
        ...
        return "<div style='border-left:1px solid #00c; padding:5px; margin-left:10px;'>{$text}</div>";
    }
    ...
    public function hasSignatureInBody($sig)
    {
        ...
        if (!empty($plainSig) && false !== strpos($plain, $plainSig)) {
            return true;
        }
...
eoq;
    ...
    public function populateBeanFromRequest($bean, $request)
    {
        ...
        if (isset($_REQUEST['refer_action']) && !empty($_REQUEST['refer_action'])) {
            $referActions = ['Forward', 'ReplyTo', 'ReplyToAll'];
