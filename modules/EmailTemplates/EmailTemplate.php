 ...
 *******************************************************************************
 * @property bool disable_row_level_security
 */
...
class EmailTemplate extends SugarBean
{
    ...
    public function get_list_view_data()
    {
        ...
        $fields['DATE_MODIFIED'] = isset($fields['DATE_MODIFIED']) && !empty($fields['DATE_MODIFIED']) ? substr($fields['DATE_MODIFIED'],
            0, 10) : false;
...
//function all string that match the pattern {.} , also catches the list of found strings.
    ...
    /**
     * @param array $templateTextArray
     * @param string $urlTemplate
     * @param array $trackedUrls
     * @param string $removemeUrlTemplate
     * @return mixed
     */
    public function parse_tracker_urls($templateTextArray, $urlTemplate, $trackedUrls, $removemeUrlTemplate)
    {
        global $sugar_config;
        ...
        $return_array = $templateTextArray;
        if (count($trackedUrls) > 0) {
            //parse the template and find all the dynamic strings that need replacement.
            foreach ($templateTextArray as $key => $template_text) {
                if (!empty($template_text)) {
                    ...
                    if ($count > 0) {
                        for ($i = ($count - 1); $i >= 0; $i--) {
                            ...
                            if (!empty($trackedUrls[$url_key_name])) {
                                if ($trackedUrls[$url_key_name]['is_optout'] == 1) {
                                    $tracker_url = $removemeUrlTemplate;
                                } else {
                                    $tracker_url = sprintf($urlTemplate, $trackedUrls[$url_key_name]['id']);
                                }
                            }
                            if (!empty($tracker_url) && !empty($template_text) && !empty($matches[0][$i][0]) && !empty($trackedUrls[$matches[0][$i][0]])) {
                                $template_text = substr_replace($template_text, $tracker_url, $matches[0][$i][1],
                                    strlen($matches[0][$i][0]));
                                $template_text = str_replace($sugar_config['site_url'] . '/' . $sugar_config['site_url'],
                                    $sugar_config['site_url'], $template_text);
                            }
                        }
                    }
                }
    ...
    private function _preg_match_tracker_url($text)
    {
        ...
        for ($i = 0, $iMax = strlen($text); $i < $iMax; $i++) {
            if ($text[$i] == '{') {
                ...
                $switch = false;
                $result[] = array(substr($text, $ind, $i - $ind + 1), $ind);
            }
        }
    ...
    public function parse_email_template($template_text_array, $focus_name, $focus, &$macro_nv)
    {
        ...
        foreach ($template_text_array as $key => $template_text) {
                ...
                if ($count != 0) {
                    for ($i = ($count - 1); $i >= 0; $i--) {
                        if (!isset($matches[0][$i][2])) {
                            ...
                            $matches[0][$i][2] = substr($matches[0][$i][0], $pattern_prefix_length);
            ...
            for ($i = ($count - 1); $i >= 0; $i--) {
                ...
                //check dom
                if (isset($matches[0][$i][3]) && isset($app_list_strings[$matches[0][$i][3]][$value])) {
        ...
        foreach ($user->field_defs as $field_def) {
            ...
            if ($field_def['type'] == 'enum') {
                ...
                if (isset($user->$fieldName)) {
                    ...
                    $repl_arr['contact_user_' . $fieldName] = $this->_convertToType($field_def['type'],
                        $user->$fieldName);
                } else {
                    $repl_arr['contact_user_' . $fieldName] = '';
                }
            ...
            if (!empty($acct->id)) {
                foreach ($acct->field_defs as $field_def) {
                    ...
                    if ($field_def['type'] == 'enum') {
                        ...
                        // bug 47647 - allow for fields to translate before adding to template
                        $translated = $this->_convertToType($field_def['type'], $acct->$fieldName);
                        $repl_arr = EmailTemplate::add_replacement($repl_arr, $field_def, array(
                            'account_' . $fieldName => $translated,
                            'contact_account_' . $fieldName => $translated,
                        ));
            ...
            foreach ($contact->field_defs as $field_def) {
                ...
                if ($field_def['type'] == 'enum') {
                    ...
                    if (isset($contact->$fieldName)) {
                        // bug 47647 - allow for fields to translate before adding to template
                        $translated = $this->_convertToType($field_def['type'], $contact->$fieldName);
                        $repl_arr = EmailTemplate::add_replacement($repl_arr, $field_def, array(
                            'contact_' . $fieldName => $translated,
                            'contact_account_' . $fieldName => $translated,
                        ));
        ...
        foreach ($focus->field_defs as $field_def) {
            ...
            if (isset($focus->$fieldName)) {
                ...
                if ($field_def['type'] == 'enum' && isset($field_def['options'])) {
                    ...
                    $repl_arr = EmailTemplate::add_replacement($repl_arr, $field_def, array(
                        strtolower($beanList[$bean_name]) . '_' . $fieldName => $this->_convertToType($field_def['type'],
                            $focus->$fieldName),
                    ));
                }
            } else {
    ...
    public function cleanBean()
    {
        ...
        parent::cleanBean();
        $this->body_html = str_replace(array_values($this->storedVariables), array_keys($this->storedVariables),
            $this->body_html);
    }
    ...
    public function addDomainToRelativeImagesSrc()
    {
        ...
        $ret = $this->body_html = preg_replace('/(&lt;img src=&quot;)(public\/[^.]*.(jpg|jpeg|png|gif|bmp))(&quot;)/',
            '$1' . $domain . '$2$4', $this->body_html);
    ...
    private function repairMozaikClears()
    {
        ...
        $this->body_html = str_replace('&lt;div class=&quot;mozaik-clear&quot;&gt;&nbsp;&lt;br&gt;&lt;/div&gt;',
            '&lt;div class=&quot;mozaik-clear&quot;&gt;&lt;/div&gt;', $this->body_html);
    }
