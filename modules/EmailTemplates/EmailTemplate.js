...
function createTemplateManagerDialog(parent) {
  $("#templateManagerDialog").dialog({ width: "50%", position: { my: "left top", at: "left bottom", of: parent } });
}
...
function EmailTemplateController(action) {
  ...
  var save = function(update) {
    ...
    $.post(
      "index.php?entryPoint=emailTemplateData&func=wizardUpdate&rand=" + Math.random(),
      {
        func: func,
  ...
  var create = function() {
    ...
    $("#template_messages").html(SUGAR.language.translate("Campaigns", "LBL_TEMPLATE_SAVING"));
    $.post(
      "index.php?entryPoint=emailTemplateData&rand=" + Math.random(),
      {
        func: func,
        emailTemplateId: emailTemplateCopyId ? emailTemplateCopyId : $("#template_id").val(),
        body_html: SuiteEditor.getValue(),
...
function EmailTrackerController(action, campaignId) {
  ...
  var create = function() {
    ...
    window.parent.$(".ui-dialog-content:visible").dialog("close");
    $.post(
      "index.php?entryPoint=campaignTrackerSave",
      {
      ...
      function(resp) {
        resp = JSON.parse(resp);
        if (resp.data.id) {
          $('select[name="tracker_url"]').append(
            '<option value="{' + trackerName + '}" data-id="' + resp.data.id + '" data-url="' + trackerURL + '">' + trackerName + " : " + trackerURL + "</option>"
          );
  ...
  var save = function() {
    ...
    $.post(
      ...
      {
        module: "CampaignTrackers",
        record: $('select[name="tracker_url"] option:selected').attr("data-id"),
        action: "Save",
        campaign_id: _campaignId,
        tracker_name: trackerName,
        tracker_url: trackerURL,
