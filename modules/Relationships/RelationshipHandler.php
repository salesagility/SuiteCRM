    ...
    */
    ...
    function build_related_list($type = "base")
    {
        ...
        if ($type == "base") {
            ...
            if (empty($target_list)) {
                $target_list = search_filter_rel_info($this->base_bean, $this->rel1_bean->module_dir,
                    $this->base_vardef_field);
        ...
        if ($type == "rel1") {
            ...
            //Possibility exists that this is a new relationship, so capture via relationship fields
            if (empty($target_list)) {
                $target_list = search_filter_rel_info($this->rel1_bean, $this->rel2_bean->module_dir,
                    $this->rel1_vardef_field);
                //end if the target list is empty
            }
        }
    ...
    function get_relationship_information(& $target_bean, $get_upstream_rel_field_name = false)
    {
        ...
        if ($rel_array != null) {
            ...
            if (($id_name = $this->traverse_rel_meta($current_module_name, $target_bean,
                    $rel_array['relationship_name'])) != null
            ) {
...
///////////////////////////END functions to find relationships //////////////////////
    ...
    function build_rel2_info()
    {
        ...
        $this->rel2_bean = $this->trace_relationship_module($this->base_module, $this->base_vardef_field,
            $this->rel1_vardef_field);
