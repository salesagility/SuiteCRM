...
/*            foreach($graphs as $g)
        ...
        $stylesheetMain = file_get_contents(SugarThemeRegistry::current()->getCSSURL('style.css',false));
        $stylesheetPrint = file_get_contents(SugarThemeRegistry::current()->getCSSURL('print.css',false));
        ob_clean();
        try{
            $pdf=new mPDF('en','A4','','DejaVuSansCondensed');
            $pdf->setAutoFont();
            $pdf->WriteHTML($stylesheetMain,1);
            $pdf->WriteHTML($stylesheetPrint,1);
