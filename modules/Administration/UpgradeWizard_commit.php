...
if ($install_type == 'patch' || $install_type == 'module') {
    switch ($mode) {
        case 'Install':
            ...
            if (is_file($file)) {
                echo "{$mod_strings['LBL_UW_INCLUDING']}: {$file} <br>\n";
                include $file;
                pre_install();
            }
...
switch ($install_type) {
    case 'langpack':
        ...
        if ($mode == 'Install' || $mode == 'Enable') {
            $sugar_config['languages'] = $sugar_config['languages'] + [$_REQUEST['new_lang_name'] => $_REQUEST['new_lang_desc']];
        } else {
...
<?php
echo '<div>';
echo getUITextForType($install_type) . ' ' . getUITextForMode($mode) . ' ' . $mod_strings['LBL_UW_SUCCESSFULLY'];
echo '<br>';
echo '<br>';
echo "<input type=submit value=\"{$mod_strings['LBL_UW_BTN_BACK_TO_MOD_LOADER']}\" /><br>";
echo '</div>';
echo '<br>';
if (isset($lang_changed_string)) {
    echo $lang_changed_string;
}
if ($install_type != 'module' && $install_type != 'langpack') {
    if (count($files_to_handle) > 0) {
        ...
        echo "{$mod_strings['LBL_UW_FOLLOWING_FILES']} {$file_action}:<br>\n";
        echo "<ul id=\"subMenu\">\n";
        foreach ($files_to_handle as $file_to_copy) {
            echo "<li>{$file_to_copy}<br>\n";
        }
        echo "</ul>\n";
        echo '</div>';
    } else {
        if ($mode != 'Disable' && $mode != 'Enable') {
            echo "{$mod_strings['LBL_UW_NO_FILES_SELECTED']} {$file_action}.<br>\n";
        }
    }

    echo $mod_strings['LBL_UW_UPGRADE_SUCCESSFUL'];
    echo "<input class='button' type=submit value=\"{$mod_strings['LBL_UW_BTN_BACK_TO_UW']}\" />\n";
}
?>
</form>
