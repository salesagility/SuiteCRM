...
unset($_SESSION['rebuild_relationships'], $_SESSION['rebuild_extensions']);
...
if (!file_exists($base_tmp_upgrade_dir) && !mkdir(
    $base_tmp_upgrade_dir,
    0755,
    true
) && !is_dir($base_tmp_upgrade_dir)) {
...
if (isset($installdefs, $installdefs['id'])) {
...
$new_files = findAllFilesRelative("{$unzip_dir}/{$zip_from_dir}", []);
...
?>
<form action="<?php echo $form_action . '_commit'; ?>" name="files" method="post"  onSubmit="return validateForm(<?php echo $require_license; ?>);">
<?php
if (empty($new_studio_mod_files)) {
    ...
    foreach ($new_studio_mod_files as $the_file) {
        ...
        echo "<li><input id=\"copy_{$count}\" name=\"copy_{$count}\" type=\"checkbox\" value=\"" . $the_file . '"> ' . $new_file . '</li>';
        $count++;
    }
}
echo '<br>';
...
switch ($mode) {
    case 'Install':
        if ($install_type == 'langpack') {
            echo $mod_strings['LBL_UW_LANGPACK_READY'];
            echo '<br><br>';
        }
    ...
    case 'Uninstall':
        if ($install_type == 'langpack') {
            echo $mod_strings['LBL_UW_LANGPACK_READY_UNISTALL'];
            echo '<br><br>';
        } else {
            if ($install_type != 'module') {
                echo $mod_strings['LBL_UW_FILES_REMOVED'];
            }
        }
    ...
    case 'Disable':
        if ($install_type == 'langpack') {
            echo $mod_strings['LBL_UW_LANGPACK_READY_DISABLE'];
            echo '<br><br>';
        }
    ...
    case 'Enable':
        if ($install_type == 'langpack') {
            echo $mod_strings['LBL_UW_LANGPACK_READY_ENABLE'];
            echo '<br><br>';
        }
...
if ($remove_tables == 'prompt' && $mode == 'Uninstall') {
    echo '<br/><br/>';
    echo "<input type='radio' id='remove_tables_true' name='remove_tables' value='true' checked>" . $mod_strings['ML_LBL_REMOVE_TABLES'] . '&nbsp;';
    echo "<input type='radio' id='remove_tables_false' name='remove_tables' value='false'>" . $mod_strings['ML_LBL_DO_NOT_REMOVE_TABLES'] . '<br>';
}
$count = 0;
    ...
    foreach ($new_sugar_mod_files as $the_file) {
        ...
        if ($mode == 'Install') {
            ...
            if ($checked != '' && $disabled != '') {    // need to put a hidden field
                echo "<input name=\"copy_{$count}\" type=\"hidden\" value=\"" . $the_file . "\">\n";
            }
            echo "<li><input id=\"copy_{$count}\" name=\"copy_{$count}\" type=\"checkbox\" value=\"" . $the_file . "\" {$checked} {$disabled} > " . $highlight_start . $new_file . $highlight_end;
            ...
            if ($mode == 'Uninstall' && file_exists($new_file)) {
                ...
                echo "<li><input name=\"copy_{$count}\" type=\"checkbox\" value=\"" . $the_file . "\" {$checked} {$disabled} > " . $highlight_start . $new_file . $highlight_end . "<br>\n";
            }
        }
        $count++;
    }
...
if ($mode == 'Disable' || $mode == 'Enable') {
    ...
    if (count($modified_files) > 0) {
        ...
        echo '}</script>';
        echo '<b>' . $mod_strings['ML_LBL_OVERWRITE_FILES'] . '</b>';
        echo '<table><td align="left" valign="top" colspan=2>';
        echo "<input type='radio' id='radio_overwrite_files' name='radio_overwrite' value='overwrite'>{$mod_strings['LBL_OVERWRITE_FILES']}&nbsp;";
        echo "<input type='radio' id='radio_do_not_overwrite_files' name='radio_overwrite' value='do_not_overwrite' checked>{$mod_strings['LBL_DO_OVERWRITE_FILES']}";
        echo '</td></tr></table>';
        echo '<ul>';
        foreach ($modified_files as $modified_file) {
            echo '<li>' . $modified_file . '</li>';
        }
        echo '</ul>';
    } else {
        echo '<script>' .
...
?>
    ...
    <input type="hidden" name="copy_count" value="<?php echo $count; ?>"/>
