...
function getSystemInfo($send_usage_info = true)
{
    ...
    if ($send_usage_info) {
        ...
        $info['system_name'] = (!empty($administration->settings['system_name'])) ? substr($administration->settings['system_name'],
            0, 255) : '';


        $result = $db->getOne("select count(*) count from users where status='Active' and deleted=0 and is_admin='1'",
            false, 'fetching admin count');
        if ($result !== false) {
        ...
        if (!$send_usage_info) {
            ...
            $info['users_active_30_days'] = $db->getOne("SELECT count( DISTINCT users.id ) user_count FROM tracker, users WHERE users.id = tracker.user_id AND  tracker.date_modified >= $lastMonth",
                false, 'fetching last 30 users count');
        }
        ...
        if (!$send_usage_info) {
            ...
            $id = $db->getOne("select id from tracker order by date_modified desc", false,
                'fetching most recent tracker entry');
            if ($id !== false) {
...
function check_now($send_usage_info = true, $get_request_data = false, $response_data = false, $from_install = false)
{
    ...
    if (!$response_data) {
        ...
        $sclient = new nusoapclient('https://updates.sugarcrm.com/heartbeat/soap.php', false, false, false, false,
            false, 15, 15);
