...
class ImportDuplicateCheck
{
    ...
    public function isADuplicateRecordByFields($fieldList)
    {
        foreach ($fieldList as $field) {
            if ($field == 'email1' || $field == 'email2') {
                ...
                $email = $field;
                if ($emailAddress->getCountEmailAddressByBean($this->_focus->$email, $this->_focus,
                        ($field == 'email1')) > 0
                ) {
                    return true;
                }
    ...
    public function isADuplicateRecord($indexlist)
    {
        ...
        if (in_array('full_name', $indexlist)) {
            $newfocus = loadBean($this->_focus->module_dir);
            $result = $newfocus->retrieve_by_string_fields(array(
                'deleted' => '0',
                'first_name' => $this->_focus->first_name,
                'last_name' => $this->_focus->last_name
            ), true);
    ...
    public function getDuplicateCheckIndexedFiles()
    {
        ...
        $mstr_exclude_array = array(
            'all' => array('team_set_id', 'id', 'deleted'),
            'contacts' => array('email2'),
            array('leads' => 'reports_to_id'),
            array('prospects' => 'tracker_key')
        );

        //create exclude array from subset of applicable mstr_exclude_array elements
        $exclude_array = isset($mstr_exclude_array[strtolower($this->_focus->module_dir)]) ? array_merge($mstr_exclude_array[strtolower($this->_focus->module_dir)],
            $mstr_exclude_array['all']) : $mstr_exclude_array['all'];
