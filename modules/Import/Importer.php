...
class Importer
{
    ...
    protected function importRow($row)
    {
        ...
        for ($fieldNum = 0; $fieldNum < $_REQUEST['columncount']; $fieldNum++) {
            ...
            $fieldTranslated = translate((isset($fieldDef['vname']) ? $fieldDef['vname'] : $fieldDef['name']),
                    $focus->module_dir) . " (" . $fieldDef['name'] . ")";
            $defaultRowValue = '';
            ...
            if (isset($row[$fieldNum])) {
                $rowValue = $locale->translateCharset(strip_tags(trim($row[$fieldNum])),
            ...
            if (array_key_exists($field,
                    $focus->get_import_required_fields()) && empty($rowValue) && $rowValue != '0'
            ) {
            ...
            if ($focus->object_name == "Contact" && $field == 'sync_contact') {
                ...
                if (true == $rowValue || 'true' == strtolower($rowValue)) {
                    ...
                    if (!$returnValue) {
                        $this->importSource->writeError($mod_strings['LBL_ERROR_SYNC_USERS'], $fieldTranslated,
                            $bad_names);
                        $do_save = 0;
                    } else {
            ...
            if ($field == 'email1' || $field == 'email2') {
                ...
                if ($returnValue === false) {
                    $do_save = 0;
                    $this->importSource->writeError($mod_strings['LBL_ERROR_INVALID_EMAIL'], $fieldTranslated,
                        $rowValue);
                } else {
            ...
            if (!empty($rowValue)) {
                ...
                if ($field == "email_addresses_non_primary" && is_array($rowValue)) {
                    foreach ($rowValue as $tempRow) {
                        $tempRow = $this->sanitizeFieldValueByType($tempRow, $fieldDef, $defaultRowValue, $focus,
                            $fieldTranslated);
                        if ($tempRow === false) {
                            $rowValue = false;
                            $do_save = false;
                    ...
                    $rowValue = $this->sanitizeFieldValueByType($rowValue, $fieldDef, $defaultRowValue, $focus,
                        $fieldTranslated);
                }
        ...
        if (isset($focus->field_defs['parent_name']) && isset($focus->parent_name) && ($focus->field_defs['parent_name']['type'] == 'parent')) {
            ...
            // now validate it
            $returnValue = $this->ifs->parent($focus->parent_name, $focus->field_defs['parent_name'], $focus,
    ...
    protected function sanitizeFieldValueByType($rowValue, $fieldDef, $defaultRowValue, $focus, $fieldTranslated)
    {
        ...
        switch ($fieldDef['type']) {
            ...
            case 'multienum':
                ...
                if ($returnValue === false) {
                    $this->importSource->writeError($mod_strings['LBL_ERROR_NOT_IN_ENUM'] . implode(",",
                            $app_list_strings[$fieldDef['options']]), $fieldTranslated, $rowValue);
            ...
            default:
                ...
                if (!$returnValue) {
                    $this->importSource->writeError($mod_strings['LBL_ERROR_INVALID_' . strtoupper($fieldDef['type'])],
                        $fieldTranslated, $rowValue, $focus);
    ...
    protected function saveImportBean($focus, $newRecord)
    {
        ...
        */
        ...
        if ($focus->object_name == "Contact" && isset($focus->sync_contact)) {
            ...
            if ($focus->object_name == "User" && !empty($current_user) && $focus->is_admin && !is_admin($current_user) && is_admin_for_module($current_user,
                    'Users')
            ) {
        ...
        if ($newRecord) {
            $this->importSource->writeRowToLastImport($_REQUEST['import_module'],
                ($focus->object_name == 'Case' ? 'aCase' : $focus->object_name), $focus->id);
        }
    ...
    protected function saveMappingFile()
    {
        ...
        $result = $mapping_file->save($current_user->id, $_REQUEST['save_map_as'], $_REQUEST['import_module'],
            $_REQUEST['source'],
            (isset($_REQUEST['has_header']) && $_REQUEST['has_header'] == 'on'), $_REQUEST['custom_delimiter'],
    ...
    protected function populateDefaultMapValue($field, $fieldValue, $fieldDef)
    {
        ...
        if ($fieldDef['type'] == 'date' && $this->ifs->dateformat != $timedate->get_date_format()) {
            $defaultRowValue = $timedate->swap_formats($defaultRowValue, $this->ifs->dateformat,
                $timedate->get_date_format());
        }

        if ($fieldDef['type'] == 'time' && $this->ifs->timeformat != $timedate->get_time_format()) {
            $defaultRowValue = $timedate->swap_formats($defaultRowValue, $this->ifs->timeformat,
        ...
        if (($fieldDef['type'] == 'datetime' || $fieldDef['type'] == 'datetimecombo') && $this->ifs->dateformat . ' ' . $this->ifs->timeformat != $timedate->get_date_time_format()) {
            $defaultRowValue = $timedate->swap_formats($defaultRowValue,
                $this->ifs->dateformat . ' ' . $this->ifs->timeformat, $timedate->get_date_time_format());
        }

        if (in_array($fieldDef['type'], array(
                'currency',
                'float',
                'int',
                'num'
            )) && $this->ifs->num_grp_sep != $current_user->getPreference('num_grp_sep')
        ) {
            $defaultRowValue = str_replace($current_user->getPreference('num_grp_sep'), $this->ifs->num_grp_sep,
                $defaultRowValue);
        }

        if (in_array($fieldDef['type'],
                array('currency', 'float')) && $this->ifs->dec_sep != $current_user->getPreference('dec_sep')
        ) {
            $defaultRowValue = str_replace($current_user->getPreference('dec_sep'), $this->ifs->dec_sep,
                $defaultRowValue);
        }
