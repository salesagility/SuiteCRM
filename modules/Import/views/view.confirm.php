...
class ImportViewConfirm extends ImportView
{
    ...
    public function display()
    {
        ...
        if (isset($_FILES['userfile']) && $uploadFile->confirm_upload()) {
            ...
            $this->_showImportError($mod_strings['LBL_IMPORT_MODULE_ERROR_NO_UPLOAD'], $_REQUEST['import_module'],
                'Step2', true, null, true);
        ...
        if (isset($_FILES['userfile']['size']) && $_FILES['userfile']['size'] == 0) {
            ...
            $this->_showImportError($mod_strings['LBL_NO_LINES'], $_REQUEST['import_module'], 'Step2', false, null,
                true);
        ...
        if (isset($_FILES['userfile']['type']) && strpos($_FILES['userfile']['type'],
                'octet-stream') === false && strpos($_FILES['userfile']['type'], 'text') === false
        ...
        $importFile = new ImportFile($uploadFileName, $_REQUEST['custom_delimiter'],
            html_entity_decode($_REQUEST['custom_enclosure'], ENT_QUOTES), false);
        ...
        if (!$importFile->fileExists()) {
            $this->_showImportError($mod_strings['LBL_CANNOT_OPEN'], $_REQUEST['import_module'], 'Step2', false, null,
                true);
        ...
        if (!empty($maxLineCount) && ($lineCount > $maxLineCount)) {
            ...
            $maxRecordsWarningMessg = string_format($mod_strings['LBL_IMPORT_ERROR_MAX_REC_LIMIT_REACHED'],
                array($lineCount, $maxLineCount));
        }
    ...
    private function setImportFileCharacterSet($importFile, $field_map = array())
    {
        ...
        $charsetOptions = get_select_options_with_id($locale->getCharsetSelect(),
            $charset_for_import);//wdong,  bug 25927, here we should use the charset testing results from above.
    ...
    private function setCurrencyOptions($field_map = array())
    {
        ...
        $cur_id = isset($field_map['importlocale_currency']) ? $field_map['importlocale_currency'] : $locale->getPrecedentPreference('currency',
            $current_user);
