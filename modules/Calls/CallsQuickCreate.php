    ...
    function process()
    {
        ...
        if ($this->viaAJAX) { // override for ajax call
            $this->ss->assign('saveOnclick',
                "onclick='if(check_form(\"callsQuickCreate\")) return SUGAR.subpanelUtils.inlineSave(this.form.id, \"activities\"); else return false;'");
            $this->ss->assign('cancelOnclick',
                "onclick='return SUGAR.subpanelUtils.cancelCreate(\"subpanel_activities\")';");
        }
        ...
        if (strpos($time_pref, 'a') || strpos($time_pref, 'A')) {
            ...
            if (strpos($time_pref, 'a')) {
                $this->ss->assign("TIME_MERIDIEM",
                    get_select_options_with_id($options, strpos($focus->time_start, 'a') ? 'am' : 'pm'));
            } else {
                $this->ss->assign("TIME_MERIDIEM",
                    get_select_options_with_id($options, strpos($focus->time_start, 'A') ? 'AM' : 'PM'));
            }
        ...
        $this->ss->assign("TIME_START_MINUTE_OPTIONS",
            get_select_options_with_id($focus->minutes_values, $time_start_minutes));

        $this->ss->assign("DURATION_HOURS", $focus->duration_hours);
        $this->ss->assign("DURATION_MINUTES_OPTIONS",
            get_select_options_with_id($focus->minutes_values, $focus->duration_minutes));
        ...
        $this->ss->assign("DIRECTION_OPTIONS",
            get_select_options_with_id($app_list_strings['call_direction_dom'], $focus->direction));
        $this->ss->assign("STATUS_OPTIONS",
            get_select_options_with_id($app_list_strings['call_status_dom'], $focus->status));
