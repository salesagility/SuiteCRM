...
class SVG
{
    ...
    public $svg_string;        //	String contenant le tracage du svg en lui m�me.
        ...
        if (isset($critere_style['style'])) {
            if (preg_match('/fill:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\)/', $critere_style['style'], $m)) {
                ...
                $tmp = preg_replace("/(.*)fill:\s*([a-z0-9#_()]*|none)(.*)/i", "$2",
                    $critere_style['style']);    // mPDF 4.4.003
    ...
    function svgText()
    {
        ...
        if (isset($this->txt_data[2])) {
            ...
            $path_cmd = sprintf('q BT /F%d %s %.3F Tf %.3F %.3F Td %s Tr %s %s %s Tj ET Q ',
                    $this->mpdf_ref->CurrentFont['i'], $opacitystr, $this->mpdf_ref->FontSizePt, $pdfx * $this->kp,
                    $pdfy * $this->kp, $render, $fillstr, $strokestr, $txt) . "\n";
...
//		$path_cmd .= 'h ';	// mPDF 5.0
            ...
            if (!$found) {
                foreach ($aux_fontlist AS $f) {
                    ...
                    if (in_array($v, $this->mpdf_ref->sans_fonts) || in_array($v,
                            $this->mpdf_ref->serif_fonts) || in_array($v, $this->mpdf_ref->mono_fonts)
                    ) {
                        $current_style['font-family'] = $v;
                        break;
                    }
    ...
    function ImageSVG($data)
    {
        ...
        if (preg_match('/xlink:href=/si', $data)) {
            // Get links
            preg_match_all('/(<(linearGradient|radialgradient)[^>]*)xlink:href=["\']#(.*?)["\'](.*?)\/>/si', $data,
                $links);
            if (count($links[0])) {
                $links[5] = array();
            }    // mPDF 4.5.010
            ...
            // Get targets
            preg_match_all('/<(linearGradient|radialgradient)([^>]*)id=["\'](.*?)["\'](.*?)>(.*?)<\/(linearGradient|radialgradient)>/si',
                $data, $m);
            $targets = array();
            $stops = array();
            // keeping in $targets
            ...
            function xml_svg2pdf_start($parser, $name, $attribs)
            {
                ...
                switch (strtolower($name)) {
                    ...
                    case 'stop':
                        ...
                        if (isset($attribs['style']) AND preg_match('/stop-color:\s*([^;]*)/i', $attribs['style'],
                                $m)
                        ) {
                            $color = trim($m[1]);
                        } else {
                        ...
                        if ($col{0} == 3 || $col{0} == 5) {    // RGB
                            $color_final = sprintf('%.3F %.3F %.3F', ord($col{1}) / 255, ord($col{2}) / 255,
                                ord($col{3}) / 255);
                        ...
                        $stop_opacity = 1;
                        // mPDF 4.4.003
                        if (isset($attribs['style']) AND preg_match('/stop-opacity:\s*([0-9.]*)/i', $attribs['style'],
                                $m)
                        ) {
                            $stop_opacity = $m[1];
                        } else {
