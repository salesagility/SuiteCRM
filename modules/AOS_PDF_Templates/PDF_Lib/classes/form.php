...
// FORMS
    ...
    function print_ob_text($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->SetFormText($w, $h, $objattr['fieldname'], $val, $val, $objattr['title'], $flags, $fieldalign, false,
                $objattr['maxlength'], $js, $objattr['background-col'], $objattr['border-col']);
    ...
    function print_ob_textarea($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->SetFormText($w, $h, $objattr['fieldname'], $texto, $texto, $objattr['title'], $flags, $fieldalign,
                false, -1, $js, $objattr['background-col'], $objattr['border-col']);
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        } else {
    ...
    function print_ob_select($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->mpdf->Cell($w - ($this->mpdf->FontSize * 1.4), $h, $texto, 1, 0, $rtlalign, 1, '', 0,
                ...
                $this->form_element_spacing['select']['inner']['h'] / $k, 'M');
...
// In _putpages
    ...
    function print_ob_imageinput($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->SetJSButton($w, $h, $objattr['fieldname'], $objattr['value'], $js, $objattr['ID'], $objattr['title'],
                $flags, $objattr['Indexed']);
        } else {
...
// In _putannots
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            if ($objattr['subtype'] == 'RESET') {
                ...
                $this->SetFormReset($w, $h, $objattr['fieldname'], $objattr['value'], $objattr['title'], $flags,
                    $objattr['background-col'], $objattr['border-col'], $objattr['noprint']);
            } else {
                if ($objattr['subtype'] == 'SUBMIT') {
                    ...
                    $this->SetFormSubmit($w, $h, $objattr['fieldname'], $objattr['value'], $url, $objattr['title'],
                        $type, $method, $flags, $objattr['background-col'], $objattr['border-col'],
                        $objattr['noprint']);
                } else {
                    if ($objattr['subtype'] == 'BUTTON') {
                        ...
                        $this->SetJSButton($w, $h, $objattr['fieldname'], $objattr['value'], $js, 0, $objattr['title'],
                            $flags, false, $objattr['background-col'], $objattr['border-col'], $objattr['noprint']);
                    }
                }
            }
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
    ...
    function print_ob_radio($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, $x, $y)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->SetRadio($w, $h, $objattr['fieldname'], $objattr['value'], $objattr['title'], $checked, $flags,
                $objattr['disabled']);
            if ($this->formUseZapD) {
                $this->mpdf->SetFont($save_font, '', 0);
                $this->mpdf->currentfontfamily = $save_currentfont;
