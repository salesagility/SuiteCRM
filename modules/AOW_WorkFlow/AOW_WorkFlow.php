...
class AOW_WorkFlow extends Basic
{
    ...
    public function save($check_notify = false)
    {
        if (empty($this->id) || (isset($_POST['duplicateSave']) && $_POST['duplicateSave'] == 'true')) {
            unset($_POST['aow_conditions_id'], $_POST['aow_actions_id']);
        }
    ...
    public function build_flow_query_where($query = [])
    {
        global $beanList;
    ...
    public function build_query_where(AOW_Condition $condition, $module, $query = [])
    {
    ...
    public function check_valid_bean(SugarBean $bean)
    {
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            ...
            $dateFields = ['date', 'datetime', 'datetimecombo'];
            if ($this->isSQLOperator($condition->operator)) {
                ...
                switch ($condition->value_type) {
                    case 'Field':
                        ...
                        break;
                    case 'Any_Change':
                        if ($data['type'] === 'relate' && isset($data['name'], $condition_bean->rel_fields_before_value[$condition->field])
                             ) {
