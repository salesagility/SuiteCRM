...
class AOW_WorkFlowController extends SugarController
{
    protected function action_getModuleFields()
    {
        if (!empty($_REQUEST['aow_module']) && $_REQUEST['aow_module'] != '') {
            ...
            echo getModuleFields($module, $view, $value, [], $override);
        }
        die;
    }
    ...
    protected function action_getModuleOperatorField()
    {
        ...
        if ($vardef) {
            switch ($vardef['type']) {
                ...
                case 'float':
                case 'currency':
                $valid_opp = ['Equal_To', 'Not_Equal_To', 'Greater_Than', 'Less_Than', 'Greater_Than_or_Equal_To', 'Less_Than_or_Equal_To', 'is_null'];
                ...
                case 'enum':
                case 'multienum':
                $valid_opp = ['Equal_To', 'Not_Equal_To', 'is_null'];

                    break;
                default:
                $valid_opp = ['Equal_To', 'Not_Equal_To', 'Contains', 'Starts_With', 'Ends_With', 'is_null'];
    ...
    protected function action_getFieldTypeOptions()
    {
        ...
        switch ($vardef['type']) {
            ...
            case 'float':
            case 'currency':
                $valid_opp = ['Value', 'Field', 'Any_Change'];
            ...
            case 'datetime':
            case 'datetimecombo':
                $valid_opp = ['Value', 'Field', 'Any_Change', 'Date'];

                break;
            case 'enum':
            case 'dynamicenum':
            case 'multienum':
                $valid_opp = ['Value', 'Field', 'Any_Change', 'Multi'];

                break;
            case 'relate':
            case 'id':
                $valid_opp = ['Value', 'Field', 'Any_Change', 'SecurityGroup'];
        ...
        switch ($vardef['type']) {
            ...
            case 'relate':
                ...
                if ($vardef['module'] == 'Users') {
                    $valid_opp = ['Value', 'Field', 'Round_Robin', 'Least_Busy', 'Random'];
                }
