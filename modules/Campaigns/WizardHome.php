...
//account for use within wizards
    if ($focus->campaign_type == 'NewsLetter') {
        echo getClassicModuleTitle($mod_strings['LBL_MODULE_NAME'],
            array($mod_strings['LBL_NEWSLETTER_WIZARD_START_TITLE'] . $focus->name), true, false);
    } else {
        echo getClassicModuleTitle($mod_strings['LBL_MODULE_NAME'],
            array($mod_strings['LBL_CAMPAIGN_WIZARD_START_TITLE'] . $focus->name), true, false);
    }
    ...
    */
    ...
    //account for use within wizards
    echo getClassicModuleTitle($mod_strings['LBL_MODULE_NAME'],
        array($mod_strings['LBL_CAMPAIGN_WIZARD'] . $focus->name), true, false);
    ...
    foreach ($fields as $key) {
        ...
        if (!empty($focus->$key) && !empty($mod_strings[$focus->field_name_map[$key]['vname']])) {
            ...
            if ($key == 'team_name') {
                ...
                $cmpgn_tbl .= "<td scope='row'>" . TeamSetManager::getCommaDelimitedTeams($focus->team_set_id,
                        $focus->team_id, true) . "</td></tr>\n";
            } else {
    ...
    if (count($pl_lists) > 0) {
        ...
        foreach ($pl_lists as $pl_id) {
            ...
            if (isset($pl_focus->id) && !empty($pl_focus->id)) {
                ...
                $pl_tbl .= SugarThemeRegistry::current()->getImage('edit_inline', 'border=0', null, null, ".gif",
                        $mod_strings['LBL_EDIT_INLINE']) . "</a>&nbsp;";
                ...
                $pl_tbl .= SugarThemeRegistry::current()->getImage('view_inline', 'border=0', null, null, ".gif",
                        $mod_strings['LBL_VIEW_INLINE']) . "</a></td>";
