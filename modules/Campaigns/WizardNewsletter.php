...
if ((isset($_REQUEST['wizardtype']) && $_REQUEST['wizardtype'] ==1) || ($focus->campaign_type=='NewsLetter')) {
    ...
    if (count($prospect_lists)>0) {
        foreach ($prospect_lists as $pl_id) {
            ...
            $trgt_html .= "> <input id='existing_target_name" . $trgt_count ."' type='hidden' type='text' size='60' maxlength='255' name='existing_target_name". $trgt_count ."'  value='". ($pl_focus?$pl_focus->name:'-')."' ><a href=\"index.php?module=ProspectLists&action=DetailView&record=" . $pl_focus->id . "\" target=\"_blank\" title=\"" . $mod_strings['LBL_OPEN_IN_NEW_WINDOW'] . "\">". ($pl_focus?$pl_focus->name:'-')."</a></td>";
            $trgt_html .= ">" .($pl_focus?$pl_focus->get_entry_count():'-')."</td>";
            $trgt_html .= "><input type='hidden' size='60' maxlength='255' name='existing_tracker_list_type" . $trgt_count ."'   id='existing_tracker_list_type". $trgt_count ."' value='".$pl_focus->list_type."' >".$app_list_strings['prospect_list_type_dom'][$pl_focus->list_type];
