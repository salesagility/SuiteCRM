...
function checkBouncedEmailForIdentifier($email_description)
{
    ...
    if (preg_match('/X-CampTrackID: [a-z0-9\-]*/', $email_description, $matches)) {
        ...
        if (preg_match('/index.php\?entryPoint=removeme&identifier=[a-z0-9\-]*/', $email_description, $matches)) {
            $identifiers = preg_split('/index.php\?entryPoint=removeme&identifier=/', $matches[0], -1,
                PREG_SPLIT_NO_EMPTY);
            $found = true;
            $GLOBALS['log']->debug("Found campaign identifier in body of email");
        } else {
            if (preg_match('/index.php\?entryPoint=addme&identifier=[a-z0-9\-]*/', $email_description, $matches)) {
                $identifiers = preg_split('/index.php\?entryPoint=addme&identifier=/', $matches[0], -1,
                    PREG_SPLIT_NO_EMPTY);
                $found = true;
                $GLOBALS['log']->debug("Found campaign identifier in body of email");
            }
