...
function get_campaign_mailboxes(&$emails, $get_name = true)
{
    ...
    while (($row = $db->fetchByAssoc($result)) != null) {
        if ($get_name) {
            ...
            $return_array[$row['id']] = InboundEmail::get_stored_options_static('from_name', $row['name'],
                $row['stored_options']);
        }
        $emails[$row['id']] = InboundEmail::get_stored_options_static('from_addr', 'nobody@example.com',
            $row['stored_options']);
    }
...
function log_campaign_activity($identifier, $activity, $update = true, $clicked_url_key = null)
{
    ...
    if ($row == null) {
        ...
        if (!($identifier && $activity == 'added')) {
            $query .= " and activity_type='targeted'";
        }
...
function get_subscription_lists($focus, $descriptions = false)
{
    ...
    foreach ($news_type_list_arr as $news_list) {
        ...
        if (($match == 'false') && (strpos($news_list['list_type'],
                    'exempt') === false) && (!array_key_exists($news_list['name'], $subs_arr))
        ) {
            $unsubs_arr[$news_list['name']] = "prospect_list@" . $news_list['prospect_list_id'] . "@campaign@" . $news_list['campaign_id'];
        }
...
function get_subscription_lists_keyed($focus)
{
    ...
    foreach ($news_type_list_arr as $news_list) {
        ...
        $news_list_data = array(
            'prospect_list_id' => $news_list['prospect_list_id'],
