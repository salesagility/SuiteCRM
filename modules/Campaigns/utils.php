 ...
 *
 */
function get_message_scope_dom($campaign_id, $campaign_name, $db = null, $mod_strings = [])
{
    if (empty($db)) {
 ...
 *
 * @param mixed $campaign_id
 * @param mixed $parent_bean
 * @param mixed $child_bean
 ...
 *
 * @param mixed $focus
 * @param null|mixed $additional_fields
 */
...
function process_subscriptions($subscription_string_to_parse)
{
     ...
     */
    function diagnose(&$errors = [], &$links = [])
    {
        global $mod_strings;
        global $current_user;
...
function filterFieldsFromBeans($beans)
{
    ...
    foreach ($beans as $b) {
        ...
        if (isset($b->field_defs['webtolead_email1'], $b->field_defs['email1'], $b->field_defs['email1']['required'])) {
        ...
        foreach ($b->field_defs as $field_def) {
            ...
            if (isset($field_def['studio'], $field_def['studio']['editview']) && $field_def['studio']['editview'] === false) {
                continue;
            }
