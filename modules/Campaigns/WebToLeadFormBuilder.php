...
HTML;
    ...
    {
        return '<label>' . $fieldLabel . ($fieldRequired ? "<span class='required'>{$webRequiredSymbol}</span>" : '') . '</label>';
    }
    ...
    private static function getFieldEnumMultiSelectHTML($fieldName, $leadOptions, $fieldRequired)
    {
        ...
        return "<select id=\"{$fieldName}\" multiple=\"true\" name=\"{$fieldName}[]\" tabindex=\"1\"{$_required}>{$leadOptions}</select>";
    }
    ...
    private static function getFieldEnumSelectHTML($fieldName, $leadOptions, $fieldRequired)
    {
        ...
        return "<select id=\"{$fieldName}\" name=\"{$fieldName}\" tabindex=\"1\"{$_required}>{$leadOptions}</select>";
    }
    ...
    private static function getOptInCheckboxHTML($fieldName)
    {
        ...
        return $tpl->fetch('modules/Campaigns/WebToLeadFormBuilderOptInCheckbox.tpl');
    }
    ...
    private static function getArrayOfFieldInfo($lead, $colsField, &$requiredFields)
    {
        ...
        if ($colsField == 'webtolead_email1' && isset($lead->field_defs['email1'], $lead->field_defs['email1']['required'])) {
        ...
        return [$field_name, $field_label, $field_type, $field_required, $field_options];
    }
}
