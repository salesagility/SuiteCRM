...
if (isset($focus->campaign_type) && $focus->campaign_type == 'NewsLetter') {
    echo getClassicModuleTitle($mod_strings['LBL_MODULE_NAME'], [$mod_strings['LBL_NEWSLETTER'], $focus->name], true);
} else {
...
if ($deletedCampaignLogLeadsCount > 0) {
    $subpanel->subpanel_definitions->layout_defs['subpanel_setup']['lead']['top_buttons'][] = [
        'widget_class' => 'SubPanelTopMessage',
        'message' => string_format($mod_strings['LBL_LEADS_DELETED_SINCE_CREATED'], [$deletedCampaignLogLeadsCount]),
    ];
}
