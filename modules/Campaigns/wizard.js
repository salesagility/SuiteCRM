...
if(typeof campaignBudget!='undefined'&&campaignBudget){finish_button.style.display='';}
if($('[name="record"]').val()==""){$("#wiz_submit_button").hide();}}else{if(current_step.value<1){back_button_div.style.display='none';}else{back_button_div.style.display='';}
...
$.post($('#wizform').attr('action'),$('#wizform').serialize(),function(data){var re=/{"record":"\w{1,}-\w{1,}-\w{1,}-\w{1,}-\w{1,}"}/g;var found=data.match(re);if(found==null){console.log('Error getting record id');}else{var response=jQuery.parseJSON(found[0]);$('input[name="record"]').val(response.record);$('input[name="campaign_id"]').val(response.record);$('input[name="action"]').val('WizardTargetListSave');$("#wiz_submit_button").show();}});}
