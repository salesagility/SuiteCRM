...
return true;}
var onEmailTemplateChange=function(elem,namePrefixCopyOf){var emailTemplateId=$(elem).val();$('#email_template_view_html').html('');$('#email_template_view').html('');$.post('index.php?entryPoint=emailTemplateData',{'emailTemplateId':emailTemplateId},function(resp){var results=JSON.parse(resp);$('#email_template_view_html').html(results.data.body_html);$('#email_template_view').html(results.data.body);var htmlCode=$('<textarea />').html(results.data.body_html).text();$('#email_template_editor').html(htmlCode);$('#email_template_editor').mozaik(window.mozaikSettings.email_template_editor);$('#template_name').val(namePrefixCopyOf+results.data.name);$('#template_subject').val(results.data.subject);});};var onScheduleClick=function(e){$('input[name="action"]').val('WizardMarketingSave');$('input[name="module"]').val('Campaigns');$('#show_wizard_summary').val('1');$('#sendMarketingEmailSchedule').val('1');$('#wizform').submit();};var onSendAsTestClick=function(e,campaignId,marketingId){$('input[name="action"]').val('WizardMarketingSave');$('input[name="module"]').val('Campaigns');$('#show_wizard_summary').val('1');$('#sendMarketingEmailTest').val('1');$('#wizform').submit();};var addTargetListData=function(id){var result_data={"form_name":'wizform',"name_to_value_array":{popup_target_list_id:id,popup_target_list_name:targetListDataJSON[id].name,popup_target_list_type:targetListDataJSON[id].type},"passthru_data":Object(),"popupConfirm":0};set_return_prospect_list(result_data);};