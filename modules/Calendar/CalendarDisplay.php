...
class CalendarDisplay
{
     ...
     * @param array $views
     */
    public function __construct(Calendar $cal, $dashlet_id = '', $views = [])
    {
        global $sugar_config;
    ...
    public function display()
    {
        ...
        if ($this->cal->enable_repeat) {
            ...
            for ($i = $fdow; $i < $fdow + 7; $i++) {
                $day_index = $i % 7;
                $dow[] = ['index' => $day_index, 'label' => $GLOBALS['app_list_strings']['dom_cal_day_short'][$day_index + 1]];
            }
            $ss->assign('dow', $dow);
        }
        echo $ss->fetch($main);
     ...
     * display header.
     *
     * @param bool $controls display ui contol itmes
     */
    ...
    protected function load_settings_template(&$ss)
    {
        ...
        $TIME_START_MINUTES_OPTIONS = get_select_options_with_id(['0' => '00', '15' => '15', '30' => '30', '45' => '45'], $d_start_min);
        $TIME_END_HOUR_OPTIONS = get_select_options_with_id($hours_arr, $d_end_hour);
        $TIME_END_MINUTES_OPTIONS = get_select_options_with_id(['0' => '00', '15' => '15', '30' => '30', '45' => '45'], $d_end_min);
