...
class Calendar
{
    public $activityList = ['FP_events' => ['showCompleted' => true, 'start' => 'date_start', 'end' => 'date_end'],
    ...
    public $views = ['agendaDay' => [], 'basicDay' => [], 'basicWeek' => [], 'agendaWeek' => [], 'month' => [], 'sharedMonth' => [], 'sharedWeek' => []];
    ...
    public $acts_arr = []; // Array of activities objects
    public $items = []; // Array of activities data to be displayed
    public $shared_ids = []; // ids of users for shared view
    ...
    /**
     * @var bool whether is print mode
     */
    private $print = false;
     ...
     * @param array $views
     */
    public function __construct($view = 'agendaWeek', $time_arr = [])
    {
        ...
        if (!empty($_REQUEST['year'])) {
            if ($_REQUEST['year'] > 2037 || $_REQUEST['year'] < 1970) {
                ...
                echo $calendarStrings['ERR_YEAR_BETWEEN'];
                exit;
            }
        ...
        foreach ($this->acts_arr as $user_id => $acts) {
            if (isset($acts) && empty($acts)) {
                ...
                if ($shared_calendar_separate) {
                    ...
                    $this->items[$GLOBALS['current_user']->id][] = [];
                }
    ...
    public function init_shared()
    {
        ...
        if (!empty($user_ids) && count($user_ids) != 0 && !isset($_REQUEST['shared_ids'])) {
            ...
            if (isset($_REQUEST['shared_ids']) && count($_REQUEST['shared_ids']) > 0) {
                ...
                $this->shared_ids = [$current_user->id];
            }
        }
    }
