...
class CalendarActivity
{
     ...
     * @return string
     */
    public static function getOccursWithinWhereClause(
        $table_name,
        $rel_table,
        $start_ts_obj,
        $end_ts_obj,
     ...
     * @return string
     */
    public static function getOccursUntilWhereClause(
        $table_name,
        $rel_table,
        $start_ts_obj,
        $end_ts_obj,
    ...
    public static function getFreebusyActivities($user_focus, $start_date_time, $end_date_time)
    {
        $act_list = array();
        $vcal_focus = new vCal();
    ...
    public static function getActivities(
        ...
        foreach ($activities as $key => $activity) {
            ...
            if (ACLController::checkAccess($key, 'list', true)) {
                ...
                $where = self::getOccursUntilWhereClause(
                    $bean->table_name,
                ...
                foreach ($focus_list as $focusBean) {
                    ...
                    $show_as_busy =
                        !(ACLController::checkAccess($key, 'list', $current_user->id == $user_id, 'module', $in_group));
    ...
    protected static function getOccursWhereClauseGeneral(
        ...
        if (function_exists($callback)) {
            ...
            $GLOBALS['log']->warn(" call_user_func() expects parameter 1 to be a valid callback,"
                                  ." function '{$callback}' not found or invalid function name "
                                  ." in modules/Calendar/CalendarActivity.php on line 318 ");
        }


        if ($rel_table != '' && function_exists($callback)) {
            $where .= " AND $rel_table.accept_status != 'decline'";
        } elseif ($rel_table != '' && !function_exists($callback)) {
            $where .= "$rel_table.accept_status != 'decline'";
        }
    ...
    {
        return "($field_date >= $start_day AND $field_date < $end_day)"
               ."OR ($field_date < $start_day AND $field_date_end > $start_day)";
    }
}
