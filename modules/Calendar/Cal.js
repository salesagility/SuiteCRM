...
CAL.reset_edit_dialog = function() {
  var e;
  document.forms["CalendarEditView"].elements["current_module"].value =
    "Meetings";
...
CAL.fill_repeat_data = function() {
  if (
    CAL.enable_repeat &&
    (CAL.get("current_module").value == "Meetings" ||
      CAL.get("current_module").value == "Calls")
  ) {
    if (
      (repeat_type = document.forms["CalendarRepeatForm"].repeat_type.value)
    ) {
      document.forms["CalendarEditView"].repeat_type.value = repeat_type;
      document.forms["CalendarEditView"].repeat_interval.value =
        document.forms["CalendarRepeatForm"].repeat_interval.value;
      if (document.getElementById("repeat_count_radio").checked) {
        document.forms["CalendarEditView"].repeat_count.value =
          document.forms["CalendarRepeatForm"].repeat_count.value;
        document.forms["CalendarEditView"].repeat_until.value = "";
      } else {
        document.forms["CalendarEditView"].repeat_until.value =
          document.forms["CalendarRepeatForm"].repeat_until.value;
        document.forms["CalendarEditView"].repeat_count.value = "";
      }
      if (repeat_type == "Weekly") {
        var repeat_dow = "";
        for (var i = 0; i < 7; i++)
          if (CAL.get("repeat_dow_" + i).checked) repeat_dow += i.toString();
...
CAL.fill_repeat_tab = function(data) {
  ...
  if (
    typeof data.default_repeat_until != "undefined" &&
    set_default_repeat_until
  )
...
CAL.GR_update_focus = function(module, record) {
  if (record == "") {
    ...
    var url =
      "index.php?module=Calendar&action=GetGR&sugar_body_only=true&type=" +
      module +
      "&record=" +
      record;
    ...
    var callback = {
      success: function(o) {
        ...
        if (res.access == "yes") {
          ...
          CAL.script_evaled = false;
          fc.innerHTML =
            '<script type="text/javascript">CAL.script_evaled = true;</script>' +
            res.html;
...
CAL.change_activity_type = function(mod_name) {
  if (typeof CAL.current_params.module_name != "undefined")
    if (CAL.current_params.module_name == mod_name) return;
  var e, user_name, user_id, date_start;
  CAL.get("title-cal-edit").innerHTML = CAL.lbl_loading;
  document.forms["CalendarEditView"].elements[
    "current_module"
  ].value = mod_name;
...
CAL.dialog_save = function() {
  ...
  var callback = {
    success: function(o) {
      ...
      if (res.access == "yes") {
        ...
        if (
          res["duration_hours"] % 24 === 0 &&
          res["time_start"] == "12:00am"
        ) {
        ...
        if (res["repeat"]) {
          $.each(res["repeat"], function(key, value) {
            ...
            newEvent.start = new Date(
              moment.unix(value["ts_start"]).format("MM/DD/YYYY") +
                " " +
                moment(res["time_start"], "hh:mma").format("HH:mm")
            );
            newEvent.end = moment(
  ...
  var url =
    "index.php?module=Calendar&action=SaveActivity&sugar_body_only=true";
...
CAL.dialog_remove = function() {
  ...
  var callback = {
    success: function(o) {
      $("#calendar" + global_current_user_id).fullCalendar(
        "removeEvents",
        CAL.deleted_id
      );
    },
...
CAL.refresh = function() {
  ...
  var url =
    "index.php?module=Calendar&action=getActivities&sugar_body_only=true";
  YAHOO.util.Connect.asyncRequest("POST", url, callback, CAL.toURI(data));
  CAL.clear();
};
CAL.clear = function() {
...
CAL.goto_date_call = function() {
  ...
  window.location.href =
    "index.php?module=Calendar&view=" +
    CAL.view +
    "&day=" +
    date_arr[1] +
    "&month=" +
...
CAL.update_vcal = function() {
  ...
  var callback = {
    success: function(result) {
      ...
      GLOBAL_REGISTRY.freebusy[v] = SugarVCalClient.parseResults(
        result.responseText,
        false
      );
      GLOBAL_REGISTRY.freebusy_adjusted[v] = SugarVCalClient.parseResults(
        result.responseText,
        true
      );
      SugarWidgetScheduler.update_time();
  ...
  $.each(calendar_items, function(user_id, user_calendar_activities) {
    ...
    $.each(user_calendar_activities, function(index, element) {
      ...
      if (undefined !== global_colorList[element.module_name]) {
        valueToPush["backgroundColor"] =
          "#" + global_colorList[element.module_name].body;
        valueToPush["borderColor"] =
          "#" + global_colorList[element.module_name].border;
        valueToPush["textColor"] =
          "#" + global_colorList[element.module_name].text;
      }
      if (
        element["duration_hours"] % 24 === 0 &&
        (element["time_start"] == "12:00am" || element["time_start"] == "00:00")
      ) {
      ...
      .fullCalendar({
        ...
        select: function(date, jsEvent, view) {
          if (global_edit == true) {
            ...
            if (
              $(view.target).hasClass("fc-day-top") &&
              date_duration <= 86400000
            ) {
        ...
        eventDrop: function(event, delta, revertFunc) {
          ...
          if (event.allDay == true) {
            data.allDay = true;
            data.enddatetime = event.start
              .add(1, "days")
              .format(global_datetime_format);
          }
          var url =
            "index.php?module=Calendar&action=Reschedule&sugar_body_only=true";
        ...
        navLinkDayClick: function(weekStart, jsEvent) {
          if (global_edit == true) {
            if ($(jsEvent.currentTarget).hasClass("fc-day-number")) {
              var dateStr = $(jsEvent.currentTarget)
                .closest(".fc-day-top")
                .attr("data-date");
              var dateMoment = new moment(dateStr);
              var url =
        ...
        eventResize: function(event, delta, revertFunc) {
          var url =
            "index.php?module=Calendar&action=Resize&sugar_body_only=true";
        ...
        eventRender: function(event, element) {
          ...
          var title =
            ...
            "</div>";
          var body = SUGAR.language.translate(
            "app_strings",
            "LBL_LOADING_PAGE"
          );
          if (
