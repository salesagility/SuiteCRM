    ...
    if (!empty($rModule) && $rModule != 'MailMerge') {
        ...
        foreach ($_SESSION['MAILMERGE_RECORD'] as $record_id) {
            if ($rModule == 'Campaigns') {
                ...
                $prospect_module_list = ['leads', 'contacts', 'prospects', 'users'];
                foreach ($prospect_module_list as $mname) {
                    $pList = $prospect->retrieveTargetList("campaigns.id = '{$record_id}' AND related_type = #{$mname}#", ['id', 'first_name', 'last_name']);
