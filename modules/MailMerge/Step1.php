        ...
        foreach ($_SESSION['MAILMERGE_RECORD'] as $record_id) {
            if ($rModule == 'Campaigns') {
                ...
                foreach ($prospect_module_list as $mname) {
                    $pList = $prospect->retrieveTargetList("campaigns.id = '$record_id' AND related_type = #$mname#",
...
if (isset($_SESSION['MAILMERGE_MODULE'])) {
    ...
    $xtpl->assign("MAILMERGE_NUM_SELECTED_OBJECTS",
        count($_SESSION['MAILMERGE_RECORD']) . " " . $_SESSION['MAILMERGE_MODULE'] . " Selected");
} else {
