...
class UserPreference extends SugarBean
{
    ...
    public function reloadPreferences($category = 'global')
    {
        ...
        $result = $GLOBALS['db']->query("SELECT contents FROM user_preferences WHERE assigned_user_id='$user->id' AND category = '" . $category . "' AND deleted = 0",
    ...
    public function resetPreferences(
        ...
        if ($category) {
            ...
            if (!empty($_COOKIE['sugar_user_theme']) && !headers_sent()) {
                setcookie('sugar_user_theme', '', time() - 3600, null, null, false,
                    true); // expire the sugar_user_theme cookie
            }
