...
class UserDemoData
{
    ...
    function create_demo_data()
    {
        ...
        foreach ($sugar_demodata['users'] as $v) {
            $this->_create_seed_user($v['id'], $v['last_name'], $v['first_name'], $v['user_name'], $v['title'],
                $v['is_admin'], $v['reports_to'], $v['reports_to_name'], $v['email']);
    ...
    function _create_seed_user(
        ...
        // bug 15371 tyoung set a user preference so that Users/DetailView.php can
        // find something without repeatedly querying the db in vain
    ...
    function _quick_create_user($name)
    {
        ...
        if (!$this->_user->retrieve($name . '_id')) {
            $this->_create_seed_user("{$name}_id", $name, $name, $name,
                $sugar_demodata['users'][0]['title'], $sugar_demodata['users'][0]['is_admin'], "seed_jim_id",
