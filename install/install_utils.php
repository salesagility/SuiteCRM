...
function commitLanguagePack($uninstall = false)
{
    ...
    if ($uninstall == false && isset($_SESSION['INSTALLED_LANG_PACKS']) && in_array($zipFile,
    ...
    if (isset($_SESSION['INSTALLED_LANG_PACKS']) && !empty($_SESSION['INSTALLED_LANG_PACKS'])) {
        if (count($_SESSION['INSTALLED_LANG_PACKS'] > 0)) {
            foreach ($_SESSION['INSTALLED_LANG_PACKS'] as $file) {
                ...
                $deletePackage = getPackButton('uninstall', $target_manifest, $file, $next_step, $uninstallable,
                    $showButtons);
                //$ret .="<table width='100%' cellpadding='0' cellspacing='0' border='0'>";
                $ret .= "<tr>";
...
{
    return DBManagerFactory::getTypeInstance($_SESSION['setup_db_type'],
        array("db_manager" => $_SESSION['setup_db_manager']));
}
...
function handleDbCreateSugarUser()
{
    ...
    $db = getDbConnection();
    $db->createDbUser($setup_db_database_name, $setup_site_host_name, $setup_db_sugarsales_user,
...
{
    return PackageManagerDisplay::getLicenseDisplay($license_file, 'install.php', $next_step, $zipFile, $type,
        $manifest, $clean_file);
}
    ...
    foreach ($files as $file) {
        ...
        if ($manifest['type'] != 'module' || 0 == sizeof($md5_matches)) {
            ...
            if (is_file($license_file)) {
                //rrs
                $ret .= "<tr><td colspan=6>";
                $ret .= getLicenseDisplay('commit', $target_manifest, $file, $next_step, $license_file,
                    $clean_field_name);
                $ret .= "</td></tr>";
