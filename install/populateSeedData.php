...
$possible_duration_hours_arr = [0, 1, 2, 3];
$possible_duration_minutes_arr = ['00' => '00', '15' => '15', '30' => '30', '45' => '45'];
$account_ids = [];
$accounts = [];
$opportunity_ids = [];
...
for ($i = 0; $i < $number_companies; $i++) {
    ...
    $amount = ['10000', '25000', '50000', '75000'];
    $key = array_rand($amount);
    $opp->amount = $amount[$key];
    $probability = ['10', '70', '40', '60'];
    ...
    $opp->set_relationship('accounts_opportunities', ['opportunity_id' => $opp->id, 'account_id' => $account->id], false);
}
...
for ($i = 0; $i < $number_contacts; $i++) {
    ...
    $contact->set_relationship('accounts_contacts', ['contact_id' => $contact->id, 'account_id' => $account_id], false);
    // This assumes that there will be one opportunity per company in the seed data.
    $opportunity_key = array_rand($opportunity_ids);
    $contact->set_relationship('opportunities_contacts', ['contact_id' => $contact->id, 'opportunity_id' => $opportunity_ids[$opportunity_key], 'contact_role' => $app_list_strings['opportunity_relationship_type_default_key']], false);
