...
SugarWidgetSchedulerSearch.prototype.display = function () {
  ...
  create_invitees.setAttribute('class', 'schedulerInvitees');
  ...
  var invitableModules = ['Contacts', 'Users'];

  invitableModules.forEach(function (moduleName) {
    var button = document.createElement("button");
    button.setAttribute('class', 'create_invitee_button');
    button.setAttribute('type', 'button');
    button.setAttribute('onclick', "SugarWidgetSchedulerAttendees.openSelectorPopup('" + moduleName + "')");
    button.innerHTML = SUGAR.language.translate('app', 'LBL_' + moduleName.toUpperCase());
    create_invitees_buttons.appendChild(button);
  });
...
SugarWidgetSchedulerAttendees.form_add_attendees_from_json_data = function (data) {
  data.list.forEach(function (element) {
    if (typeof(GLOBAL_REGISTRY.focus.users_arr_hash[element.fields.id]) == 'undefined') {
      GLOBAL_REGISTRY.focus.users_arr.push(element);
    }
  })
  GLOBAL_REGISTRY.scheduler_attendees_obj.display();
}

SugarWidgetSchedulerAttendees.form_add_attendees_from_selection_list = function (data) {

  var conditions = [];
  var groupBy = "or";
  if (typeof data.name_to_value_array !== 'undefined') {

    // a single contact was selected
    conditions.push({"name": "id", "op": "like_custom", "value": data.name_to_value_array['subpanel_id']});
    var groupBy = "and";

  } else if (typeof data.selection_list !== 'undefined') {

    // a list of contacts was selected
    for (var property in data.selection_list) {
      if (data.selection_list.hasOwnProperty(property)) {
        conditions.push({"name": "id", "op": "like_custom", "value": data.selection_list[property]});
      }
    }
  }
  ...
  var requestId = global_rpcClient.call_method('query', query);
  global_request_registry[requestId] = [SugarWidgetSchedulerAttendees, 'form_add_attendees_from_json_data'];
}
